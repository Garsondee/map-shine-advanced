...
### 9g. Vegetation joins the depth authority (2026-08-04, same day) β€” BUILT, not yet live

**The problem, found by reading `vt-pan-viewer.js`'s real code, not assumed:** a Case-2 tree/bush overlay (`Vegetation.md`'s mode 2 β€” a plain tile/background with a discovered `_Tree`/`_Bush` sibling file) has no `id`, no `key`, and no membership in `foundry/scene-layers.js`'s collection functions β€” it lives ONLY as `state.vegetationOverlays[kindId]` on its HOST item's own residency-state object. `buildItems(floorIndex)` (the real closure is `boot.js`'s own, injected into `vt-pan-viewer.js`) therefore never produces an entry for it, `depthAuthority.rebuild()` never ranks it, and `rebuildSceneDepthProxies` skips it by construction (its own `state.wholeImage.tiles` check finds nothing, because the overlay's real geometry/texture live under the sibling key `state.vegetationOverlays`, not that one) β€” exactly the gap Β§9d/Β§9f both already named honestly rather than silently.

**Why a REAL, distinct rank β€” not `rankOfElevation`'s query path.** `rankOfElevation`/`rankOfKey` deliberately return an EXISTING real item's own rank (the nearest one at-or-below) β€” load-bearing for a query like "is anything above this light," wrong for a MESH that needs its own Z. Borrowing an existing item's rank for a vegetation proxy's own depth-pass Z would draw it at the EXACT same Z as whatever item it borrowed from; `buf:scene.depth`'s writer is `LessDepth` with no blending, so an exact tie means only whichever mesh renders first actually writes β€” order-dependent, unstable frame to frame. Vegetation needed a genuine, distinct member of the sorted array, not a borrowed number.

**The fix β€” `effects/vegetation-render.js#buildVegetationDepthItems(items, floors, urlByItemId)`,** a new pure, Node-tested function (mirroring this file's own `vegetationOverlayRenderOrder`, which already solves an adjacent-but-different problem β€” THREE's own paint order β€” using the identical floor-band/elevation math). For every host with a discovered `_Tree`/`_Bush` sibling AND a bounded floor band, it synthesizes one item per (host, kind): `{id: 'veg:<hostId>:<kindId>', key: makeLayerKey({elevation: vegetationPassiveElevation(kind, band), sortLayer: SORT_LAYERS.SCENE_EFFECTS}), kind: 'vegetationOverlay', levelId: hostFloorId, vegHostItemId, vegKindId}`. An unbounded band produces nothing for that pair β€” a named gap, never a guessed elevation, same posture `vegetationPassiveElevation` already takes for its existing consumer.

**⚠️ `levelId` is the HOST's own resolved floor, never left to re-derive from the synthetic elevation.** A tree (`passiveElevationFraction = 1`) sits EXACTLY at its floor's own `elevationTop` β€” precisely [[feedback_half_open_band_excludes_its_own_member]]'s own case: `resolveElevationFloorIndex`'s half-open `[bottom,top)` band would attribute that exact value to the FLOOR ABOVE. Setting `levelId` explicitly routes `resolveSceneDepthFloorIndex`'s "membership first" branch around the half-open lookup entirely, the same guard that function's own doc already names for a Level's own foreground.

**Wiring, `vt-pan-viewer.js#updateResidencyUnguarded` β€” a SECOND `depthAuthority.rebuild()` call, deliberately not a mutation of the array `stampVegetationRenderOrders` and the per-item build loop already consumed.** Those two existing, already-shipped consumers' own "how many REAL items sort below" logic must never see these synthetic entries, or every overlay's working THREE `renderOrder` would shift by however many vegetation items happen to precede it β€” a regression this round has no reason to risk. So: the FIRST `rebuild()` (unchanged) feeds `stampVegetationRenderOrders` and the per-item loop (which refreshes `state.vegetationOverlays` for THIS pass); only AFTER that, `buildVegetationDepthItems(items, floors, getVegetationRenderState().urlByItemId)` runs and, if it found anything, a SECOND `rebuild([...items, ...vegItems])` produces the array `rebuildSceneDepthProxies` actually consumes β€” which is also the table every subsequent PER-FRAME query (a point light's own `resolveExpectedDepth`) reads until the next residency pass, so lights correctly rank against vegetation canopies too, not just against real Foundry documents.

**`rebuildSceneDepthProxies` gained a second branch**, keyed on `item.kind === 'vegetationOverlay'`: look up `itemStates.get(vegHostItemId)?.vegetationOverlays?.[vegKindId]`, skip unless `status === 'ready'` with a visible mesh, else build a depth-writer proxy from that record's own `{geometry, tex}` exactly like the existing tile loop does β€” one proxy per overlay (never an array of tiles, unlike a host). No flags set (a canopy does not restrict light in this round β€” scoped to vegetation's own rank, not vegetation-as-occluder, a deliberate, named deferral).

**Proof, `effects/__tests__/vegetation-render.test.mjs`:** unit coverage for `buildVegetationDepthItems`'s own contract (unbounded band skipped, no-URL skipped, stable id, `levelId` inheritance), AND an end-to-end proof against the REAL `scene/depth-authority.js#createDepthAuthority` β€” not a hand-rolled comparison β€” confirming a floor's own tree outranks that SAME floor's own bush, a HIGHER floor's bush still outranks a LOWER floor's tree (floor identity dominates, the exact property point-light occlusion already relies on), and a tree outranks the very host tile it grows on. Full project suite: **7395 passed, 0 failed**, structure gates unchanged (29 rules, 9 ratcheted).

**What is deliberately still NOT true.** Not run against a real Foundry session β€” this is `BUILT (unverified)`, the same honest limit every prior round in this document has named, and this one has NOT yet had the author's own eyes on a real scene with real `_Tree`/`_Bush` art. Vegetation does not (yet) restrict light β€” only its own rank participates. Case-1 self-vegetation (a tile whose own texture IS `_Tree`/`_Bush`-suffixed) needed no changes at all β€” it already flows through the ordinary item pipeline and was never part of this gap.
...
**⚠️ ROUND 2 (2026-08-05, same day) — THE FIX ABOVE WAS REAL BUT INSUFFICIENT.** Author, re-tested: *"Not fixed I'm afraid... when panning they rapidly oscillate between illuminated and not."* — and a crucial new detail: correctly dark at night when a light is below them, ONLY unstable while panning. That pointed away from a same-kind tiebreak collision (which would show as an occasional RELATIVE rank swap, not rapid on/off oscillation of a SPECIFIC bush against a SPECIFIC light) and toward something that recurs on every single residency pass, every time.

**The real root cause, found by reading `updateResidencyUnguarded`'s actual control flow, not assumed:** it is `async`, and it genuinely suspends — `states.push([item, await ensureItemLoaded(item)])`, inside its own PHASE 1 loop, over every item in the draw list. The render loop's own `requestAnimationFrame` keeps firing DURING that suspension (this is required for smooth panning — loading must not freeze rendering), calling `pointLights.update()` → `resolveExpectedDepth` → `depthAuthority.rankOfElevation(...)` every frame, against WHATEVER `depthAuthority`'s internal table happens to hold AT THAT INSTANT. The vegetation-inclusive second `rebuild()` call from §9g's own original code was placed AFTER phases 1 and 2 — i.e. AFTER the real async loading — which means `depthAuthority` held the REAL-ITEMS-ONLY table (published by the FIRST rebuild, vegetation absent entirely) for the ENTIRE async loading window of EVERY residency pass. Panning triggers residency passes constantly, each one real async work spanning several rendered frames. A light's occlusion query landing inside that window found no vegetation rank at all and passed straight through (illuminated, wrong); once the pass finished and the second rebuild finally ran, vegetation reappeared and correctly occluded again (dark, right) — repeating every single pass, which during continuous panning is exactly "rapidly oscillate."

**Why this was placed late in the first place — a false assumption, not a considered tradeoff:** the original comment reasoned "`state.vegetationOverlays` is fresh for THIS pass... so this is the first point synthesis CAN run." That is wrong: `buildVegetationDepthItems` never reads `state.vegetationOverlays` (the LOADED mesh/texture) at all — only `items`, floor bands, and the STATIC discovery map (`getVegetationRenderState().urlByItemId`, resolved once at scene load, not per-pass). It never had a real dependency on phases 1/2 finishing.

**The fix — move the vegetation-rank computation (and the second `rebuild()` call) to run IMMEDIATELY after the first rebuild + `stampVegetationRenderOrders`, with NO `await` in between.** Since JS is single-threaded, two synchronous statements back-to-back cannot be interleaved by anything else on the event loop — the transition from "last pass's full table" to "this pass's full table" is now atomic from a concurrently-rendering frame's point of view; there is no synchronous gap for a frame to land in and observe vegetation missing. `rebuildSceneDepthProxies(itemsWithVegetation)` still runs at the end, after phases 1/2 — it draws the GPU proxy from the freshly-LOADED mesh/texture, which genuinely does not exist any earlier; that part was never the bug. Deliberately still TWO `rebuild()` calls, not merged into one — `stampVegetationRenderOrders` still needs the real-items-only array for its own "how many real items sort below" scan, unchanged from §9g's own reasoning.
