...
**Status:** DESIGN SPEC, 2026-07-26. **PHASES 1–3 BUILT — THE EFFECT RENDERS.** `src/effects/fluid/`: declaration, tube-net extractor, pack builder, TSL surface material, surface subsystem, registration; wired through `boot.js` + `vt-pan-viewer.js` with a **Fluid** card in the workshop panel and `MapShine.setFluid`. 171 Node tests. **Tiers 0–3 only, and only the ADD half of the two-blend split** — absorption under the art (§5.6), the 1-D sim (§5.3; slugs today are the analytic PRESCRIBE mode), bubbles, optics, emission-as-light and spray are all unbuilt.
...
```
src/effects/fluid/
  fluid.js               FLUID_PARAMS + the FLUID manifest — pure data          (~400)
  fluid-net.js           CPU extractor: components, geodesic s, A(s)/R(s), graph (~450)
  fluid-pump.js          Q(t) per tube + the impulse queue. Pure, Node-tested   (~200)
  fluid-pack.js          TSL builders: the GPU bake (upsample + JFA + pack)     (~300)
  fluid-sim.js           TSL builders: prescribe + transport, ping-ponged       (~350)
  fluid-render.js        TSL builders: the surface material, tier-gated         (~700)
  fluid-pass.js          buildFluidSimPass(ctx) — replaces the seam             (~300)
  fluid-registration.js  cascade layer, live override, MapShine.setFluid, card  (~130)
  __tests__/{fluid,fluid-net,fluid-pump}.test.mjs
```

⚠️ `buildFluidSimPass` currently lives in `src/effects/water/water-pass.js` — it moves to `effects/fluid/`, and `effects/index.js` + `graph/pass-seams.js` follow. Fluid is not part of the water family and should never have shared its file.
...
Five, all mid-session knobs. Iridescence spread, absorption, wall rim, noise scale, pump waveform and every tier threshold are ROH — the full schema auto-categorised.
...
| **3** | ✅ **DONE 2026-07-26** — tiers 0–3: `fluid-render.js` (the ADD half), `fluid-surface-subsystem.js` (mask → net → pack → texture → mesh, AABB-cropped), `fluid-registration.js` (cascade, card, `MapShine.setFluid`), boot + viewer wiring.                                                 | ⬜ **NOT live-confirmed** — author has not yet seen it on a real map with a painted mask. The multiply half (absorption under the art) remains.                                             |
| **4** | **Tier 3** — the analytic material coordinate and the one noise fetch                                                                                                                                                                                                                      | structure visibly rides with the flow, not past it                                                                                                                                          |
| **5** | The sim — `sims.fluids` live in PRESCRIBE mode, then TRANSPORT, then the pump. **Tier 4.**                                                                                                                                                                                                 | a tube visibly fills, drains, and speeds up through a constriction                                                                                                                          |
| **6** | **Tier 5** — bubbles                                                                                                                                                                                                                                                                       | bubbles bunch at a constriction on a real map                                                                                                                                               |
| **7** | **Tiers 6–7** — optics, emission                                                                                                                                                                                                                                                           |                                                                                                                                                                                             |
| **8** | **Tier 8** — spray, through the one engine                                                                                                                                                                                                                                                 |                                                                                                                                                                                             |

**Phases 1–4 are a complete, shippable effect that is already better than V2** and contain no sim at all. That is deliberate: if the sim proves fiddly, there is a product either way.
