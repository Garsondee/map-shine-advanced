...
| A7f ✅ | **Cap mask-source uploads** (§13.2): outdoors recovery/promote paths went through the uncapped generic loader → ~549 MB transient uploads mid-load | S | **Done 2026-07-09** — both `GpuSceneMaskCompositor` sites now use `loadImageTexture(DATA_MASK, MASK_MAX_SIZE)`. **Needs runtime verify: does the Mansion now load?** |
...
| A9 | **Demote Foundry-side full-res textures** (§13.2): the 1.46 GB of hidden PIXI copies (12000² background + overhead) freed after MSA's copies exist; `baseTexture.dispose()` or downscale-in-place, sequenced after `_tryLoadFromFoundryPixi` consumers | M | `pixiTextures.estTotalMB` drops by ~1.4 GB on the Mansion; scene still renders + streams correctly |
