...
| Thin utility G-buffer (the floor-attribute MRT) | §14.3, B0-1 | This *is* the modern "forward+ with a thin attribute buffer" shape — and it is correctly binary, NEAREST, and exact-decode. |
...
So the answer to "do we have everything we need?" is: **no load-bearing architecture is missing — what's missing is governance.** Every AAA engine is *budget-driven*: each frame has a time budget, each subsystem has a slice, and the engine **changes its own behavior** (resolution, LOD, work deferral) when the budget is exceeded. V3 today has budgets only for VRAM, applied statically at load (`texture-budget-policy`), and the A10 native crash showed even those aren't enforced at the "native" setting. The plan *measures* superbly (frame-graph timings, §13's instrumentation) but nothing yet *acts* on a measurement at runtime. Closing that loop is the P-track.
...
Today shader warmup *polls* `renderer.info.programs` until the count stabilizes (§13.9's `waitForCompiledPrograms`) — the A11 premature-"Ready!" bug is that gate's UX symptom, and a poll can only ever approximate "what will this frame need." Proposal: V3 pipelines are **declared** — each pass enumerates its shader permutations as a small explicit `#define` matrix (mask-packed?, indoor/outdoor?, shadow terms?, DRS upsample flavor?). Load-time warmup = compile the declared set asynchronously via `KHR_parallel_shader_compile` (already referenced by the V2 warmup — [canvas-replacement.js](../../scripts/foundry/canvas-replacement.js), [FloorCompositor.js](../../scripts/compositor-v2/FloorCompositor.js)) through the P3 governor; readiness becomes a *fact* ("declared set compiled"), not a heuristic — A11's fix falls out structurally under V3. Budget the permutation count (target: ≤ ~64 programs for the whole V3 frame, reported in diagnostics); 48 bespoke effects collapsing into shared shader modules with flags is also how the ~104k effect LOC shrinks (§16.4, B4 note). **This is simultaneously the single highest-value WebGPU preparation** — a declared permutation *is* a `GPURenderPipeline` descriptor (W rule 1). **Exit:** loading a golden scene compiles a known, logged pipeline list; "Ready!" appears only when that list is done; zero first-interaction compile hitches on pan/zoom/floor-change.
...
| When | Do |
|---|---|
| **Now** (with the A10 tile-ready fix) | P3's task-budget rule — the A10 fix *is* its first application, so land the rule with the fix; draft P1's budget table; W0 telemetry (one diagnostics field) |
...
- **Every V2-reuse bridge is WebGPU debt.** `V3PostBridge` running `ColorCorrectionEffectV2`/`BloomEffectV2`, `V3EffectsBridge` ticking V2 effect instances, and the `FloorRenderBus` dependency all drag raw-GLSL V2 code into the V3 frame. The B2-doc endgame (CC demoted to one thin grade) and B7 (V2 deleted) are therefore *also* the WebGPU unblockers. **No WebGPU implementation work before B7** beyond W0/W1 — this is the honest version of "we can't focus on it much yet."
...
4. **Design to the stricter of the two APIs.** Precedent already set correctly: WebGL2's shared-blend-state-across-MRT rule (B0-1 §3.1's zero-write convention) is *stricter* than WebGPU's per-attachment blend — and the convention works on both. Keep making that choice.
5. **Shader-source discipline:** V3-authored GLSL stays in a transpilable ES 3.0 subset, organized as shared modules with flags (P6), so the eventual translation (TSL rewrite or naga-class transpile) is mechanical per *module*, not per *effect*.
6. **No WebGPU-only features in the reference path — ever.** WebGL2 remains the floor indefinitely (Foundry's user base decides this, not us). WebGPU is an accelerator *tier* selected at boot, golden-scene-gated, and may only ever add optional visual/perf headroom — never required capability.

**Milestones:**
- **W0 (S, now):** capability telemetry — record `navigator.gpu` presence + adapter info in the diagnostics/crash reports, so the user-base readiness number is *known*, not guessed, when W1 decides. Zero rendering changes.
...
- **W2 (L, post-B7, gated):** port per W1; P1 budgets + golden parity required on *both* backends; ship as an opt-in tier first.
...
| Deferred-shading G-buffer | **Rejected** (re-affirming B0-1/B2) | Alpha-heavy content, tiny material space; the attribute buffer already *is* the thin G-buffer forward+ wants. |
...
- **Radiance-cascades 2D GI** — screen-space cascaded probes; near-constant cost at screen resolution (fits the §14.1 invariant by construction); would subsume much of window-light/candle bleed as *actual* bounced light. The natural successor to the B2 model's `glow` term, and the single biggest "AAA look" unlock available to a 2.5D renderer.
