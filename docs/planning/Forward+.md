...
| A10 | **Attribute and fix the remaining multi-second CPU stall (TDR root cause)** (§13.5–13.9): a 3.8–6.6 s main-thread block during `binding_effects`/late-load, not yet attributed to a specific function; suspected cause of the Native-quality crash | M | **Open, active.** All instrumentation needed to solve this is already deployed (`slowGlOps`, `slowSections`, `sectionTrail`, `longTasks`) — next step is purely "capture one more report at Native quality and read it," not more code. Do not guess at a fix before an instrument names the call |
...
> **Live status (2026-07-11): B1 is largely built and V3 is DEFAULT-ON — see [scripts/compositor-v3/README.md](../../scripts/compositor-v3/README.md), the authoritative status doc (this section lags it).** Built: frame graph, unified albedo, interim *non-clustered* forward lighting, first-cut candle/vegetation, HDR highlight rolloff. NOT built: the **B0-1 attribute buffer** (still single-attachment — B3/B4/B5 depend on it), clustered lighting (B2 proper), shadows (B3), water (B5), and the entire V2 post/effect stack V3 currently drops (CC, bloom, fog, window/fire/player light, specular, stylizers). The per-item exit criteria below still stand; the prose "not started" markers are stale where the milestone box above says otherwise.
