...
**Status:** Research document — diagnosis verified against source (2026-07-09). Target architecture in §14, staged plan of attack in §15, **performance-first engine review + the P/W tracks (the build standard for all V3 work) in §16 (added 2026-07-14)**. Stage A stabilization work (§15) is now substantially underway — see the milestone box below before reading further. Line numbers cited throughout are pointers verified on 2026-07-09/10 and will drift as code changes.
...
> **Author's framing (2026-07):** The module in its current form is _not reliable, not safe to run_ on constrained hardware and needs a considerable rethink. Tile streaming was attempted to beat the VRAM problem and was not enough. This document is the honest account of _where all the problems actually lie_ before any rebuild starts — including the parts of the earlier "just do what game engines do" advice that don't survive contact with MSA's real constraints.
...
- **Build standard for all resumed V3 work: §16 (added 2026-07-14).** It reorders nothing in B0–B7, but defines the P-track performance backbone (frame-time budgets, dynamic resolution, the unified work governor, pass memoization, one residency ledger, declared pipelines) that each B2+ item is built against, and the W-track WebGPU convergence rules that keep the eventual port mechanical. Read it before scoping any new V3 pass or effect port.
...
**What actually fixed it:** not the architecture refactor below (none of Phase 1–6 has started) — a chain of _verified, evidence-driven_ fixes to uncapped/uninstrumented loader paths, found by building crash-report instrumentation and refusing to guess:
...
- **A10 — reload/settings TDR crash (the top blocker).** Changing certain Tweakpane settings at High — anything that forces a resolution/resize → a scene _reload_ — crashes WebGL. Latest crash report (2026-07-14) is definitive about the shape: **not VRAM** (`rtVramEstimate` 1051 MB < 1200 budget; MSA textures near-zero), **not V3 render** (crash is during load `binding_effects`, before V3 draws a frame). It is a **driver watchdog reset (TDR) from an ~8,853 ms main-thread stall** during the reload. On 2026-07-14 the stall was UNATTRIBUTED because two instrument defects hid it — `sectionTrail`'s pre-stall breadcrumbs were flushed by the burst of `safeCall`s _after_ the stall (the ring emitted only the last ~48, all timestamped after the stall), and the heavy `binding_effects` init calls weren't individually labelled.
  ...
  **Instrumentation upgraded 2026-07-14 (built + Node-tested; the next action is now to REPRODUCE and read, not to write more instrument code):**
  ...
  - **`stallWitnesses` — the load-bearing fix (`safe-call.js`).** The instant a long gap between two section-starts is detected (the first section-start _after_ a stall ends — which the post-stall burst guarantees will fire), the section that ran across the gap is copied into a dedicated list retained by _largest gap_, not FIFO. The recovery burst can no longer bury the ~8.8 s witness. Immune to the exact failure that lost it on 2026-07-14.
  - **Windowed `sectionTrail`.** The report now emits a window _centred on the longest `longTask`_ (pre-stall + a little post) instead of a blind last-48 tail, so the breadcrumb that _started_ the stall is always included. Ring deepened 160→512.
  - **`sectionTimings.running`.** The coarse `createThreeCanvas` section left open (start, no end) at crash — a burst-immune "what phase was live" signal. `window.MapShine._sectionTimings` is now exposed _live_ (was only set at load end, so a mid-reload crash previously read the _previous_ load's finished timers).
  ...
  **RESULT — the instrumentation worked; culprit region NAMED (crash report 2026-07-14 12:39).** The "Likely culprit" line read: _longest stall **11,211 ms** (40717→51928 ms), context lost ~3.8 s after; stall-witness `tileManager.syncAllTiles` (marked) held the main thread; last breadcrumb before the stall `overlay.tokens`._ The overlap logic correctly ignored a _larger_ 12,059 ms witness (`v2.populate.kickoffEarly` — a separate earlier stall, multiple this load) because its window didn't intersect the worst `longTask`. Confirmed **not memory** (9 MSA textures, 286 MB PIXI, RT est. under budget). This matches §13.8's earlier _suspicion_ of the tile path — now proven.

  **Mechanism (deduced from a key discrepancy).** `tileManager.syncAllTiles` is wrapped in `markSection`, yet it produced **no `slowSections` entry** — so its _own synchronous body was fast_. Yet the ~11 s stall is one continuous `longTask` and there is an `await new Promise(r => setTimeout(r,0))` immediately after the call ([canvas-replacement.js ~8403](../../scripts/foundry/canvas-replacement.js)). The only consistent explanation: `createTileSprite` calls `loadTileTexture(...).then(cb)`; on a **reload the tile textures resolve from cache**, so the 6 heavy tile-ready callbacks `cb` (effect fan-out `onTileReady` + four `_ensure*Mesh` builds, [tile-manager.js:4534+](../../scripts/scene/tile-manager.js)) **drain as microtasks in one task at that `await`** — after `markSection`'s `finally`, which is why the witness names the _preceding_ label. `bigCanvasOps` empty + `slowGlOps` quiet in the window ⇒ **pure-CPU JS**, not GL/readback.
...
- **Sepia stylizer not applied under V3.** Author uses the Sepia CC to unify scene colours; without it the look is off. Sepia (and the rest of the stylizer chain: sharpen/vignette/dotscreen/halftone/etc.) is _not_ yet in the V3 post pass. Add `SepiaEffectV2` to `V3PostBridge` after CC, the same `render(in,out)` pattern bloom uses. Straightforward — but mind the extra RT at High (see A10 VRAM headroom).
...
- Indoor/outdoor breaking on camera _pan_ (`51fd1f2`) — `composer.js pan()` now calls `updateMatrixWorld()`; the shared raycast refreshes the matrix defensively.
...
Map Shine Advanced (MSA) bounds GPU/CPU memory to **world size × per-floor mask count**, where a modern real-time engine bounds it to **screen resolution**. The mismatch isn't raw scene complexity — it's that MSA's cost model scales with the size and authored-mask density of the _world_, not the size of the _view_.
...
- Mask RTs are already `UnsignedByteType` RGBA (`_createRenderTarget`, line 3800) — **4 bytes/px**. At 8192² that's **256 MB per mask**. The code comment at line 3759 states this plainly: _"on a 12000px scene meant ~256MB per mask × many floors/variants = multi-GB VRAM blowout."_
- Partially mitigated: `_highDetailMaskTarget()` (line 3767) now scales data-mask resolution by the budget policy (floor of 0.18×), and `_floorCache` (line 392) has LRU eviction (`_maxCachedFloors`) + `evictDistantFloorCaches()` (line 3830). These bound _retention_, not _per-floor authored footprint_.
...
`scripts/compositor-v2/effects/` holds **~48 `*EffectV2` classes** totaling ~104k lines. The largest: `FireEffectV2` (6861), `LightingEffectV2` (5891), `WaterEffectV2` (5174), `PlayerLightEffectV2` (5029), `FogOfWarEffectV2` (4083), `WaterSplashesEffectV2` (3854), `WindowLightEffectV2` (3667), `CandleFlamesEffectV2` (3570), `OverheadStampEffectV2` (3216). Many hold **multiple full-screen RTs** (`OverheadStampEffectV2` has ~35 RenderTarget references; the crash report counts ~19 live full-screen RGBA8 targets from it alone). The level RT pool is only _part_ of the compositor's VRAM — the effect stack is the rest, and per-floor effects multiply it.
...
1. **Per-floor render-target stacks** (§2.2) — framebuffer VRAM scales with visible-floor count. Real but _secondary_ to masks for the 2-floor Mansion.
...
- `_resolveWaterSourceFloorForView(viewedFloorIndex)` ([FloorCompositor.js:8303](../../scripts/compositor-v2/FloorCompositor.js)) scans floors `0..viewed` and picks the highest _lower_ floor that has water data.
...
- **Hi-Z / opaque inter-floor culling is fundamentally unsafe here.** You cannot cull or early-Z-discard the lower floor when the upper floor has holes the water shows through. This _kills_ the earlier "Hi-Z culling is the holy grail" idea as a general strategy — it's only valid for _fully opaque_ upper floors, which the plank case violates by definition.
- **A naive single-depth-buffer unified pass (Phase 1) must still reproduce this cross-floor visibility + occlusion**, including running the water _simulation_ for an off-view floor. Depth testing alone doesn't give you "simulate the thing below and show it through a hole with the correct occluder" — that's a compositing relationship, not a depth relationship. Phase 1 has to keep an explicit lower-floor-visible-through-upper path.
...
**Framing:** all of the above tune _resolution_ and _retention_. The refactor changes the _cost model_ so it stops scaling with world size and floor count.
...
Replace the per-level RT pool + bottom→top `LevelCompositePass` with a single screen-sized framebuffer where floors get real Z (floor _n_ → Z = 100n) and the hardware depth buffer resolves occlusion; upper-floor holes use `alphaTest`/`discard`.
...
> **Expected effect:** framebuffer VRAM `O(floors)` → `O(1)` for the _opaque_ case; biggest single lever after masks.
...
### Phase 2 — Mask virtualization: tile + stream the painted masks _(highest structural value, given §4.1)_
...
> **Infra-reuse assessment (researched 2026-07-09) — half is reusable, half is genuinely new.** Split the streaming stack into _storage_ and _consumption_:
>
> - **Storage/build half — HIGH reuse.** `pyramid-indexed-db.js` is fully generic (keyed by arbitrary string, stores blobs — reusable for mask tiles verbatim). `texture-pyramid-builder.js` is a generic URL→tiled-pyramid→worker-decode mechanism, currently only _wired_ to `__bg_image__`/`__fg_image__`; the machinery is mask-agnostic. **Channel-packing already has a real head-start:** [mask-channel-pack.js](../../scripts/streaming/mask-channel-pack.js) defines `PACKABLE_BINARY_MASKS = ['fire','dust','ash','outdoors']` packed into one RGBA atlas (R/G/B/A). So ~4 of the binary masks can share one texture — but the RGB masks (`_Normal`, `_Windows`, `_Specular` color, `_Water`) can't 4-pack, so channel-packing helps a minority.
> - **Consumption half — LOW reuse, this is the real cost.** Album images are **drawn as tile _meshes_ at world positions** → simple frustum culling decides which meshes to draw (what the existing streaming does). **Masks are sampled by _world coordinate inside effect shaders_** — verified: `water-shader.js` samples `sampleOutdoorsMask(worldSceneUv)`, `specular-shader.js` uses `worldPatternUv`, all `texture2D(mask, worldUV)`. A shader can request _any_ world position any frame, so "stream only visible mask tiles" forces **every mask consumer to handle non-resident regions** (page-table lookup + residency test + low-res fallback). That is Sparse Virtual Texturing proper, a per-shader change across ~15 mask consumers — **not** solved by the existing draw-cull streaming. **This is why Phase 2 is a real project, not a wiring job**, and why the SVT page-table work (above) is unavoidable, not optional, once you go past a low-res whole-floor fallback.
>   **Practical sequencing implication:** the cheapest first win is _not_ tiling — it's **(a) per-floor mask LRU eviction to a low-res whole-floor fallback** (bounds VRAM without shader changes), then **(b) channel-pack the 4 binary masks** (one-time atlas, no per-consumer SVT), and only then **(c) full SVT** for the large RGB masks. Steps (a)/(b) may relieve the crash enough to defer (c).

### Phase 3 — Screen-space / SDF masks for the _geometry-semantic_ masks only
...
**Expected effect:** removes the largest _derived_ world-space bakes (`skyReach`/`floorAlpha` at up to 8192²) from VRAM.
...
### Phase 6 — SDF-based GPU shadows & vision _(from ideas.md, verified applicable)_
...
- **KTX2/Basis compression — deferred, likely hard here.** MSA masks/images are GM-uploaded at runtime, not build-pipeline assets; `KTX2Loader` only _decodes_ pre-transcoded files, so this needs a client-side Basis _encoder_ (WASM) running in a worker against arbitrary 4K–12K uploads. Feasibility unproven. Treat as a timeboxed spike _after_ Phase 2, not an early win — Phase 2 tiling delivers most of the VRAM relief without it.
...
- Does the water-source-floor path (§4.2) generalize to _other_ see-through effects (fire glow through floor holes, foliage)? Audit before Phase 1 locks the unified-pass compositing contract.
...
6. **Run a "live-path audit" of the Class C/D files** (§12.5) — `WaterEffectV2` (95 legacy markers), `LightingEffectV2` (91), the shadow effects — mark each fallback live/dead, delete dead, document the one intended path, _before_ rewriting them.
...
_Researched 2026-07-09 against the unminified V14 client source in `foundryvttsourcecode_v14/resources/app/client/` and MSA's `scripts/vision/` + `scripts/foundry/`. This is the near-term priority._
...
- Maintains its **own** ping-pong exploration accumulation RT, **per elevation band** (`_explorationGpuBandKey` — this is _more_ than Foundry, which persists one exploration per level), seeds it from Foundry's saved `FogExploration` at load, and writes back via a **throttled, tiled GPU→CPU readback + WebP encode** ([lines 223–237](../../scripts/compositor-v2/effects/FogOfWarEffectV2.js)) — the acceptable persistence readback of §11.2.
...
| `VisionPolygonComputer.js` | 1296  | **LIVE — keep**       | Constructed by `FireEffectV2:739`, and imported by `CandleFlamesEffectV2`, `LightningEffectV2`, `FogOfWarEffectV2`, `ThreeLightSource`, `light-interaction` — used for **per-effect occlusion geometry** (light-blocking for fire/candle/lightning glow), _not_ the vision mask. |
| `VisionSDF.js`             | 458   | **LIVE — keep**       | Constructed by `FogOfWarEffectV2:1212` for the fog-edge SDF.                                                                                                                                                                                                                     |

So the safe deletion is **~1338 lines** (`VisionManager` + `FoundryFogBridge` + `FogManager` + `GeometryConverter`), not the ~3100 an earlier draft claimed. **`VisionPolygonComputer` and `VisionSDF` must not be deleted.** Caveat: `VisionPolygonComputer`'s use for effect occlusion is a bespoke raycaster and _could_ carry the same cross-system fidelity divergence (§11.4.1) for those effects' light-blocking — worth a separate audit, but it is not dead code and is out of Phase 5's scope.
...
- **Drawing text fidelity** is the main scope risk. Options: (a) render each text drawing to a small offscreen canvas → `CanvasTexture` (a _bounded, per-drawing, on-change_ readback — acceptable, unlike full-layer capture); (b) MSDF font atlas (best quality, most work). Decide early.
...
- **Multi-floor vision gap (pre-existing, verify):** Foundry initializes vision sources for the **active view level**. MSA composites multiple floors — confirm `FogOfWarEffectV2` gets correct `.los` polygons for **non-active** floors (it tracks per-band exploration, implying it does, but the _live vision_ for a lower floor viewed from above — the water-under-planks case, §4.2 — needs explicit verification that Foundry still exposes usable vision sources for it, or that MSA falls back to full-visibility there).
...
**What MSA must _not_ do (cross-system traps):**

- Don't reimplement detection modes / vision modes / senses — delegate to `token.isVisible` and consume the resulting vision sources. Systems (and modules like _Levels_, _Wall Height_, _Vision 5e_) register these in `CONFIG.Canvas.detectionModes` / `visionModes`; MSA reading the _output_ (polygons + `isVisible`) is automatically compatible.
...
_Traced 2026-07-09 against `FloorCompositor.js` (~10k lines), `FloorRenderBus.js`, `LevelCompositePass.js`, and the effect roster. Goal: know exactly what a unified depth-layered pass would break before committing._

### 12.1 The pivotal finding — the albedo geometry is _already_ unified

`FloorRenderBus` is **not** a per-floor-RT painter. Its own header states the design: _"One THREE.Scene. Tiles Z-ordered by floor index (floor 0 at Z=1000, floor 1 at Z=1001, etc.) so standard depth sorting handles layering"_ ([FloorRenderBus.js header](../../scripts/compositor-v2/FloorRenderBus.js)), using `camera.layers` (floors 1–19, overhead 20) and `renderOrder`. **So Phase 1's "give every floor real Z and let the depth buffer resolve occlusion" is already true for tile/token albedo.**
...
| **P1 — Global producers** (~L5316–5545)                  | Specular, Iridescence, Prism render (once); shadow _producers_ (Overhead/Building/SkyReach/Painted mask build, L5418–5474); Cloud render; Lighting `beginFrame`/ceiling-transmittance                                                                   | Run **once**, produce world/screen textures consumed later.                                                                              |
| **P2 — Per-level loop** (L9111–9410)                     | Bus albedo per floor (already Z-sorted); **Lighting** per floor (L9235); **WindowLight** per floor; **Fire glow** per floor; **Painted/Building shadow** _lit-per-floor_ (L9223/9230); **Water** per-level when single-floor (L9317)                    | **The per-floor RT stack.** Allocates `{sceneRT,postA,postB}` per visible floor; builds cross-floor water occluders from upper sceneRTs. |
...
`FilterEffectV2`, `SharpenEffectV2`, `DotScreenEffectV2`, `HalftoneEffectV2`, `AsciiEffectV2`, `DazzleOverlayEffectV2`, `VisionModeEffectV2`, `InvertEffectV2`, `SepiaEffectV2`, `LensEffectV2`, `DistortionManager`, `FloorDepthBlurEffect`, `FogOfWarEffectV2` (world-space vision RT, already floor-aware). Also the **once-run screen-space mask passes** `SpecularEffectV2`, `IridescenceEffectV2`, `PrismEffectV2` (rendered once at L5316–5318, _not_ per floor) and `ColorCorrectionEffectV2` / `BloomEffectV2` / `AtmosphericFogEffectV2` (post-merge already) — **but** these five consume a _stacked per-floor mask_ (stacked outdoors, per-floor specular/iridescence source); that aggregated input must survive the refactor, so treat them as **A-minus** (§12.6 last bullet).
...
**Class C — Hard-coupled to the per-floor RT stack (this is _why_ the stack exists).** Cannot be collapsed without moving lighting/shadows to **forward/per-fragment** shading — i.e. Phase 1 and Phase 4 are the _same move_, not sequential (§12.4).
...
- **Class C:** the per-floor RT stack only disappears if lighting/window-light/fire-glow/shadows become **forward per-fragment** in the unified geometry pass. That _is_ Phase 4 (clustered lighting). **Recommendation: merge Phase 1 and Phase 4 into a single "unified forward pipeline" workstream** — attempting Phase 1 without Phase 4 just moves the per-floor passes around without removing them.
- **Class D:** must be designed _first_, as an explicit contract, because it constrains the entire unified-pass architecture. Everything else can adapt to it; it cannot adapt to them. **Now concrete (§12.3 resolution):** the contract is "reproduce the current-floor + below-floor screen-space presence masks." One primitive, ~6 consumers — bounded and testable, not open-ended.
...
2. **The crash cause is outside everything the reports could see.** Leading hypotheses: (a) **Foundry/PIXI-side texture memory** — Foundry's own renderer holds full-resolution source images (12000² background ≈ 576 MB; tiles similar) in the same GPU memory, and _no MSA budget, probe, or report ever counted it_; the `_tryLoadFromFoundryPixi` load paths confirm those PIXI textures exist by design, meaning large images may be resident **twice** (PIXI full-res + MSA's capped copy). (b) **Driver watchdog (TDR)** during `binding_effects` shader compilation — resets the device regardless of VRAM.
...
- **Crash unchanged: 36.25 s** (vs 36.3 s prior run — deterministic to ~0.1 s), `binding_effects`, bare mode, 9 MSA textures, 228 MB heap. **Total _accounted_ GPU < ~500 MB on an 8 GB card and it still died → resident texture memory is eliminated as the cause.** What remains is a _specific operation_ at that moment.
- **The operation, found by caller tracing:** in every run, ~10 `loadTextureAsync` textures finish ~300 ms before death. §13.2's fix capped `GpuSceneMaskCompositor`'s two sites — but **`PaintedShadowEffectV2` (3 sites), `WindowLightEffectV2` (2), `SkyReachShadowsEffectV2` (1) import the uncapped generic `loadTexture` directly** and load _authored full-res masks_ during `binding_effects`. Worse, `loadTextureAsync` internally routes through **Foundry's `loadTexture`** (repopulating PIXI's cache — explains the 22→39 creep) and then **clones mask-suffixed sources to canvas at FULL resolution** (a 12000² mask ≈ 549 MB decoded, ×10). Decoded bitmaps live in the browser's image/GPU-transfer memory — **counted by neither the JS heap nor GL texture counters**, which is why every snapshot looked clean.
...
- **The old ~2 GB heap signature returned on a clean run** (1910 MB) alongside the decisive new data: `longTasks` recorded repeated **1.3–2.5 s main-thread stalls** through `binding_effects` and one **5.8 s stall** at 32.7 s; crash at 40.9 s. Multi-second _synchronous_ work allocating GB-scale heap.
...
- **`usedJSHeapMB: 224`** (vs 1910 the run before) — heap was _normal_ and it **still crashed**. **The 2 GB heap is refuted as a cause**; it was a symptom of some runs, not the mechanism.
- **What the instruments did show — the sharpest signal in the whole investigation:** `longTasks` recorded a **6,618 ms** main-thread stall starting at 41.19 s (preceded by a 3,824 ms one), and the **context was lost 111 ms after that stall ended** (crash at 47.92 s). Two cloud textures finished loading 16 ms before the stall began — but cloud loads are sequential `await`s and may merely _resolve between_ other synchronous work, so **correlation is not causation here** (the §13.4 lesson: do not guess again).

**Current diagnosis (evidence-ranked):** every memory hypothesis is now eliminated — resident VRAM (§13.3, <500 MB accounted), PIXI-side (demoted to ~316 MB), JS heap (normal), decoded-bitmap memory (all loads capped). What remains is a **multi-second synchronous block** that starves Chrome's GPU channel into a **driver watchdog reset (TDR)** — Windows' default TDR timeout is ~2 s, and we are blocking for 3.8 s and 6.6 s. The crash _immediately follows_ the block. Consistent with `binding_effects` being the phase (48 effects, large shaders) and with `programs: 0`/`renderCalls: 0` (nothing ever rendered).
...
**Standing conclusion unchanged and strengthened:** the Mansion crash is a **GPU driver watchdog reset (TDR)** provoked by multi-second synchronous blocking, _not_ memory exhaustion. Every memory hypothesis is eliminated (§13.3–13.5). The eventual fix is to **chunk/yield the offending CPU work** and stagger GPU submissions — Stage A10 — and it remains orthogonal to the Phase 1–5 refactor.
...
**Foundry's own PIXI renderer uploads the 6408×5121 tile image (~131 MB), four times, 191–351 ms each.** MSA never touched them. This vindicates the §13.7 correction (attributing them to `TileManager` was an assumption) and adds a fourth entry to the running theme: _the expensive things are usually not where the module's own accounting looks._ Note A9's demotion sweep only disposes textures ≥8192 px, so 6408 px tile images slip under it.

**User observation, and it corroborates the stack.** The loading overlay reads **"Setting up tokens…"** when the context dies. That stage (`scene.tokens`, [loading-stages.js:19](../../scripts/ui/loading-stages.js)) is set with `keepAuto: true` at [canvas-replacement.js:8360](../../scripts/foundry/canvas-replacement.js) and then **`TileManager` construct/initialize/`syncAllTiles()` run underneath it** — the label is stale, and the work actually happening is _tiles_. The visible label and the PIXI tile-upload stack point at the same region of the load.
...
- **`sectionTrail` was structurally useless at 32 entries** — the burst of `safeCall`s _after_ a stall flushed every pre-stall entry, so the report showed only post-stall labels. Ring raised to **160** (report emits last 48). `longTasks` ring raised 16→40 (emits 24).
...
**"Native" quality still crashes to a black screen.** No new evidence has been gathered specifically on a Native-quality run — the fixes above were validated at High/800×450/bare-mode variants. Given every _memory_ hypothesis is now eliminated at those settings (§13.5–13.8), and Native mainly increases render **resolution** (drawing-buffer megapixels) and re-enables the full effect stack (not bare mode), the leading hypotheses for a Native-specific crash are: (a) the same unattributed multi-second CPU stall, worse at higher resolution or with more effects live; (b) a resolution-scaled version of one of the already-fixed-but-not-fully-generalized uncapped paths (e.g. a mask or tile path that scales with _render_ resolution rather than _source_ resolution and wasn't covered by the §13.2–13.6 fixes); or (c) a genuinely new, not-yet-instrumented bottleneck that only appears with the full (non-bare) effect stack. **Next action: reproduce at Native, capture a crash report, read `slowGlOps`/`slowSections`/`sectionTrail`/`longTasks`/`pixiTextures` — do not re-guess which of (a)/(b)/(c) it is.**
...
- The loop does update the overlay text to `"Compiling shaders (ready/total)…"` every 250 ms _while `total > 0`_ — so if this is silently stuck, it's likely because `renderer.info.programs` count keeps changing (new deferred/lazy effect programs still registering) and `stable` never reaches `requiredStableFrames`, causing the **full 15 s timeout** to elapse before falling through anyway. That would explain ~15 s of stall plus a few seconds of curtain-transition, matching the user-observed ~20 s.
- **Not yet fixed.** Cheapest correct fix is probably not to display "Ready!" until _after_ this second gate resolves (show a genuine "Finalizing…" label instead, or fold this gate's progress into the earlier `shaders.compile` stage so the label never regresses to a stale "done" state). Needs a `sectionTrail`/`slowSections` capture spanning this specific window to confirm the exact mechanism before touching the code — same discipline as §13.5–13.8.

This is unrelated to the Native-quality crash (§13.9 above) — track separately as **Stage A11** (§15).

---

## 14. Architecture review — the target state and its principles _(added 2026-07-09)_

_Big-picture pass over everything above. §14.1–14.2 are decision rules and the end-state description; §14.3–14.5 are design proposals that need validation during Stage B design (§15), marked as such. Nothing here contradicts the verified findings — it organizes them into a buildable shape._

### 14.1 Principles — the decision rules for every choice below

1. **The screen-bound invariant.** Every byte of GPU memory is bounded by _screen resolution_ or a _fixed budget_ — never by world size, mask count, or floor count. Any new allocation that scales with the world is a regression, full stop. This is the one-sentence version of the whole refactor.
...
4. **No silent fallbacks.** A fallback either becomes the only path or gets deleted. `catch(_){}`-degradation and dual single-/multi-floor branches (§12.5) erode reliability _and_ misdirect debugging. New code: fail loudly in dev, degrade _visibly_ (logged + surfaced in diagnostics) in production.
...
Everything on the GPU side is screen-sized except the mask cache, which is a _fixed_ budget (e.g. 256–512 MB) regardless of map size. Target total at 1080p: **well under 1 GB VRAM for any scene** — versus multi-GB today. That number is the acceptance metric for the whole effort.
...
> **Attribute buffer (RGBA8, screen-sized, NEAREST):** R = floor index of the topmost opaque fragment · G = outdoors/skyReach value (sampled in-pass from a _low-res_ per-floor outdoors texture — outdoors is area classification and doesn't need world resolution) · B/A = reserved (presence flags, material bits).

Every downstream consumer then derives its need from one buffer: _current-floor presence_ = `floorID == viewed`; _below-floor presence_ = `floorID < viewed`; _stacked outdoors for CC/bloom_ = the G channel directly. This retires the per-effect presence scenes, the stacked-outdoors post-merge builds, **and** the A-minus caveat in §12.3 — and it folds Phase 3 into the unified pass (outdoors becomes a per-pixel attribute, not a world-space bake stack). Convention: only opaque + alphaTest fragments write attributes; transparents read them (standard). Consumers needing soft edges blur their derived mask. **Validate in Stage B design**: semi-transparent decks and the water surface are the edge cases.

### 14.4 Proposal — a minimal frame graph replaces the god-compositor

`FloorCompositor.js` is ~10k lines because pass ordering, RT lifetimes, and effect wiring are all implicit in one imperative method. V3 should introduce a _minimal_ frame graph — not an engine, ~200 lines: passes declare `{name, reads[], writes[], execute()}`; a tiny scheduler allocates transient RTs from a pool, aliases them when lifetimes don't overlap, and runs passes in dependency order. Dividends: automatic RT reuse (memory), per-pass GPU/CPU timings for free (feeds §13 diagnostics), impossible-to-miss ordering bugs, and exactly the immutable-pipeline structure §8 wants for a future WebGPU port.

### 14.5 Proposal — water surface as first-class geometry

Today, water is a screen-space composite with hand-built cross-floor occluders (§12.3 Class D, the hardest machinery in the codebase). In the unified pass, the natural form is: **the water surface is a mesh** (the water-mask region, at its floor's Z) whose shader samples the existing bounded sim. Then plank-over-river occlusion is _just depth testing_ — planks at upper Z occlude; alphaTest holes reveal — and most of `_resolvePostMergeWaterOccluderRT`'s bespoke machinery dissolves. The sim itself (already bounded, P0) is untouched. Splashes/edge-cases keep a small screen-space path reading the attribute buffer. **This is the highest-payoff simplification in the plan and also the riskiest — it gets its own milestone (B5) gated on the plank-prison acceptance scene, with V2 water kept alive behind the flag until V3 water passes.**

---

## 15. Plan of attack

Two stages plus a parallel track. Stage A makes today's module _safer and measurably lighter_ with generic, low-risk fixes — the "small improved version." Stage B is the big refactor, built as a parallel V3 pipeline behind a flag. Ordering within stages is deliberate; each item lists its exit criterion. Sizes: S = hours–a day, M = days, L = week+.
...
| A6      | **Live-path audit + de-legacy, one file at a time** (§12.5): `WaterEffectV2` → `LightingEffectV2` → `BuildingShadows` → `WindowLight` → `SkyReach`. Timeboxed per file; mark live/dead, delete dead, document THE path. No behavior changes                                                                                                                               | L    | Each file's marker count materially down; golden scenes unchanged. **Not started** — but §13's investigation already found and fixed several _specific_ legacy-divergence bugs in these files' neighbors (loader paths, not the effects themselves) without doing the full audit, which may lower A6's remaining value/urgency                                                                                                                                                                                                                                                                                                                                                                                     |
...
**B0 — Design docs first (no code).** Three short specs, validated against §12's classification: (1) the floor-attribute buffer (§14.3) including the water/transparent-deck edge cases; (2) the V3 frame graph pass list (§14.4) — every §12.2 phase mapped to a named pass; (3) the Class B transparency strategy (recommendation: painter's-order _within_ floor bands + depth test _between_ bands first; OIT only if that visibly fails). Exit: specs reviewed, golden-scene expectations written down.
...
**B1 — Frame graph + unified albedo (M).** V3 flag renders albedo-only unified pass (already Z-sorted, §12.1) + attribute buffer through the frame graph. Exit: golden scenes' _geometry_ matches V2 with lighting off; attribute buffer visualizable in a debug overlay.

**B2 — Clustered forward lighting (L).** Phase 4 proper: light bins → per-fragment loop in the unified pass. `LightingEffectV2`'s accumulation path retires _inside V3_. Exit: lighting parity on golden scenes; per-floor light snapshot RTs gone from V3 diagnostics.

**B3 — Shadows onto the forward pass (L).** Shadow producers (P1) stay; per-floor _lit_ application (`renderLitForSingleFloor`) becomes attribute-buffer-driven terms in the unified shader. Exit: shadow parity; per-floor shadow passes gone.
...
## 16. Performance-first engine review — V3 vs AAA practice, and the WebGPU road _(added 2026-07-14)_

**Status:** planning review, written at the author's direction: _"reimagine with free reign — no refactor too big; a very performance-focused rendering engine first, visually powerful second; WebGPU eventually, but it must fall back on systems that aren't ready for it."_ Assessed against the shipped V3 code ([scripts/compositor-v3/](../../scripts/compositor-v3/)), the B0 specs ([B0-1](v3/B0-1-floor-attribute-buffer.md) · [B0-2](v3/B0-2-frame-graph.md) · [B0-3](v3/B0-3-transparency.md)), and the [B2 design doc](v3/B2-lighting-grade-unification.md), all re-read today. Verdict in §16.1. The additions are the **P-track** (§16.3, the performance backbone) and the **W-track** (§16.5, WebGPU convergence). **Nothing here reorders B0–B7** — it defines the standard each B-item is built against, and it is deliberately mostly _contracts adopted while the V3 surface is small_ (4 passes today) rather than new subsystems.

### 16.1 Verdict — the skeleton is right; what's missing is the governor

The plan already contains the hard architectural decisions a AAA-style 2.5D engine needs, and they are the correct ones for MSA's constraints. Do not reopen these:

| Decision                                                                       | Where                                                                                      | Assessment                                                                                                                                                   |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `O(screen)` cost-model invariant                                               | §14.1 #1                                                                                   | The single most important sentence in the plan. Right.                                                                                                       |
| Minimal frame graph with validation + per-pass timing                          | §14.4, B0-2, [FrameGraph.js](../../scripts/compositor-v3/FrameGraph.js) (built, 546 lines) | Right, and the implementation held the "not an engine" line (THREE-free core, loud registration-time failures).                                              |
| Thin utility G-buffer (the floor-attribute MRT)                                | §14.3, B0-1                                                                                | This _is_ the modern "forward+ with a thin attribute buffer" shape — and it is correctly binary, NEAREST, and exact-decode.                                  |
| Clustered **forward**, not deferred                                            | §15 B2                                                                                     | Right for 2.5D: tiny material space, alpha-heavy content; the MRT budget is better spent on attributes than a fat G-buffer (re-affirmed in §16.6).           |
| Screen-space vector masks rejected for painted masks; tiling/residency instead | §4.1, B6                                                                                   | Right — the generic game-engine advice would have failed here.                                                                                               |
| Explicit Class D see-through contract, designed first                          | §12.3, B0-1 §4.1                                                                           | Right; exactly the constraint a naive "just use depth" rewrite silently kills.                                                                               |
| "Night lives in the illumination buffer, not the grade"                        | B2 doc §3.1                                                                                | The illumination-priority operator table (ambient → MAX lights → MIN darkness → × shadow → + emissive) is the strongest single design idea in the V3 corpus. |
| Golden scenes + flag-flip A/B as the acceptance method                         | §15 A2/B0                                                                                  | Right method — but still missing its baselines, which P1 fixes by making it executable.                                                                      |

So the answer to "do we have everything we need?" is: **no load-bearing architecture is missing — what's missing is governance.** Every AAA engine is _budget-driven_: each frame has a time budget, each subsystem has a slice, and the engine **changes its own behavior** (resolution, LOD, work deferral) when the budget is exceeded. V3 today has budgets only for VRAM, applied statically at load (`texture-budget-policy`), and the A10 native crash showed even those aren't enforced at the "native" setting. The plan _measures_ superbly (frame-graph timings, §13's instrumentation) but nothing yet _acts_ on a measurement at runtime. Closing that loop is the P-track.

The second gap is strategic: §8 files WebGPU as "a design principle, not a phase" — right when written, but a principle with no enforcement mechanism quietly erodes. §16.5 turns it into six enforceable contract rules plus a dated decision schedule.

### 16.2 The two ideas everything below follows from

1. **Frame-time is the primary metric; VRAM is second.** §14.2 sets a VRAM acceptance number (<1 GB @1080p). There is no frame-time equivalent — yet every recent crash is a _time_ failure (TDR from multi-second stalls, §13.5–13.9, A10), not a memory failure. The engine contract must be explicit: **16.6 ms/frame at High on the reference machine (RTX 3070 Laptop, 8 GB, ~2560×1080), an ~8 ms main-thread ceiling per task during play, ~50 ms during load** — and a violation is a _named bug_ (the §13 instrumentation already knows how to name one; P3 makes that a standing enforcement rather than an investigation tool).
2. **A VTT frame is usually the same frame as last frame.** MSA is not a twitch game: most frames, nothing moved but particles and water. AAA engines optimize per-frame cost because everything moves every frame; a VTT engine should optimize **not rendering** — cache at pass granularity and re-execute only what a _declared_ invalidation input dirtied. V2 already half-knows this (idle throttling to 15 fps in `render-loop.js`; `FloorCompositor`'s stacked-lit snapshot cache; `render-invalidation.js`'s dirty flags) — but as scattered bespoke caches, which is §14.1 principle 3's disease again. V3 should make it a frame-graph-native contract (P4). This is the single biggest performance lever available and it is nearly free _because_ the frame graph exists.

### 16.3 P-track — the performance backbone

Each item: what it is, what already exists to build on, exit criterion. Sizes use §15's scale. These are additive to B0–B7; §16.4 says when each lands.

**P1 — The performance contract: budgets + an executable golden-scene harness (S–M).**
Write the budget table (per tier: the 8 GB laptop reference at High; one low-tier floor; one 4K desktop ceiling) with a provisional per-pass split — e.g. at 16.6 ms: opaque+attr ≤ 3, lighting ≤ 3, effects ≤ 2.5, post ≤ 4, overlays+present ≤ 2, ~2 headroom — numbers to be _revised against measurement_; the point is that a number exists and the graph reports against it. Wire **GPU** per-pass timing: [PerformanceRecorder.js](../../scripts/core/diagnostics/PerformanceRecorder.js) already implements `EXT_disjoint_timer_query_webgl2` correctly (one `TIME_ELAPSED` query at a time, disjoint handling) — B0-2 rule 3 promised per-pass GPU timings "when available"; land it by lending the recorder's machinery to `FrameGraph.execute()`, falling back to CPU wall-clock where the extension is absent/disabled. Then make A2 executable: a console/macro harness that loads each golden scene, warms N frames, records per-pass ms + a screenshot to disk, and diffs both against a stored baseline. Full headless CI is _not_ required for the value — a one-command manual run is. **Exit:** one command produces a pass/fail report per golden scene (timing vs budget, image vs baseline); A2's missing baselines exist as a side effect.

**P2 — Dynamic resolution: split render resolution from present resolution (M).**
Today V3 sizes every `'screen'` resource from the raw drawing buffer ([V3Pipeline.js](../../scripts/compositor-v3/V3Pipeline.js) `_resolveSize`), and quality presets cap that buffer statically — with "native" running uncapped, which is exactly the A10 steady-state signature (`rtVramEstimate` 3279 MB, `overBudget: true`). Proposal: the graph gets one governed variable, `renderScale` (0.5–1.0). All `'screen'` resources allocate at `presentSize × renderScale` — the descriptor plumbing **already exists** (`ResourceDescriptor.scale`, [FrameGraph.js](../../scripts/compositor-v3/FrameGraph.js)); present upsamples to the swap chain in `FullscreenPresent` (start bilinear + optional sharpen tap; an FSR1-class spatial upscale is a shader, not a dependency). P1's timings _govern_ it: scale steps down when frame time exceeds budget for N consecutive frames, up when comfortably under for M, with hysteresis, preset-defined floor/ceiling, and scale changes preferentially applied during camera motion (imperceptible — the standard DRS trick). Foundry's DOM/HUD overlay never scales; only the world buffer does. **Consequences:** "Native" comes to mean _present_ at native while internal resolution follows the budget — structurally retiring the native-uncapped crash class (the fix §"Open issues" A10 (a) guessed at, generalized); presets collapse into policy over one knob; and since RT memory scales with `renderScale²`, this is simultaneously V3's biggest VRAM lever at native. **Exit:** golden scenes hold the tier's frame budget with scale visibly adapting under load; native preset never allocates RTs above the VRAM budget.

**P3 — One work governor for the main thread + GPU submission (M).**
[gpu-work-scheduler.js](../../scripts/streaming/gpu-work-scheduler.js) is already a real frame-paced, credit-budgeted governor — its own header: heavy GPU commits enqueue with estimated cost, "the governor runs as many commits as fit inside the current frame's credit budget… so the systems do not all spike in the same frame and trip a GPU driver reset." The problem is coverage: it is opt-in per call site, and the A10 stall lived in _exactly_ the class of work it exists to pace (tile-ready effect fan-out + mesh ensures) on a path that never enqueued. Proposal: promote it to the **single authority** all heavy work flows through — texture uploads, mask recomposes, `_ensure*Mesh` builds, effect (re)builds, shader compiles (P6) — with **two** budgets: the existing GPU-submission credit, and a main-thread task budget (~8 ms play / ~50 ms load, absorbing `yield-to-main.js`). The A10-class rule becomes _enforced, not remembered_: in dev, any synchronous section crossing the budget is logged by the existing `stallWitnesses`/`markSection` machinery as a named defect the moment it happens — the same instruments that solved A10, standing guard instead of being rebuilt per investigation. This also directly implements the stated load-path priority order (reliability first: never submit a spike that risks the TDR; smooth second: the overlay keeps animating because nothing blocks). **Exit:** no single main-thread task >50 ms during a golden-scene load _including a cache-hot reload_ (measured by `longTasks`), with the governor's deferral stats in the crash report.

**P4 — Pass memoization: render-on-change as a frame-graph contract (M).**
Add an optional `invalidateOn: [...]` to the pass declaration — a set of named version counters (camera transform, viewed floor, light-state version, sim tick, mask-residency version, weather state, DRS scale). The graph skips executing a pass when none of its declared inputs changed since its output was produced, and reports it as `cached` (visible in diagnostics, per §14.1 #6 — a skipped pass must be observable, not just fast). The version sources already exist as scattered dirty flags (`render-invalidation.js`, the stacked-lit cache's camera/floor stability checks, `requiresContinuousRender`) — this consolidates them into named, graph-visible versions and then _retires_ the bespoke caches. Payoff order: lighting (static most frames — lights change rarely in play), shadow producers (BuildingShadows already caches a world-space RT — precedent), the whole post chain when its HDR input hash didn't change. Combined with P2, an idle scene costs approximately one present pass; combined with the floor-change fast path (README §"floor-change fast path"), a floor switch becomes "bump the viewed-floor version, re-run only passes that declared it." Interaction to design for: a `renderScale` change invalidates everything (correct and automatic if scale is itself a declared version). **Exit:** an idle golden scene (no camera/sim motion beyond particles) renders at a small fraction of full-frame GPU cost with the skipped passes visibly reported as cached.

**P5 — One residency ledger for every GPU byte (M — lands with B6 design, API from day one).**
Today at least five independent budget/eviction systems coexist: `TextureBudgetTracker` (assets, 80% ceiling), `texture-budget-policy` (drawing-buffer/RT caps), `adaptive-budget-controller` (streaming), `GpuSceneMaskCompositor`'s `_floorCache` LRU + `evictDistantFloorCaches`, `LevelRenderTargetPool.releaseStale` (V2) — plus per-effect private RTs that **no ledger sees at all** (§2.3: `OverheadStampEffectV2` alone held ~19 live full-screen RTs). Proposal: V3 admits GPU memory only through one residency manager — every texture/RT registered with `{bytes, class: rt-pool | mask-cache | streamed-tile | effect-private | sim, priority, lastUse}`, one eviction policy per class, one total in every crash report. The frame-graph allocator and the B6 mask cache become its two largest clients; the streaming stack keeps its internals but reports into the same ledger. §14.2's "<1 GB @1080p" then stops being an aspiration and becomes an enforced ceiling with visible headroom. **Exit:** the crash report shows a single VRAM ledger whose total reconciles with its parts; deliberately over-subscribing on a stress scene triggers eviction, never context loss.

**P6 — Pipeline & permutation management: compile what the graph declares (M).**
Today shader warmup _polls_ `renderer.info.programs` until the count stabilizes (§13.9's `waitForCompiledPrograms`) — the A11 premature-"Ready!" bug is that gate's UX symptom, and a poll can only ever approximate "what will this frame need." Proposal: V3 pipelines are **declared** — each pass enumerates its shader permutations as a small explicit `#define` matrix (mask-packed?, indoor/outdoor?, shadow terms?, DRS upsample flavor?). Load-time warmup = compile the declared set asynchronously via `KHR_parallel_shader_compile` (already referenced by the V2 warmup — [canvas-replacement.js](../../scripts/foundry/canvas-replacement.js), [FloorCompositor.js](../../scripts/compositor-v2/FloorCompositor.js)) through the P3 governor; readiness becomes a _fact_ ("declared set compiled"), not a heuristic — A11's fix falls out structurally under V3. Budget the permutation count (target: ≤ ~64 programs for the whole V3 frame, reported in diagnostics); 48 bespoke effects collapsing into shared shader modules with flags is also how the ~104k effect LOC shrinks (§16.4, B4 note). **This is simultaneously the single highest-value WebGPU preparation** — a declared permutation _is_ a `GPURenderPipeline` descriptor (W rule 1). **Exit:** loading a golden scene compiles a known, logged pipeline list; "Ready!" appears only when that list is done; zero first-interaction compile hitches on pan/zoom/floor-change.

**P7 — Format & bandwidth discipline (S — rules adopted as each pass is built).**
Adopt now, while V3 has 4 passes instead of 40:

- **HDR targets:** audit each for alpha use; where alpha is not consumed downstream, prefer `R11F_G11F_B10F` (color-renderable under `EXT_color_buffer_float`, half the bandwidth/memory of RGBA16F). Where alpha is load-bearing (V2's solidity contract — which B0-1 relocates to `attr.a`), the descriptor documents it.
- **Single-channel data is `R8`/`R16F`**, never RGBA out of habit; masks channel-pack per Phase 2b.
- **Every pass declares clear-vs-preserve per attachment** and the graph enforces it — this is WebGPU `loadOp`/`storeOp` (W rule 2) and a real win on tiled/mobile GPUs today.
- **Present dithers:** [FullscreenPresent.js](../../scripts/compositor-v3/FullscreenPresent.js) currently has no dither before the 8-bit encode; banding on dark scenes is the most visible LDR artifact a VTT has, and an ordered/blue-noise dither is ~3 shader lines.
- **Half-res where softness allows:** expensive alpha-heavy screen passes (atmospheric fog, glow accumulation, clouds) render at `scale: 0.5` (the descriptor field again) with a bilateral upsample against `attr`/depth.
  **Exit:** a table in this doc listing every V3 RT with format + rationale; no RGBA16F target that doesn't need alpha; dithered present.

**P8 — Scene bake: move work from load-time to import-time (M–L — rides Phase 2/B6).**
Widen Phase 2's "preprocessing/export step" (§6, still an open §7 question) into a general **scene bake**: on first configure (or an explicit GM "Bake" action), produce per-scene versioned artifacts — mask tile pyramids, channel-packed atlases, low-res whole-floor fallbacks, outdoors classification at its true (low) resolution, optionally the Phase-6 SDF — cached in IndexedDB ([pyramid-indexed-db.js](../../scripts/streaming/pyramid-indexed-db.js) is already generic key→blob storage) and keyed by **content hash**, so a changed source invalidates precisely. Scene load then = fetch + upload under the P3 governor; recomputation happens only when a bake is stale and _never silently on the load path_. Add the **warm-start package**: a tiny (~1–4 MB) per-scene snapshot (present-capped albedo + lighting parameters) written after first successful load; a reload paints a correct dim first frame from it in <1 s while residency streams in behind it. This serves all three load priorities in order — reliability (nothing heavy left on the critical path), smooth (a real frame on screen immediately), quick (loads become I/O-bound) — and makes reloads, the A10 trigger, cheap by construction. **Exit:** second load of a golden scene reaches a correct first frame in <2 s with zero mask recomposition on the load path.

**P9 — Draw & bind discipline (S–M, opportunistic — measured, not dogma).**
The frame graph's timings say when draw count matters; when it does: `BatchedMesh`/instancing for static tiles/walls/map-points (§6 Phase 1 already notes this), particles keep quarks' batching, and per-frame/per-pass constants (camera, lighting environment, weather, ToD) move into shared `THREE.UniformsGroup` UBO blocks bound once — today module code sets per-material uniforms individually and UBOs are unused outside the vendored library (the `uniformBlock` strings in tile/token managers are GLSL snippets, not buffers). The frame/pass/draw three-tier block layout doubles as the WebGPU bind-group shape (W rule 3). **Exit:** opaque+attr pass ≤ ~200 draws on the Mansion at High; per-frame constants bound once per frame.

### 16.4 Sequencing — P interleaves with B; there is no "performance phase"

| When                                  | Do                                                                                                                                                              |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Now** (with the A10 tile-ready fix) | P3's task-budget rule — the A10 fix _is_ its first application, so land the rule with the fix; draft P1's budget table; W0 telemetry (one diagnostics field)    |
| **B1 / attribute-buffer work**        | P2 `renderScale` plumbing (trivial at 4 passes, painful at 40); P7 rules applied to every RT the attr work touches; P1's GPU timers into `FrameGraph.execute()` |
| **B2 clustered lighting**             | P6 declared permutations (lighting is the first big shader family); P4 `invalidateOn` for the lighting pass; the light/shadow budgets below                     |
| **B3–B4**                             | P4 rollout pass-by-pass; P9 as timings demand; B4 ports use the effect-manifest contract below                                                                  |
| **B6**                                | P5 residency ledger is the design center; P8 bake pipeline (the mask pyramids are its first artifact class)                                                     |
| **B7 (the flip)**                     | P1's harness gates it; the budget table becomes the release criteria alongside §14.2's VRAM number                                                              |

Two scope notes that belong to existing B-items (recorded here so they land in those specs):

- **B2 gains explicit runtime light/shadow budgets.** Clustered lighting bounds _fill_ cost; the CPU bin build and shadow producers still need a _count_ governor: max clustered lights per frame (importance-sorted by radius × screen coverage; overflow drops smallest), max **shadowed** lights (the rest render unshadowed), per-light update throttling for animated lights. Degradation must be deterministic and visible in diagnostics, not incidental.
- **B4's porting contract: effects become data + shared primitives.** Each ported effect is a **manifest** — `{masks consumed, passes contributed, permutation flags, params schema, invalidation inputs}` — over shared shader modules, not a translated 3k-line class. The ~104k-line effect roster is the module's maintenance debt, B4/B7 are the once-only chance to shrink it an order of magnitude, and the manifest is what makes P4 (invalidation), P6 (permutations), and P5 (private-RT registration) automatic for every effect instead of 48 hand-wirings. This is §14.1 principle 3 applied to _authoring_, not just runtime.

### 16.5 W-track — WebGPU: converge on its model, don't branch the codebase

**Reality check (verified 2026-07-14):**

- **The port boundary is the whole renderer.** WebGL2 and WebGPU cannot share textures or contexts, so there is no "port one pass" — a frame renders wholly on one API. (Foundry's PIXI staying WebGL is fine; the canvases are already separate.)
- **A `three.webgpu.custom.js` + `three.tsl.js` build is already vendored** ([scripts/vendor/three/](../../scripts/vendor/three/)) and currently unreferenced by module code — the option was pre-provisioned. Note Three's `WebGPURenderer` wants materials expressed as TSL/NodeMaterial, not raw GLSL `ShaderMaterial` — V2-style GLSL does not port for free.
- **Every V2-reuse bridge is WebGPU debt.** `V3PostBridge` running `ColorCorrectionEffectV2`/`BloomEffectV2`, `V3EffectsBridge` ticking V2 effect instances, and the `FloorRenderBus` dependency all drag raw-GLSL V2 code into the V3 frame. The B2-doc endgame (CC demoted to one thin grade) and B7 (V2 deleted) are therefore _also_ the WebGPU unblockers. **No WebGPU implementation work before B7** beyond W0/W1 — this is the honest version of "we can't focus on it much yet."

**The strategy — "WebGPU-shaped WebGL2."** Do not build a render-hardware-interface abstraction now (§16.6). Instead, every V3 pass obeys six rules that make it expressible in both APIs, enforced by review and by the frame graph's own shape:

1. **Immutable pipeline descriptors** (P6): blend/depth/format/defines fixed at declaration; no mid-pass state mutation. ≙ `GPURenderPipeline`.
2. **Explicit pass boundaries with per-attachment load/store intent** (P7): attachments + clear/preserve declared, never discovered. ≙ `GPURenderPassDescriptor`.
3. **Constants in blocks** (P9): frame/pass/draw UBO tiers. ≙ bind groups.
4. **Design to the stricter of the two APIs.** Precedent already set correctly: WebGL2's shared-blend-state-across-MRT rule (B0-1 §3.1's zero-write convention) is _stricter_ than WebGPU's per-attachment blend — and the convention works on both. Keep making that choice.
5. **Shader-source discipline:** V3-authored GLSL stays in a transpilable ES 3.0 subset, organized as shared modules with flags (P6), so the eventual translation (TSL rewrite or naga-class transpile) is mechanical per _module_, not per _effect_.
6. **No WebGPU-only features in the reference path — ever.** WebGL2 remains the floor indefinitely (Foundry's user base decides this, not us). WebGPU is an accelerator _tier_ selected at boot, golden-scene-gated, and may only ever add optional visual/perf headroom — never required capability.

**Milestones:**

- **W0 (S, now):** capability telemetry — record `navigator.gpu` presence + adapter info in the diagnostics/crash reports, so the user-base readiness number is _known_, not guessed, when W1 decides. Zero rendering changes.
- **W1 (S, at B7):** backend-seam decision spike — evaluate (a) Three `WebGPURenderer` + TSL rewrite of the by-then-small V3 shader set, vs (b) a bespoke thin WebGPU backend behind the frame graph (V3's real needs are narrow: textured quads, batched meshes, particles, fullscreen passes). Decide on evidence; the vendored builds make (a) testable in a day.
- **W2 (L, post-B7, gated):** port per W1; P1 budgets + golden parity required on _both_ backends; ship as an opt-in tier first.
- **What it buys when it lands** (recorded so today's shapes stay compatible): compute-shader light clustering (replaces the CPU bins), storage-buffer SVT feedback for B6 residency (replaces readback tricks), GPU particle simulation, `timestamp-query` per-pass timing (replacing the flaky WebGL2 extension), per-attachment blend (relaxes B0-1 §3.1 for future attribute channels).

### 16.6 Considered and deliberately not adopted (so they aren't re-litigated)

| Idea                                      | Verdict                             | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Renderer-in-a-worker (OffscreenCanvas)    | **Deferred indefinitely**           | Would immunize rendering against main-thread jank (the TDR class) — but MSA's per-frame contract is chatty with the main thread (CameraFollower reads PIXI stage state each frame; input routing; DOM overlays; Foundry hooks), and every such read becomes async messaging. The actual stall sources are _load-path_ JS, which P3/P8 fix where they live (decode workers already exist). Revisit only if P1 telemetry shows play-time (not load-time) main-thread jank. |
| A full RHI / device-abstraction layer now | **Rejected**                        | Solo-dev over-abstraction: a second API surface to maintain with one backend behind it. The frame graph + the six W-rules capture ~90% of the port surface; W1 decides the real seam when there are two real backends.                                                                                                                                                                                                                                                   |
| MSAA                                      | **Rejected**                        | 2D content; AA needs are edge-analytic (grid, walls, templates) and belong in those shaders. MSAA would tax every RT for nothing.                                                                                                                                                                                                                                                                                                                                        |
| TAA / temporal upscaling                  | **Deferred**                        | P2's spatial upscale first. TAA adds ghosting risk + motion-vector plumbing that a 2.5D VTT rarely justifies. Reconsider only if P2's quality at low scale is unacceptable on hero scenes.                                                                                                                                                                                                                                                                               |
| Deferred-shading G-buffer                 | **Rejected** (re-affirming B0-1/B2) | Alpha-heavy content, tiny material space; the attribute buffer already _is_ the thin G-buffer forward+ wants.                                                                                                                                                                                                                                                                                                                                                            |
| GPU-driven culling / indirect draw        | **Rejected for now**                | No compute/indirect in WebGL2; draw counts are low post-P9. Becomes a W2-era option if profiling ever asks for it.                                                                                                                                                                                                                                                                                                                                                       |

### 16.7 The visual ceiling this buys (V4 lookahead — ambitions on record, every one gated on a P1 budget line)

With the backbone in place, "visually powerful second" stops being risky: each candidate below is admitted only with a named budget line and a golden-scene diff, and none starts before B7 unless a B-item gets it for free.

- **Radiance-cascades 2D GI** — screen-space cascaded probes; near-constant cost at screen resolution (fits the §14.1 invariant by construction); would subsume much of window-light/candle bleed as _actual_ bounced light. The natural successor to the B2 model's `glow` term, and the single biggest "AAA look" unlock available to a 2.5D renderer.
- **SDF soft shadows / vision** — already Phase 6; slots in as a lighting-pass term once B2 proves the fragment loop.
- **Light shafts / volumetric hints** from skyReach + sun angles — a half-res post pass (P7 pattern).
- **Water polish:** planar-reflection SSR at half-res; shore foam driven by `attr.a` distance — after B5.
- **Normal-mapped clustered lighting** — the `_Normal` masks already exist; per-fragment once B2 lands.
- **Weather/wetness surface response unification** (specular × weather) and a LUT-based final grade in the thin post-B2 CC.

**Summary for a fresh session:** read §16.1's table to know what not to reopen; build every B-item against P1–P9; obey the six W-rules; and treat frame-time budgets as the definition of done. The plan was already architecturally sound — this section is what makes it an _engine_ plan rather than a rendering-correctness plan.
