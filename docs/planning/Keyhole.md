...
1b. **✅ THE COARSE-PIN BUDGET — FIXED (2026-07-17), UNVERIFIED IN-BROWSER.** Root cause: §4.1 says coarse pins are "~tens of pages TOTAL", written when one VT meant the whole world. The per-item VT design (Stage 2) gave every drawable its OWN virtual texture, and each independently called `coarseTopMipsForCap(table, {})` uncapped (~96 pages) — **nothing summed the total.** Measured on the author's real 16050×7650 3-floor scene: `coarseIntendedPages: 808`, `coarsePinShortfall: 246` (3 of 13 packs got ZERO, purely by cache-fill order), `freePages: 0`, `frameGapMaxMs: 2594.8` (a 2.6s freeze). Fix: `residency.js#computeCoarsePinBudget` — a fixed fraction of cache capacity (`DEFAULT_COARSE_BUDGET_FRACTION = 0.25`) divided EQUALLY across every pack in the WHOLE scene (deduplicated by item id across all floors, so a token visible from two floors isn't double-counted), recomputed every `updateResidency()` pass (never cached — the token-placement bug two commits earlier is the exact staleness class this avoids repeating). Masks get half the item's own share, capped at the original empirically-fine 24. Deliberately NOT priority-tiered (active floor vs. prewarm) — an equal share for every pack is what stops a background-prewarmed floor being silently starved to zero by load order, which is exactly what happened to 3 packs. **The honest tradeoff:** a big Level background that used to get ~82 coarse pages now gets ~39 on this scene — a softer coarse floor during heavy streaming, in exchange for a hard ceiling and no more freezes. Exposed live in diagnostics as `coarsePinBudget`. Sabotage-tested (reverted the division; the regression fixture's worst-case-total invariant caught it).
