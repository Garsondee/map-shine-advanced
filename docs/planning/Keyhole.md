...
**⬜ THE ONE REMAINING STEP: renderer wiring in `src/vt/vt-pan-viewer.js`.** Nothing is visible to the author until it lands, and all of its risky math is already proven. In order: (1) swap to the world-space ortho camera and delete `reframeLayer`/`reframeVisibleLayers`; (2) one mesh per draw item instead of one per floor (`side: DoubleSide` — a negative-scaleX tile reverses winding); (3) one VT pack per item, keyed by `item.id`; (4) **residency in IMAGE space** via `viewRectToImageRect` + `computeItemViewportPx` — the real change, since `streamPackResidency` currently assumes world space == image space, and passing the canvas size to `chooseMip` would make every small tile stream full resolution (the O(tiles) blowup through the back door); (5) `sortByLayer` → `mesh.renderOrder`; (6) feed items from `collectSceneLayers`, with the torture fixture synthesizing items too — **one renderer, do not fork a second path**. Known knock-on: `view-state.js` is still single-axis and its clamp should become the (rectangular) canvas rect.
