...
1. **The eviction bug** (dangling indirection texels) — **MECHANISM CONFIRMED**, a live symptom (2026-07-17): a moved tile "leaves a second copy behind". `vt-pan-viewer.js:1298`'s comment claims rebuilding the indirection fresh each pass "keeps it correct across evictions" — **true only for packs streamed in THAT pass**, because the rebuild lives inside `streamPackResidency`, which is per-pack and gated on `onScreen`. When item B's new pages evict item A's slots, A's indirection dangles until A's own next stream. It normally self-heals within a frame; it couldn't on the author's real scene because the cache had zero slack — **item 1b's fix restores that slack** (coarse pins no longer eat the whole cache), so this should now largely self-heal in practice. Still worth the real fix, not just relief: the cache should own an evict callback; no slot reusable until every texel pointing at it is cleared (fall back to coarse = blur, never another item's pixels). **Re-test after 1b before spending more time here** — it may already look fixed.
1b. **✅ THE COARSE-PIN BUDGET — BOTH HALVES LANDED (2026-07-17), UNVERIFIED IN-BROWSER.** Root cause: §4.1 says coarse pins are "~tens of pages TOTAL", written when one VT meant the whole world. The per-item VT design (Stage 2) gave every drawable its OWN virtual texture, and each independently called `coarseTopMipsForCap(table, {})` uncapped (~96 pages) — **nothing summed the total.** Measured on the author's real 16050×7650 3-floor scene: `coarseIntendedPages: 808`, `coarsePinShortfall: 246` (3 of 13 packs got ZERO, purely by cache-fill order), `freePages: 0`, `frameGapMaxMs: 2594.8` (a 2.6s freeze).
    - **Cut 1 — the ASK:** `residency.js#computeCoarsePinBudget` caps what each pack REQUESTS to a fair per-pack share (`DEFAULT_COARSE_BUDGET_FRACTION = 0.25` of capacity, divided across every pack in the whole scene, deduplicated by item id, recomputed every pass). Landed and measured **incomplete**: `coarseIntendedPages` dropped 808→423, but `coarsePinShortfall` was still **81** and `frameGapMaxMs` **barely moved** (2594.8→2536.6) — the SAME three packs (background-floor prewarm) still missed. The author caught this directly from a live report and asked "did you fix things?" — the honest answer was half.
    - **Cut 2 — the ROOM**, the actual fix: `page-cache.js` gained `coarseReservePages` — 'coarse' and 'view' were equally protected from LRU eviction with **nothing coordinating who gets first claim on a scarce cache**, so a busy viewport's 'view' pins could (and did) fill the whole cache before a background pack's ONE-TIME coarse request ever got a turn. A 'view' request now refuses (soft miss, same "not resident yet" state the sampler already handles) rather than push the view tier's own pinned count past `capacityPages - coarseReservePages`. Kept in lockstep with cut 1's `totalBudgetPages` (one source of truth). Also: `_setPin` now refuses to downgrade an already-coarse-pinned page to 'view' (routine at extreme zoom-out, where the current mip can coincide with a coarse-pinned one) — silently weakening the guarantee on the two MOST COMMON request patterns would have made the reserve nearly useless. New `coarseReserveMisses` counter: should read 0 forever; nonzero means the reserve itself is undersized, not routine pressure.
    - **Deliberately NOT "let coarse evict an already-pinned view slot"** — that would bump a page the user is CURRENTLY looking at off-screen to benefit a floor nobody is viewing. A reservation that occasionally costs the view tier some peak sharpness under heavy pan/zoom is the correct trade, not a compromise from the predictability-over-fidelity stance.
    - Exposed live in diagnostics as `coarsePinBudget` (cut 1) + `coarsePinBudget.cacheReserveCheck` / `cacheStats.coarseReservePages`/`coarseReserveMisses` (cut 2) — check `coarseReserveMisses:0` and `coarsePinShortfall:0` together; either alone is not the proof.
    - **A standing lesson recorded, not just fixed:** the interpretation text shipped with cut 1 asserted *"coarsePinShortfall should now read 0"* — unverified, and wrong. An instrument that states a claim instead of reporting a measurement is the exact failure `feedback_instruments_must_not_lie` exists to catch, and it recurred inside the very diagnostic meant to prove THIS fix. Fixed by re-wording cut 2's interpretation to point at the ground-truth field, not assert a number.
    - Sabotage-tested, both halves independently: disabling the reserve admission check reproduces the real symptom exactly (a coarse request misses under view pressure); removing the never-downgrade guard is separately caught.
1c. **⬜ THE 2.5s FREEZE — TWO REAL FINDINGS, ROOT CAUSE STILL UNCONFIRMED (2026-07-17).** The reserve fix (1b) does NOT touch this — `frameGapMaxMs` was 2594.8 before either cut and 2536.6 after both; this is a separate mechanism. Traced from the live report's own `decodeStats`: `sourcesDecoded:6` == `mainThreadFallbackSourceDecodes:6` (EVERY decoded source fell all the way to `acquireSliceSource`'s synchronous main-thread full decode — the exact operation `decode-pool.js`'s own comment calls "a giant-image decode the render loop could feel"), while `workerSourceDecodes:9` proves the worker itself was genuinely healthy (`workerStatus:'active'`, not crashed — checked BEFORE proposing anything, since a global worker-crash theory would have been directly falsified by that field). So: some requests succeed off-thread, others fall back to the slow path, per-request — and until now NOTHING recorded why, because `tryWorkerSlice`'s catch swallowed the real error (`catch(_){ return null; }`).
    - **Fixed, unambiguously correct regardless of root cause:** `getSourceDimensions`'s ranged-fetch path called `res.arrayBuffer()` unconditionally — if the asset server ignores the `Range` header and answers 200 with the WHOLE FILE (plausible; this scene's Level backgrounds can be hundreds of MB), that buffers the entire response just to read a 24-byte PNG header. Replaced with `readLeadingBytes()`, which reads only what's needed via the response's stream and **cancels** the rest. Node-tested (chunk-boundary assembly, truncation, the no-streaming-body fallback, and — the actual point — that the stream is genuinely cancelled, not drained). Sabotage-tested.
    - **Added, not yet exercised live:** `workerRequestFailures` in `getDecodeStats()` — a bounded log of `{kind, url, reason}` for every per-request worker failure, previously discarded. `workerStatus:'active'` proved the WORKER survives; this is what will show, by name, why *individual* requests still don't. Next live report — after moving/zooming to reproduce the freeze — should either populate this log (naming the real cause) or come back empty (which would itself mean some OTHER, still-unaccounted-for path reaches `acquireSliceSource`, a real gap this log makes visible rather than hides).
    - **Explicitly not claimed fixed.** The `readLeadingBytes` change is a real, independently-justified improvement; whether it's *the* freeze cause is unconfirmed. Given how many false “that’s the fix” moments this exact investigation already produced this session (twice, on the coarse-pin budget), the honest status is: two real defects found and fixed, root cause of the 2.5s stall still pending the next live report.
