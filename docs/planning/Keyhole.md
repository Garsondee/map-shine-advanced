...
1. **The pass runner.** Unchanged as the #1 gate ("what actually blocks effects", below) and re-affirmed here because it is *engine* infrastructure first: nothing walks `PASSES` to execute; the order lives hardcoded in `renderFrame`. Method is settled by precedent — the same small-step, author-live-verifies-each-round loop that did the `geometry.world`/`present.composite` split (`964820a`), never a big-bang extraction from the 1,778-line function.
2. **`frame.snapshot` goes live.** `world/{sun,environment}.js` + `core/frame-clock.js` are built, pure, Node-pinned — and unreachable from boot. Wiring one snapshot object into the loop (plus a debug-panel report showing time/sun/darkness as live facts) is reachability work, not design work, and it is the input bus every future pass reads. Pairs naturally with the runner (the runner is what hands the snapshot to passes).
3. **`buf:scene.attr` — the attribute buffer (B0-1), the keystone.** Only `scene.color` exists. `scene.attr` (floorId, outdoors, cover/overhead, material flags — §4.2) is what turns shadows, water occlusion and per-pixel floor gating into cheap screen-space reads instead of per-floor RT stacks. Its `outdoors` channel is already SERVED by the mask authority; writing it is the allocator's second real caller and the mask authority's first GPU consumer — two long-standing "first consumer unlocks it" notes close at once.
4. **The `masks.occlusion` producer.** The last piece of the original tiles directive, unblocked since tokens render. Not an effect: it is ONE screen-space RGBA mask + a 4-line shader test that every drawable consults. The two pre-found gaps are recorded in the LAYERING section above (`maskUV` fed world coords where it needs screen UV; `mapElevation` returning 1 for everything against the placeholder table) — fix them first or the mask will "work" and lie.

#### Track 2 — PROOF & INSTRUMENTS: "best rendering engine" becomes a measured claim, not a mood

5. **The performance baseline + regression harness.** Stage 2's formal gate metrics are STILL uncaptured (PIXI ≤ 60 MB, zero `texImage2D` > 32 ms, interactive ≤ 10 s) — the proxy severance is built (`foundry/pixi-proxy-textures.js`) but never certified. Capture them via a debug-panel report, plus frame-time percentiles for pan/zoom/floor-switch, and **persist the numbers as a baseline JSON in-repo** so any later session can diff instead of vibing. Re-measuring pan/zoom feel (open since the camera rewrite) is this same item. This should come FIRST — measure before touching the engine's spine, so the runner's cost (item 1) is a diff against a real baseline.
...
7. **The multi-scene validation matrix.** Exactly ONE real scene has been through the current path (the doc's own "Still open" admits it). A single debug-panel report run per scene — Church, Mansion, the torture fixture, plus one non-square multi-floor scene — each row a fact: pack count, VRAM by format, the `coarseReserveMisses`/`coarsePinShortfall` pair (the 1b watch item lands here), load time, error lists. The keyhole's whole promise is scene-size-independence; this is the instrument that checks the promise per scene instead of per anecdote.
...
10. **Fix the release workflow.** `.github/workflows/main.yml` zips `scripts/` (deleted at Stage 0) and never zips `src/` — every release it produces is a module that cannot boot, and it runs zero tests. Found 2026-07-17, still unaddressed, ~an hour. The best rendering engine ships.
...
14. **Close the absorb gap.** 5 V2 effects claimed by no V3 pass — accounting, not code; cheap; keeps `passes.js`'s "cannot silently drop an effect family" test honest.
