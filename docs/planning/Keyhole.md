...
**Author decision (2026-07-15, verbatim intent):** _"V2 is dead, long live the V3."_ No band-aids, no fallback paths, a serious and complete refactoring. This document is the main vector of thrust for every session that follows.
...
**Author directive (2026-07-16):** get _"Foreground Tiles"_ and _"Tiles"_ working, researching the real vendored Foundry source rather than guessing; and _"examine your layering system in general because eventually we're going to have a huge number of effects on lots of floors so a strong ability to control visual layering is going to be key to long term viability."_ Author chose full scope (foundation + foreground + tiles), chose to **draw tiles on every visible floor** (a deliberate divergence from Foundry — see below), and flagged **occlusion modes as critical**.

**The finding that reframes the whole task: "Foreground Tiles" is not a feature you build.** Foundry DELETED the `overhead`/`roof` booleans in v12 — `migrateOverheadTiles` reads `if (tile.overhead) tile.elevation = foregroundElevation`; the flag was converted into a number and dropped. A tile is "overhead in the Foreground" (the Tile sheet's own wording) **iff `tile.elevation >= level.elevation.top`** — which the UI labels _"Foreground Elevation"_. Implement one sort law over real elevations and both requested features fall out with **zero special-casing**. Full research, with source citations, is in project memory: `reference_foundry_v14_layering_law`.
...
Every drawable — level art, tiles, tokens, drawings, weather, and every future MSA effect — is ONE flat list under that key. **No band arithmetic:** sort the list, assign `renderOrder = index`. This replaces legacy's `LayerOrderPolicy.js` (`floorIndex*10000 + roleOffset`, 2400 slots/role), whose fixed capacity fails _silently_ on overflow, whose key is a floor ordinal rather than an elevation (so it cannot express "this roof is at elevation 10"), and which needs a new enum entry per effect type. MSA's own effect bands claim the gaps Foundry left (`SCENE_EFFECTS 250`, `TILE_EFFECTS 550`, `TOKEN_EFFECTS 750`). This is the direct answer to the author's long-term-viability concern.

**A Level is an elevation BAND, and that is the entire roof mechanism.** `_configureLevelTextures` places a Level's background at `elevation.bottom` and its foreground at `elevation.top` — _different elevations_. Tiles and tokens authored on that floor sit between them. A token at 5 is under a roof at 10 because `5 < 10`; nothing is flagged.

**Two wrong assumptions corrected — both invisible until now, neither an edge case:**

1. **World space is NOT the background image's pixel space.** The renderer used a fullscreen quad with UVs remapped to the visible region, so "UV" and "world position" were the same number. But `Scene#padding` **defaults to 0.25** (the canvas is ~1.5× the art, with the art _inset_ at `sceneRect`), `scene.width/height` are canvas pixels rather than the image's resolution (Foundry _fits_ the image), and tiles are authored in canvas space. All three were harmless while nothing but a full-scene background was drawn — the image simply _was_ the view. **Any tile exposes all three.** The 12,000² mansion is square with zero padding, which is luck, not correctness. World space is now Foundry canvas space (+Y down), and UV vs. world position are separate concepts.
2. **Non-square art threw, so most real scenes could not render at all.** `PageTable` assumed a square page grid. Square scene art is the exception, not the rule — this was a latent blocker sitting one scene away the entire time, on top of hard-blocking tiles (which are essentially never square). Now rectangular throughout, and cheaply: a mip halves both axes together, so the page **payload stays square** and the atlas/cache/upload/IndexedDB path is completely untouched — only the _grid_ is rectangular.
...
## 📍 CURRENT STATUS (updated 2026-07-18 — read this before §8's stage narrative)
...
`WebGPURenderer` + `NodeMaterial` + the VT sampler as TSL, GLSL fully deleted. Real 3-floor 16050×7650 scene: _"The real shader is working and not washed out. All floors work, layering is correct."_ Tokens render, placed and hit-tested correctly; **Foundry owns ALL input** (§4.7) — MSA's canvas is `pointer-events:none` and mirrors `canvas.stage` rather than owning a camera. The safety slide (§4.3) passed its first real unprompted test. Full story, including the TSL `.mix()` trap that cost a session (three simultaneous bugs from one wrong method form): `reference_tsl_method_chaining_trap` in memory, and the "Still open" list just below.

**🐞 ~~The one confirmed open correctness bug~~ — FIXED (2026-07-17); this paragraph outlived its own fix (corrected 2026-07-18).** `page-cache.js` now owns exactly the evict callback this paragraph asked for (`onEvict` — `page-cache.js:127/256`, registered by `vt-pan-viewer.js`, sabotage-tested against the race fixture) — item 1 of "Recommended next steps" below is the full account. Live evidence is INDIRECT: the 2026-07-18 compression session thrashed floors on the real Mansion scene with no artefact reports, but the original repro (drop a token at a full cache) has not been re-run on purpose. Corrected in place rather than deleted, because a stale "not yet fixed" sitting beside its own recorded fix is the doc-lying disease the ⚠️ CORRECTION section below names.
...
### 🗂️ THE MASK AUTHORITY — one source of truth for authored + derived content masks (2026-07-17, author directive)

**Author's ask:** _"a single source of truth for serving things... the mask textures themselves but... also the distribution center for derived or procedural masks/textures and resources"_ — with sky-reach ("I'm on the ground floor, are there any opaque art layers above me?") as the worked example. **Built and Node-proven; live browser verification pending (fixture button + `mask-authority` report).**

**The shape (boot is the composition root; no zone imports any other zone anew):**

- **`scene/mask-catalog.js` — the CANON.** Every authored kind (suffix + V2 aliases `_Windows`/`_Structural`→`window`, channels, the author-confirmed pack trio on R/G/B, **absence defaults** — outdoors absent = fully-outdoors, declared once, never guessed per-consumer) + every derived kind (`coverAbove`, `skyReach = outdoors × (1 − coverAbove)` — V2's own `legacy/masks/mask-catalog.js` definition, kept) as validated data (passes.js discipline: declaration order = evaluation order, cycles unwritable). `assembleLayerDescriptors` is the ONE packing policy — the torture fixture and real-scene discovery both go through it (trio packs iff all 3 exist; partial trios ship as singles, never a synthesized channel).
- **`foundry/mask-discovery.js`** — listing-first (ONE `FilePicker.browse` per unique art directory, zero 404 spam — deletes V2's three negative-cache layers), bounded ranged-GET probing as the _announced_ fallback (absolute URLs, denied browse). Method + failures + probe count always reported: "no masks" and "couldn't look" are different facts.
- **`scene/mask-authority.js` — the hub.** Serves with **provenance** (`source:'authored'|'default'`); ingests the pager's own coarsest-mip pages (each pack's `maxMip` page IS the whole item) via the injected `onPageDecoded` seam — **no second fetch, no second decode, no GPU readback, no cache pressure**; derives per-floor ≤512² Uint8 grids lazily (dirty-flag recompute at read, no clock, no scheduler); CPU query `sampleWorld('skyReach', floor, x, y)` costs one array read. Completeness is explicit (`missingItemIds` — a not-yet-ingested input is reported, never guessed; §4.1's soft-not-wrong).
- **Cover semantics, source-derived:** an item covers floor N iff `elevation ≥ level.elevation.top` — v14's own migrated home of `foregroundElevation` (`common/documents/scene.mjs:195`), so the floor's roof art (placed AT top) and upper floors count; the floor-9 rug does not. Null top → +Infinity → "no ceiling declared", visible in the report. Hidden items excluded and listed (players' sky is canon). Tokens never participate.
- **`masks/authority-only` (wall #23, at zero, sabotage-tested against the real tree)** — a mask suffix literal anywhere outside the catalog fails the build; the V2 alias suffixes and the not-yet-declared kinds (`_Roughness` etc.) are RESERVED, so a future effect must declare before it can hardcode. It fired on its own birth commit three times (boot's fixture literals; two prose strings; boot's inline `getImageData`, which `no-gpu-readback` then correctly exiled to `vt/decode-pool.js#readPageBitmapPixels`) — each fix made the tree cleaner, none widened a door.
- **Real scenes now stream masks** (`startRealSceneViewer` awaits discovery, then `extraLayersForItem` is a sync manifest lookup) — the vt-pan-viewer comment "real scenes store masks as scene flags... a later step" is superseded: the convention is sibling files per Level background, discovered.

**Deliberately NOT built (recorded, unwired-museum rule):** the GPU handle for derived grids (one small DataTexture through vt/ the day `light.visibility` — its first consumer — lands); per-page fine-mip extractors (Particles' spawn-point design owns that); tile-attached mask files (level backgrounds only, V2's authoring convention); mid-session re-discovery on a Level art URL swap (scene restart re-discovers, same limit as the pack cache).

**Watch on the next real-scene load:** pack count grows (each floor with masks adds up to +5 packs → the 1b coarse budget divides across more packs). Check `coarseReserveMisses: 0` and `coarsePinShortfall: 0` still hold together on the author's 13-pack scene — the exact pair cut 2 taught us to read together.

### 🗜️ TEXTURE COMPRESSION (BC1/BC7) — the WebGPU device-loss fix, full quality (2026-07-18)

**The bug this closes:** a real scene (Mansion, 12,000² × 2 floors, alpha overlays) lost the WebGPU device on a floor switch — the flight recorder caught it at `estTextureVramMB: 2190` while a second floor's art was still loading, past Chrome's measured ~2.5 GB device-loss wall on this hardware (a wall WebGL degrades past gracefully; WebGPU does not). Root cause chain, found across four rounds of flight-recorder evidence, not guessed:

1. **BC1 uploaded at non-multiple-of-4 dimensions** (1050×1050, 450×450 tile art) — WebGPU rejects this outright; a validation-error storm froze the scene art while PIXI chrome kept panning underneath it. Fixed: upload at the padded (×4-aligned) size, sample only the logical sub-rect via a `uUvScale` uniform — the edge-clamped padding sits at bottom/right, invisible.
2. **Uncompressed alpha layers** (overhead/roof overlays — BC1 has no alpha channel) stayed raw at ~549 MB each; two floors' worth breached the wall. **Author's call, verbatim: _"BC7 compress alpha — and don't be shy about it, no half measures."_** Full 12,000² pixel-perfect quality over any downscale, chosen knowingly against the tradeoff below.
3. **BC7 on a 144-megapixel image OOM'd the worker** (`getImageData` on the whole image = 576 MB peak, alongside the source bitmap and the block buffer) — the encode never returned, so the layer silently fell through to the 549 MB raw path and re-triggered the exact crash BC7 was meant to fix. Fixed with a **striped/banded encoder** (`encodeStriped` in `src/vt/block-compress.js`): pulls 4-row-aligned bands via a small band canvas, encodes each independently into the same output buffer — proven **bit-identical** to a whole-image encode (9 Node tests: multi-band, a final partial band, non-multiple-of-4 dims and strip heights all match byte-for-byte). Peak worker readback dropped from ~576 MB to ~25 MB/band.
4. **A real, separate visibility bug** surfaced only once the crash stopped hiding it: whole-image meshes were never hidden when their item left the draw list on a floor switch — the dropout-hide path only cleared `state.mesh`, which whole-image items don't use (they draw through `state.wholeImage.tiles[].mesh`). Symptom: the upper floor's roof stayed pasted over the ground floor after switching down a level. Fixed in `vt-pan-viewer.js`'s residency refresh loop.

**Instruments caught lying twice, per §0 rule 5 — fixed rather than trusted:**

- The VRAM diagnostic counted every whole-image tile as raw RGBA8 regardless of its actual GPU format — reporting a 68.7 MB BC1 floor as 549 MB, an 8× lie, at the exact moment the real number was needed most (mid device-loss hunt). Now honest per-format (`bc1ByteLength`/`bc7ByteLength` against the item's actual `compressed` field), in both the live diagnostics report and the device-lost crash snapshot.
- A per-item `alphaStats {min,max,mean}` field was added specifically because a BC7 item can read `status:ready, visible:true, compressed:"bc7"` — every other instrument agreeing it succeeded — and still render solid black; min/max near 255 vs near 0 is what separates "the decode was already wrong" from "the encode/upload is wrong" without guessing a third time. In the event the real answer was neither (see below) — the instrument still earns its place; it will catch the next one.

**✅ LIVE-VERIFIED, author's words: _"everything is full resolution and it's not crashing bravo/brava!"_** Both floors, BC1 backgrounds + BC7 alpha overlays, full 12,000² quality, survives repeated floor switching.

**⬜ THE LOADING-CURTAIN FIX — UNVERIFIED IN-BROWSER (2026-07-18, same session, after the confirmation above).** The remaining confusion: BC7 on a 144-megapixel image takes tens of seconds, genuinely serialized one item at a time (`wholeImageLoadChain` gates even the network fetch behind the PRIOR item's full worker round-trip), and this ran **entirely fire-and-forget** — nothing ever awaited it, not even for the floor the scene opens on. The loading curtain could drop claiming "Ready" while the viewed floor's own art was still invisible mid-encode. Author, naming it exactly: _"We need to do this encoding during loading I think otherwise people will get confused"_ — correctly spotting the same "0%/98% two-gate 'Ready!' lie" `load-progress.js`'s own header puts on the kill list, just relocated from the progress bar to the floor switch. Fixed by:

- Exposing `ensureWholeImageMeshes`'s previously-thrown-away promise as `wi.loadPromise` (always resolves — every error path sets `wi.status='error'` and does not re-throw, so it is safe to await unguarded).
- `startVtPanViewer`'s startup now **awaits** the viewed floor's compression, reporting real progress (`"Compressing textures" N of M`) through the existing honest `onLoadProgress`/`load-progress.js` machinery, before the curtain drops.
- The pre-existing "prewarm other floors" loop — previously a dead stub in whole-image mode, since it only probed source dimensions and never triggered compression — now also compresses **adjacent floors (±1)** in the background, non-blocking, so the common "step one floor at a time" case is often already warm by the time of the switch. **Deliberately NOT every floor**: a compressed whole-image texture has no eviction once built (unlike a coarse pin), so eagerly compressing an entire many-floor scene would grow VRAM without bound — reopening, through a different door, the exact ceiling this whole section was about closing. A distant floor jump (0 → 3 directly) still compresses lazily on arrival, same as before this change.
- **Not built:** re-evaluating the adjacent window as the author navigates (currently set once, from the scene's initial floor, not re-centered on each switch); a worker pool to run two encodes concurrently (would roughly halve the ~20–40s/item first-load tax for scenes with several alpha layers) — a real next lever, flagged rather than built unprompted, since it doubles peak worker CPU memory while both are busy.

### 🧭 THE INFRASTRUCTURE MENU — the non-effects track (2026-07-18, author directive)

**Author's ask, verbatim intent:** with albedo passes, tiles, background/foreground art and token rendering all live, choose next moves that are _"critical infrastructure for making this the best rendering engine possible"_ — explicitly **not** effects. This menu is the curated answer. It ABSORBS AND RE-RANKS the overlapping items in "Recommended next steps" (2026-07-17, in THE FRAMEWORK section below); where the two disagree, this section is newer.

**Two stale claims found and corrected while compiling it (a menu built on stale facts ranks the wrong things):**

1. The "one confirmed open correctness bug" paragraph above said the eviction/indirection bug was unfixed — it was fixed 2026-07-17; corrected in place.
2. Project memory (`keyhole-device-loss-large-map`) said the native `queue.copyExternalImageToTexture` upload fix was "not yet applied" — it **is applied**: `vt/atlas.js:311` calls the queue-level native path directly (2026-07-18, mock-proven in `atlas.test.mjs` to be exactly one call per page upload). The memory is updated alongside this edit.

#### Track 1 — THE SPINE: unblocks everything else, effects included; this is the architecture finishing itself

1. **✅ The pass runner — BUILT + NODE-TESTED (2026-07-18), NOT YET LIVE-VERIFIED.** `graph/run-frame.js` (new, pure, no THREE/DOM import — Node-testable like `passes.js`/`pass-health.js` before it): `planFrame(passes, {fromStage,toStage})` filters the real `PASSES` array to the ordered ids that are `status:'live'` within a stage range (array order IS frame order, per `validatePassGraph`'s own invariant, so this is a filter, never a sort); `runPassPlan(ids, impls, ctx)` calls each real implementation in that order and **throws, naming the id,** if a planned (`status:'live'`) pass has no registered impl — a live pass with no code is exactly the drift `pass-impls.js` exists to catch, now checked against a real frame instead of only the static registry. Deliberately does **not** sample any clock (`time/one-clock` restricts `performance.now()`/`Date.now()` to `diag/` + `core/frame-clock.js` — per-pass timing is item 6, built ON this, not folded in) and does **not** touch `vt.residency`/`frame.snapshot` (both have their OWN drivers — document hooks, not yet wired — that are not "once per drawn frame"; a caller scopes `fromStage`/`toStage` to the stages it actually owns).
   - **Wired into `vt-pan-viewer.js`, behaviour-identical.** `renderFrame`'s two hardcoded calls (`renderer.setRenderTarget(sceneColor); renderer.render(...); renderer.setRenderTarget(null)` and `presentQuad.render(renderer)`) are now named functions (`runGeometryWorldPass`, `runPresentCompositePass`) called through `runPassPlan(framePlan.ids, passImpls, {})`, where `framePlan = planFrame(PASSES, {fromStage:'geometry', toStage:'present'})` is computed once at setup (passes.js doesn't change mid-session). **Same two GPU calls, same order, same targets** — the only change is that the order is now looked up from the real declared graph instead of written inline. A Node test (`graph/__tests__/run-frame.test.mjs`, asserting against the REAL `PASSES`, not a fixture) pins today's plan to exactly `['geometry.world', 'present.composite']` — so a future pass added to this stage range with no `passImpls` entry fails that test, not a silently-dark screen.
   - **What this buys, concretely, and is the entire point:** wiring `masks.occlusion` (item 4 below) in requires exactly two edits — add `'masks.occlusion': runMaskOcclusionPass` to `passImpls`, widen `fromStage` to `'masks'` — and zero changes to `renderFrame`'s control flow. `passes.js` stops being a comment for every pass added inside this loop from here on.
   - **`graph/pass-impls.js` intentionally UNCHANGED** — it still honestly names `startVtPanViewer` as `geometry.world`/`present.composite`'s reachable entry point (`separatelyInvocable: false`), because externally that remains the only door in; `runGeometryWorldPass`/`runPresentCompositePass` are private closures inside that one instance, not new exports. No new claim was made there, so no test there needed touching.
   - **1765 assertions, 0 failed, lint/format/structure clean** (`npm run verify`, 2026-07-18). **NOT YET LIVE-VERIFIED** — same caveat as every prior touch of `renderFrame` (Rounds 1–9's own lesson): load a scene and confirm rendering is pixel-identical to before this change. `getDiagnostics().framePlan` (surfaced in the existing "Diagnostics" debug-panel report, zero new wiring needed) reports `{planned, skipped, ranLastFrame}` — `ranLastFrame` should read `['geometry.world', 'present.composite']` every frame, identical to `planned`.
2. **🔶 `frame.snapshot`'s `res:env` third — BUILT + WIRED + NODE-TESTED (2026-07-18), NOT YET LIVE-VERIFIED. Status STAYS `'future'` in `graph/passes.js`, deliberately.** The pass declares THREE outputs (`res:env`, `res:view`, `res:scene`) but only `res:env` (time/sun/weather/wind/darkness) has a real, fully-designed shape (`world/environment.js#buildEnvSnapshot`, already built+tested). `res:view`/`res:scene` have no real consumer yet to define their shape against — inventing one now would be designing for a hypothetical the doctrine already warns against. Flipping status to `'live'` with 2 of 3 declared outputs missing would hand `pass-health.js` a real `MISSING` finding the moment anything reads them from a live-status pass — exactly the drift this whole framework exists to catch. So: build the real third, wire it in for real every frame, leave the status honest.
   - **New:** `foundry/scene-environment.js` (`readSceneDarkness`/`deriveDarkness`, split pure-vs-live the same way `canvas-compositing.js` splits `decideArtSuppression`/`readCompositingFacts` — reads `canvas.scene.environment.darknessLevel`, the real v14 field, `common/documents/scene.mjs:117`, an `AlphaField`). "Could not read" (`source:'default'`, a reason string) and "read zero" are kept distinguishable, per doctrine #5.
   - **Wired into `vt-pan-viewer.js`:** `makeFrameClock()` + `buildEnvSnapshot()` called every frame via a new `updateEnvSnapshot()`, called directly (NOT through `runPassPlan`/`framePlan` — `planFrame` correctly skips a `'future'`-status pass, so genuinely-not-ready work stays hand-wired the same way `updateCamera()`/`syncTokenPlacements()` already are, never faked into the graph early to look more finished than it is).
   - **`todHour` has NO real source yet** (no calendar, no author-facing control built this session) — hardcoded to noon and labelled `todHourSource:'default:no-calendar-input-yet'` in diagnostics, rather than left as an implicit fallthrough. `world/sun.js#normalizeHour`'s own existing rule ("a broken input reads as noon") made explicit, not new behaviour.
   - **Surfaced in the existing "Diagnostics" debug-panel report** (zero new UI, same pattern as item 1's `framePlan`): `getDiagnostics().envSnapshot` = `{status:'future', env, darknessSource, darknessReason, todHourSource, notYetBuilt:['res:view','res:scene']}`.
   - **1784 assertions, 0 failed** (`npm run verify`, 2026-07-18; +12 in `foundry/`). **NOT YET LIVE-VERIFIED** — load a scene, check `envSnapshot.env.sun`/`.darkness01` are plausible numbers that update frame-to-frame (`env.time.frame` incrementing), and that `envSnapshot.darknessSource` reads `'scene'` on a real scene (not `'default'` — that would mean the darkness reader silently isn't finding `canvas.scene.environment`, worth checking live since this is genuinely new Foundry-reading code).
   - **Next, when picked up again:** `res:view`/`res:scene` need a real consumer to define their shape (the natural one is `masks.occlusion`, item 4 below, which declares `reads: ['res:scene', 'res:view']`) — build those two alongside that pass, then flip `frame.snapshot` to `'live'` honestly, all three outputs real at once.
3. **⬜ `buf:scene.attr` — the attribute buffer (B0-1), the keystone. SCOPED-NOT-ATTEMPTED (2026-07-18) — read before starting.** Only `scene.color` exists. `scene.attr` (floorId, outdoors, cover/overhead, material flags — §4.2) is what turns shadows, water occlusion and per-pixel floor gating into cheap screen-space reads instead of per-floor RT stacks. **On inspection, neither channel has a safe first slice the way `frame.snapshot`'s `res:env` did:** `outdoors` (the channel with a real spec, served by the mask authority) needs a genuinely NEW per-pixel GPU texture-sampling path — uploading the mask authority's derived grid as a DataTexture and a new UV mapping, real render-loop risk, not yet built. `floorId` (the channel with no new sampling needed) has NO real consumer yet to define what value it should even hold — inventing an encoding now would be designing for a hypothetical, the exact thing this project's doctrine warns against. **Do this together with `light.visibility`/`post.grade`'s FIRST real read** (whichever lands first), so the channel's shape is defined by its actual consumer, not guessed ahead of one — mirrors how `masks.occlusion`'s real producer only got built once tokens gave it something real to test against.
4. **🔶 The `masks.occlusion` producer — BUILT + NODE-TESTED (2026-07-18), RADIAL-ONLY, NOT YET LIVE-VERIFIED.** Status flipped `'seam'` → `'live'` in `graph/passes.js`, honestly scoped: the RADIAL (green) channel is real and runs every frame against real document data; FADE/VISION stay inert (unwired, matching their pre-existing behaviour); SURFACE — read the vendored `client/canvas/layers/masks/occlusion.mjs` before assuming this was a render-target problem alone — is driven by a **completely separate Foundry Region/Surfaces system** (`Scene#getSurfaces()`, `region.polygonTree`) this project has never touched. **Level/roof art defaults to SURFACE mode** (`foundry/scene-layers.js:273`), so **the headline "see the token under the roof" case is NOT yet affected** — that needs the Regions half, scoped as its own future item, not a corollary of this one. What IS real: any author-authored TILE with RADIAL `occlusion.modes` set now fades/reveals correctly for a real token disc.
   - **New:** `foundry/scene-occlusion-sources.js` (`readGridDistancePixels`/`computeTokenOcclusionRadiusPx`, replicating `Token#getLightRadius`'s formula from real document data — no PIXI Token object read, matching this project's documents-only adapter discipline).
   - **The render target is real:** `occlusionMask` is now a screen-sized RGBA8 target (allocator-created, ThreeAllocator's Nth caller), MIN-blended (`THREE.CustomBlending`+`MinEquation`+`OneFactor`/`OneFactor`, both RGB and alpha) — Foundry's own `PIXI.BLEND_MODES.MIN_ALL` composite rule, verified against the vendored source. A per-token disc pool (`occlusionDiscs`) reconciles every frame: geometry/materials built once per token (no per-frame shader recompile), position/scale/elevation-uniform updated every frame.
   - **Both `maskUV` bugs fixed** — the streaming path's ONE-LINE fix (`positionGeometry.xy` → `THREE.TSL.screenUV`) plus a NEW, matching occlusion chain added to `buildWholeImageMaterial` (whole-image mode is the DEFAULT active path for real scenes; wiring only the streaming material would have made this real but invisible on every deployed scene — the SAME "correct but never runs" trap already ruled out once this session for a different reason). Deliberately NOT refactored into one shared helper across both material builders — touching the streaming path's already-proven-live code beyond its one-line bug fix was judged the bigger risk; the two occlusion chains are hand-kept in lockstep instead.
   - **Camera-sharing, not a new transform:** the mask-drawing pass reuses the SAME `camera` (and same-sized target) as `geometry.world`, so `screenUV` lines up by construction — no new world→screen math was hand-derived, sidestepping this project's own recurring Y-flip bug class rather than re-risking it.
   - **Known, documented gap (not silently ignored):** `occlusionDiscs` entries are hidden, not disposed, when a token drops off the current draw list — full lifecycle cleanup needs a `deleteToken` hook wired from `boot.js`, not built this cut (bounded leak: grows with distinct tokens ever seen in a session, not per frame).
   - **1799 assertions, 0 failed** (`npm run verify`, 2026-07-18; +12 in `foundry/`). **NOT YET LIVE-VERIFIED** — this is real render-loop surgery (new render target, new MIN-blend material, a screen-space UV fix) built and reasoned through without any way to see a rendered pixel in this environment. `getDiagnostics().occlusion` reports `{elevationTable, poolSize, activeDiscs, rt:{width,height}}` — `elevationTable` should read something other than `[-Infinity]` whenever an occludable token (nonzero `occludable.radius`, not hidden) is on screen; a RADIAL-mode tile should visibly fade/reveal near that token.

#### Track 2 — PROOF & INSTRUMENTS: "best rendering engine" becomes a measured claim, not a mood

5. **✅ The performance baseline + regression harness — BUILT (2026-07-18), NOT YET RUN.** New debug report `stage-gate-baseline` (`src/boot.js`) captures, in one click: PIXI residency total vs the ≤60 MB gate (aggregates the pre-existing `pixi-residency-report`'s per-src entries into one number), load time vs the ≤10 s gate (`load-progress.js`'s own `lastLoad.totalMs`/`worstStallMs` receipt — already tracked, just never surfaced as a gate check), and frame-gap **percentiles** (p50/p95/p99, new — `vt-pan-viewer.js`'s existing 300-frame rolling window previously only exposed avg/max). **`texImage2D` timing is honestly reported `available:false`** — it's a V2-era WebGL-call-level metric with no V3 equivalent (a different upload architecture entirely — page-atlas vs whole-image), and inventing a number for it would be exactly the lying-instrument class this project has already paid for once. Deliberately READ-ONLY (calls existing accessors, changes no render behaviour) — the safe kind of work to bank while live verification is unavailable, unlike items 1–4 above.
   - Every gate value is quoted directly from Keyhole.md §1/§8 in the code, not reinvented from memory. A `pass:null` (vs `true`/`false`) means "not measured yet," never a guessed failure.
   - **NOT YET RUN** — needs a real scene, real pan/zoom/floor-switch input (frame-gap percentiles need real motion in the window), and a completed load (loadTimeMs needs `lastLoad` to be non-null). This is exactly the tool the next live session should reach for FIRST, before touching anything else — it verifies items 1–4 in one click instead of ad hoc clicking.
   - "Persist as a baseline JSON in-repo" from the original ask needs a human at a browser: run the report, copy the JSON, paste it back to Claude, who commits it. No debug report can write to the repo from inside Foundry.
6. **🔄 Per-pass GPU timing on WebGPU — PLANNED IN FULL, PHASE 0 LANDED (2026-07-27). Plan of record: `docs/planning/Performance.md`.** Rides the pass runner exactly as this item always said — `runPassPlan` gains `{onPassBegin, onPassEnd}` hooks and the clock stays in `diag/`, the one zone `time/one-clock` allows to read it. **But the harvested `GpuPassTimer` is NOT the reference, and has been DELETED** along with `v3-perf.js` (reachability ratchet 3 → 1): it wraps `EXT_disjoint_timer_query_webgl2`, structurally unable to measure the WebGPU backend this renderer actually runs, and it duplicates what vendored three already implements internally (`three.webgpu.js:67928`). The real path was hiding behind a constructor argument nobody passed — `trackTimestamp` is **constructor-only and defaults to `false`** (`:64637`), then AND-ed with the feature check (`:75258`), so the earlier "`supported:false`" reading was a never-armed flag, not a hardware limit; it stood in two headers as a hardware fact for a week. Granularity is per `renderer.render()` call, because three submits one command buffer per call (`:75866`) — exactly the granularity wanted, and it works on both backends. Deliverable: a debug-panel report carrying per-zone ms, the **un-attributed residual** (a breakdown that cannot account for the frame must not look confident), and measured ms/Mpx against each manifest's declared `cost.estMsPerMp` — closing a loop `effect-manifest.js` has validated since it was written and that has never once been checked against reality. Plus a live HUD and a camera-path-driven benchmark run, so "did my optimisation work?" is an A/B rather than two hand-panned sessions. Without this, every future "effect X is slow" hunt starts blind.
7. **✅ The multi-scene validation matrix — THE INSTRUMENT IS BUILT, THE MATRIX ITSELF IS NOT RUN.** `stage-gate-baseline` (item 5, same report) IS this item's per-scene row generator — running it once each on Church, Mansion, the torture fixture, and one non-square multi-floor scene produces exactly the matrix this item asked for (pack count, VRAM, the `coarseReserveMisses`/`coarsePinShortfall` pair, load time, error lists — all in one payload). One instrument now serves both items 5 and 7; building them separately would have been the same work twice.
8. **VRAM headroom tripwire.** The device-loss wall is MEASURED (~2.5 GB on the 3070/Chrome) and the flight recorder already computes `estTextureVramMB` honestly per-format. A loud warning at ~70% — naming the top consumers — turns the next device-loss hunt from four rounds of archaeology into one report. Instruments-must-not-lie applies: it reports an ESTIMATE against a MEASURED wall and says so.

#### Track 3 — LOADING, RESILIENCE, HYGIENE: cheap-to-medium, real leverage, good breather work

9. **BC encode worker pool (2) + re-center the ±1 prewarm window on floor switch.** Both flagged in the compression section above as the next levers; the pool roughly halves the worst first-load tax, re-centering makes "step one floor at a time" always warm. First-encounter cost only — encodes persist to IndexedDB.
10. **✅ Fix the release workflow — DONE (2026-07-18).** `.github/workflows/main.yml` zipped `scripts/` (deleted at Stage 0) and never zipped `src/` — every release since Stage 0 shipped a module.zip with NO runtime code (`module.json`'s `esmodules:["src/boot.js"]` pointing at a file the archive didn't contain). Fixed: `scripts/` → `src/` in the zip file list (every referenced path — `src/`, `templates/`, `styles/`, `languages/`, `assets/`, `data/`, `README.md`, `LICENSE` — verified to exist on disk before landing this). Also added the missing test gate the finding named: `npm ci` + `npm run verify` now run before packaging, so a release can never ship something the project's own gate would reject. Verified without a browser: the YAML parses cleanly (`js-yaml`, step order confirmed), every zipped path exists on disk — genuinely checkable outside Foundry, unlike the render-loop work above.
11. **Cache-on-disk governance audit.** IndexedDB now holds the page pyramids AND the BC block cache, keyed by URL — nothing is known to cap or evict either across scenes/worlds (NOT verified; that is the audit). A hit browser origin quota would present as mystery load failures. Deliverable: measured growth numbers + a policy (LRU by scene, version-keyed invalidation), or a written finding that caps already exist.
12. **The WebGL2 rung of the tiered ladder** (§4.3's deferred direction, §9 risk 6). TSL compiles both backends from one source, so the auto-detect rung (try WebGPU → WebGL2 → the safety slide) is now mostly boot wiring + a test matrix, not new architecture. GM-enforceable mid-session stays future. This is the reliability half of "best engine" — the author's own framing: reliability outranks visuals.
13. **Ratchet paydown + the `ui/` zone door.** Shrink-only, satisfying, no design needed; `ui/` is the last missing door. Includes the mechanical `vt-pan-viewer.js` → `scene-renderer.js` rename (its own commit; the 1,778-line function shrinks ONLY via item 1's extractions, never a rewrite).
14. **✅ Close the absorb gap — DONE (2026-07-18).** The 5 unclaimed V2 effects now have homes, each a real judgment call, recorded: `SceneWindField` → `frame.snapshot` (its 35 controls are exactly `world/environment.js`'s `DEFAULT_WIND` shape, already built this session). `InvertEffectV2`/`SharpenEffectV2`/`GridRenderer` → `post.grade`'s stylizer chain (matching the existing Ascii/Halftone/DotScreen/Sepia entries there; GridRenderer's claim is scoped to its cosmetic ghost-grid/styling layer only — Foundry's own grid _drawing_ stays with PIXI's interface group regardless, per `keyhole-interface-seam`, so nothing here contradicts that). `VisionModeEffectV2` → `light.accumulate`, marked `(SUPERSEDED — stays with Foundry, not MSA-rendered)` — the same honest DELETED-style marker `DynamicLightShadowLift` already uses, since fog/vision deliberately stay with Foundry until `keyhole-vision-fog-direction`'s "eventually." Regenerated `docs/reference/v2-effect-params/` via `tools/harvest-params.mjs` (a machine-generated reference — hand-editing it would just be overwritten next run): **"0 effects are claimed by NO V3 pass"**, down from 5.

**Suggested sequence, stated once:** 5 (baseline first — never operate on an unmeasured engine) → 1+6 together (runner + timing) → 2 → 3 → 4, with Track 3 items slotted as breathers between rounds. The params service (old item 5 below) is deliberately NOT in this menu's spine: it still has no data waiting for it and its first real consumer is an effect — it re-enters with Stage 6.

---

## 🏗️🏗️🏗️ THE FRAMEWORK — a full V2 audit + a skeleton built from it (2026-07-16/17)

**Author directive that started this:** _"Take every horrible thing you learned about V2... burn it into your memory... we must avoid the problems of the original module at all costs."_ Followed by: _"Is there a way we can make a non-functional skeleton... a rigid but correct shape... in a way that future LLMs cannot fail but act inside of?"_ Then: _"I would rather this project was too fussy about following rules than too lenient."_
...
1. **"the other 9 — including `zones/one-door` — sit at zero, walled before the code that would exploit them exists."** `zones/one-door` was **ratcheted at 19**, not zero. And 4 of the 8 zones it governs (`vt/`, `graph/`, `foundry/`, `ui/`) **had no `index.js` to comply with** — 11 of the 19 violations are `boot.js`, which _could not_ obey a rule whose door did not exist. That is not indiscipline; Skeleton.md §0 called this exact shot: _"a defect in the skeleton, not in the person who routed around it."_
...
Much of that is legitimately "the rooms aren't built yet" — a seam door _should_ be unreachable. But the shape is exactly V2's: **`EffectComposer` had 5 importers and the god-object had 92.** Here the skeleton is not wrong, it is **detached** — a scale model beside the building, while the building (`vt-pan-viewer.js`: 2,318 lines, one **1,778-line function** spanning lines 304–2082) grows past it. The next wall this project needs is probably a **reachability ratchet**: an unreachable module must be accounted for by a `seam`/`future` declaration, or it fails the build. That is the check that would have caught 5-vs-92 in V2 during the week it happened.

### ✅ WHAT IS ACTUALLY BUILT (not just designed) — all Node-tested, all in `npm run verify` (true as of 2026-07-17)

- **`tools/verify-structure.mjs` — 23 walls as of 2026-07-17, 9 ratcheted** (existing debt frozen, shrink-only — `tools/structure-ratchets.json` is the live ledger and `npm run verify` prints the live count; the tally HERE has drifted stale twice already, so trust the tool, not this sentence). Newest: `log/one-door` (ratcheted 30) and `masks/authority-only` (zero — born with the mask authority, sabotage-tested on its birth commit). Every unratcheted wall sits at **zero** — walled before the code that would exploit it exists.
...
**Test count (discovered, not hand-counted): 12 suites · 1,615 assertions · one command (`npm test`), as of 2026-07-17 (mask authority landed).** vt 288 · foundry 330 (incl. mask-discovery) · graph 238 · scene 233 (incl. mask-catalog/derive/authority) · diag 114 · core 59 · ui 71 · world 47 · effects/particles 21 · the 174-case wall regression proof · the gate's own 16 · reachability 24. The runner discovers suites off disk — `npm test`'s own final line is the live number; this sentence is a snapshot.

### 🔒 THE LAW WAS NOT INSTALLED (found + fixed 2026-07-17, commit `5776a11`)

Keyhole is **named** for one law — §0's _"Nothing is ever allocated at world resolution. Ever."_ — made physical by `ThreeAllocator`'s >2048px throw (§4.6). It had **zero callers**, which alone is only "the rooms aren't built" (no pass allocates a target yet, so there was nothing to route). The real defect was worse:
...
Fixed by **enforcement by absence**: THREE is a required constructor argument; there is no global left to go stale. The refusal names the fix and cites §4.6. Six assertions now pin the _door_, not just the room. **The generalisable lesson: test the way IN, not only the behaviour inside.** Every wall in this repo was proven to bite; none was proven to be passable.
...
- **`gpu/textures-in-vt-only`** — because the single biggest measured offender was never a render target, it was **one texture**: the 8250² `LightCovers.webp` at **345 MB**, part of PIXI's ~719 MB (§1). `new THREE.Texture(sourceBitmap)` on a 12K map is one line, looks entirely reasonable, and _is_ the crisis. `vt/` is the sanctioned door.
...
**`graph/reachable-from-boot` (new, ratcheted at 14)** — the wall for the whole _class_ of problem, not just this instance: a static import-graph walker rooted at `boot.js`, ratcheted shrink-only like the other 7. Wiring the doors above paid the count down from 28 to 14 for free, and auto-tightened `zones/one-door` 19→15 as a side effect.
...
**`docs/reference/v2-effect-params/` — 45 effects, 2,240 controls, 25 carrying the author's own prose.** Machine-extracted from `legacy/` by `tools/harvest-params.mjs` before Stage 7 deletes it. Each page is cross-referenced to the V3 pass that replaces it (from `passes.js`'s `absorbs`), so the index answers _"I'm building `post.grade`; what did the 13 effects it replaces do?"_.
...
> _"Given that every single effect is going to be rewritten from scratch I think the parameters and things that I've built up are only really useful as a reference to the sort of features I'd like to see eventually. We're going to be building the effects in modern TSL so it's very likely that even using the exact same settings wouldn't give us the result we want."_

**Do not port the values.** `intensity: 1.23` was taste expressed through V2's GLSL math, and that math is being deleted — the number is not portable, the INTENT is. `Params.md` §4's _"the values are the product"_ was wrong and is now struck.
...
1. **It kept the numbers and binned the prose** — extracted `help.summary`/`help.glossary` (which `Params.md` §4 itself calls _"above all… irreplaceable"_) into a variable and never wrote it out. Exactly inverted.
...
### Recommended next steps, in order (updated 2026-07-17, after the interface seam — **RANKING SUPERSEDED by 🧭 THE INFRASTRUCTURE MENU in CURRENT STATUS, 2026-07-18**; the per-item detail below is still the authoritative account of each item)
...
1. **✅ THE DANGLING-INDIRECTION BUG — FIXED (2026-07-17), UNVERIFIED IN-BROWSER.** Author, after a zoom/floor thrash test: _"lots of weird tile artefacts... tiles of textures at the wrong scale and in the wrong place appearing across the scene. I think you need a full audit of that system, the one that maps and serves mips."_ The audit found it, and the earlier diagnosis (recorded here as "item B evicts item A's PINNED slots") was **wrong** — pinned pages of either class cannot be evicted, so that mechanism was impossible. The real one is sharper and lives entirely **inside a single pack's own `streamPackResidency`**: 1. **release** — `cache.unpin(key)` for pages the new view no longer wants. They stay RESIDENT and become LRU-evictable. **The pack's indirection still points at them.** 2. **`await requestDecodeUpload(...)`** — yields. The render loop draws frames across this await, and evictions (this pack's own new requests, or any other pack's) reassign those very slots. 3. **rebuild** — the indirection is finally rewritten, correct again.
   Between 1 and 3, every drawn frame samples through pointers that may already be lying — resolving to a different mip (**wrong scale**), different coords (**wrong place**), or another pack's texture entirely. It renders as confident garbage, never as blur or magenta. Under thrash (**11,608 evictions** in the author's session) that window is hit constantly. - **The instructive part:** two comments, each individually correct, with an unexamined interaction. `streamPackResidency`'s release-first comment reasons about PIN COUNT ("unpinning is safe — the page stays resident") — true, and it must NOT be reverted; release-first is what caps peak pinned at max(old,new) and fixes the 215k-miss bug. The rebuild comment reasons about the END of the pass ("rebuilt fresh every time — correct across evictions") — also true. Neither asked what the indirection referenced BETWEEN them. - **Fix — enforce the invariant where the slot's identity actually changes, not by reasoning about which windows are safe:** `page-cache.js` gained an `onEvict(key)` callback, fired the instant a slot stops backing its page and before it is rebound. `vt-pan-viewer.js` registers one that zeroes that key's indirection texel (`pageOwners` maps key → the exact texel; safe even for stale entries because texel↔key is a **bijection** within a pack — no other key can ever write that texel). A cleared texel reads "not resident" → the sampler walks up to the coarse pin → **blur**. That is §4.1's "not loaded yet means SOFT, not WRONG" applied to the one case that was silently violating it. - Sabotage-tested: suppressing `onEvict` reproduces the real symptom, caught by the race regression fixture (which models the actual release→await→rebuild sequence and asserts _no texel points at a slot that no longer backs it_), not merely by "the callback didn't fire".
   1e. **✅ THE GHOST — FOUND, FIXED (2026-07-17), UNVERIFIED IN-BROWSER. It was the MIP BLEND, not the pager.** Author, after a thrash: _"a partially transparent version of a section of the map appearing very large and in the wrong position... fully zooming in on that transparent ghost copy doesn't cause it to get evicted."_ Every one of those four properties is `vt-sample.tsl.js`'s bracket cross-fade, and the author's own report carried all three numbers that prove it: - `prefetchSkippedPacks: 7` (of 11) — under pressure `streamPackResidency` DECLINES the speculative tiers, so `plan.prefetchCoarser` (mip 2) never streams while `plan.fine` (mip 1) always does. - `mip.requested: 1`, `requestedFraction: 1.494` → blend weight t = **0.494**. - `coarseTopMips: 4` — mips 3..6 are coarse-**pinned**, never evictable.
   So `colorLo`'s walk finds mip 1 (sharp); `colorHi`'s walk asks for mip 2, finds it absent, and falls back to **mip 3 — the coarse pin**. `mix()` then cross-fades the sharp image with a **4x-coarser** one at 49.4%: _partially transparent_ (literally the mix weight), _very large_ (4x coarser features), _never evicted_ (mip 3 is pinned; mip 2 is never coming). The author's screenshot — the same content visibly duplicated at two scales — is exactly this. - **The fallback was never the bug.** §4.1's "not loaded yet means SOFT, not WRONG" is right. The bug is **blending a fallback against a non-fallback** — producing something WRONG out of two things each individually correct. Fixed: `sampleFromMip` now reports the mip it ACTUALLY landed on, and the cross-fade only runs when both brackets resolved where they were asked. On a mismatch `colorLo` passes through alone — by construction the finest resident level, so this is both the sharpest and the honest answer. - **Why the geometry was ruled out first, with data:** `drawList[].placementPx` came back sane and identical for every non-token item (`{x:8025, y:3825, w:10650, h:4950}` — this scene's tiles are full-map overlays), which killed the misplaced-quad hypothesis outright and forced the search onto the sampler. That diagnostic existed only because the previous three explanations had each been wrong. - ⚠️ **Cannot be Node-tested** (TSL emits WGSL/GLSL; no browser here). The out-parameter relies on TSL **inlining** `sampleFromMip` — which is the default; a typed by-value function requires `.setLayout()`, absent here. **The failure mode is safe**: if the assignment did not propagate, both found-mips stay `-1`, the guard is always false, `t` is always 0, and `colorLo` passes through — a poppier mip transition, no ghost, nothing broken.

1d. **✅ A 404'd ASSET IS NO LONGER RETRIED FOREVER — FIXED (2026-07-17), UNVERIFIED IN-BROWSER.** Found in the same audit, confirmed by the author's own report: ten identical `HTTP 404` entries for one token image, `mainThreadFallbackSourceDecodes: 12`. `ensureItemLoaded` throws for a broken source, so `itemStates` never gets an entry, so it was re-attempted on **every** residency pass — seventy-seven of them in that thrash session — each paying a ranged fetch, a worker dimensions round-trip, AND a main-thread fallback decode attempt (the operation `decode-pool.js` itself calls "a giant-image decode the render loop could feel"). **`itemLoadErrors` deduped the REPORT; nothing deduped the WORK** — so the report looked tidy at one broken item while the cost repeated forever. A genuine contributor to the freeze budget (1c), found only because the artefact audit read the same report again with a different question. Fixed with a `failedItemIds` set, declared beside `itemLoadErrors` precisely because they are two halves of one thing (what the author SEES vs. what the loader DOES — letting them drift is what hid this). **The deliberate trade:** a source that starts working later now needs a reload rather than self-healing next pass. Correct way round — a 404 is overwhelmingly _gone_, not _late_, and paying an unbounded per-frame cost forever on the chance it returns is the reactive-mechanism shape Keyhole exists to delete. The failure stays LOUD: permanently in `layerLoadErrors` in every report, never silently skipped. 2. **The pass runner** — see "what actually blocks effects" above. This is the real gate for Stage 6, and every session that adds an effect before it makes it harder. 3. **Ratchet paydown** — **8** one-door (was 15; `foundry/index.js` paid 7), 33 catches, 41 clocks, 12 Foundry reaches, 7 global-bus, 14 unreachable. `ui/` is the last missing zone door. Shrink-only, satisfying, no design needed. 4. **Close the absorb gap** — 5 V2 effects are claimed by no V3 pass (see the harvest section above). Cheap, and `passes.js`'s own test asserts the 48→~12 collapse "cannot silently drop an effect family" — which is exactly what 5 unclaimed effects look like. 5. **The params service** (get/set/subscribe) — `Params.md` §6 build order item 3. Note it has **no data waiting for it**: the harvest is a reference, not a schema, so the first real params arrive when the first V3 effect declares its own (informed by, never copied from, `docs/reference/v2-effect-params/`). 6. Name and build out `masks.occlusion` (the last piece of the original tiles ask, now unblocked since tokens render) or `light.visibility`/`light.accumulate` — whichever the author wants to see rendering first. **`light.visibility`'s sky-reach input is now SERVED** (the mask authority derives it per floor — see "THE MASK AUTHORITY" above); its first GPU consumer is what unlocks uploading the grid as a DataTexture through vt/.
...
1. **✅ ~~There is no pass runner~~ — BUILT (2026-07-18), NOT YET LIVE-VERIFIED.** `graph/run-frame.js` + the CURRENT STATUS infrastructure-menu entry above have the full account. `renderFrame` now executes `runPassPlan(planFrame(PASSES, {fromStage:'geometry', toStage:'present'}).ids, passImpls, {})` instead of two hardcoded calls — same two GPU calls, same order, now looked up from the declared graph. The eventual gate this closes for real: the first effect that flips `seam`→`live` needs one `passImpls` entry and (if it's outside `geometry..present`) a widened stage range — never a hand-edit to `renderFrame`'s control flow.
...
Asked for as a follow-on to the pass runner; measured and deliberately **deferred rather than half-done**, because it is a different _kind_ of task than everything else in this section — not a refactor of existing code, a piece of **unbuilt architecture**.
...
1. **One path per behavior.** No fallback that routes through legacy code. If a capability isn't built yet, the feature is _absent and fails loudly_ — it is never quietly served by V2.
...
5. **Instruments must not lie** (added 2026-07-16, earned the hard way — five self-deceiving instruments in one session, each costing a live round-trip; see CURRENT STATUS's methodology note and `feedback_instruments_must_not_lie`). _"I could not measure this"_ must never look like _"the thing is broken."_ Never encode a result on an axis where a failure lands on a valid value. **Every skip, drop and early-return reports its reason and its id** — `skipped: []` must mean _nothing was skipped_, never _I didn't look_. When a measurement contradicts a working system, suspect the measurement. And when printed values say a bug is impossible but the screen disagrees, **suspect the OPERATION, not the values**.
...
V3 (scripts/compositor-v3/) is **not being discarded — it is being completed.** It got the _drawing_ right: frame graph, unified geometry, Foundry-v14-accurate forward lighting with wall clipping, DRS governor, per-pass GPU timing, present/tonemap. All of that is harvested nearly intact (§6).
...
**Page format.** 256×256-texel RGBA8 pages, 4-texel borders baked on all sides (248² payload). 256 KB per page. Rationale: big enough that indirection tables stay tiny, small enough that one upload is invisible (~0.25 MB `texSubImage2D` — the 100–300 ms giant-upload class of GL stall becomes _unrepresentable_).

**Physical cache.** One `THREE.DataArrayTexture` (or N 4096² atlases if array-texture filtering disappoints): 4096² holds 16×16=256 pages = 64 MB/layer; 8 layers = **2048 pages = 512 MB, allocated once at boot, never resized, never exceeded.** Budget scales by GPU tier (4 GB → 256 MB, 8 GB → 512 MB, 16 GB+ → 1 GB) but is _fixed for the session_. LRU eviction; two pin classes that are never evicted:

- **Coarse pins:** the top mips of every layer of every floor (whole world at low res — ~tens of pages total). Guarantees the _entire scene renders, always, instantly_, just soft. There is no "grey fallback" state and no "not resident → crash" state. Worst case is blur, never black, never loss.
...
**Per-page CPU extraction (kills the getImageData class).** Any CPU consumer of mask pixels (fire spawn points, vegetation clump fields, map-point seeding) registers a per-page extractor that runs _in the decode worker at decode time_ on 248² pages, accumulating world-space results incrementally. The 8250² 260 MB `getImageData` in `fire-behaviors.js:readImageRgba` — flagged in every crash report — becomes structurally impossible.

### 4.2 The frame graph and O(screen) render targets (`src/graph/`, `src/scene/`)

Harvest V3's FrameGraph/ThreeAllocator/GpuPassTimer/v3-perf/FullscreenPresent nearly whole. Pass list (evolves, but the _inventory_ is law-bound):
...
**Proxy textures — prevention, not demotion (the single biggest instant win).** Intercept Foundry's texture loading for scene backgrounds and tile documents _before_ PIXI ever decodes them, and hand PIXI a ≤1024px proxy generated from our own pyramid (the mip already exists in IndexedDB). Foundry never touches the 8250² file. This kills, permanently and by construction: the 719 MB PIXI residency, the 345 MB LightCovers hostage, the continuous 6408×5121 re-upload storm, _and_ the browser-process decode spikes on every `canvas.draw`. The five generations of demotion/sweep machinery become deletable. (Foundry scales tile meshes to document dimensions regardless of texture resolution, and the PIXI canvas is visually suppressed under MSA anyway — the only observable cost is lower-fidelity alpha in PIXI-side hover hit-tests. Accepted.)

**What Foundry remains authoritative for (documents & simulation — never rendered by PIXI on our watch):** scene/tile/token/wall/light documents, vision & fog _computation_ (`ClockwiseSweepPolygon` LOS/FOV polygons — CPU data we consume), levels flags, turn order, chat, all DOM UI.
...
§4.4 says WHICH effects exist and where they land. **`Effects.md` says how each one is LAID OUT so it costs what the machine can afford** — the author's tiering directive (_"the most basic tier being deliberately lightweight"_, §9 Q3) taken to a spec. Read it before porting any effect.

Its thesis is this plan's own, moved from the memory axis to the time axis: **tier 0 is the effect's coarse pin** — always compiled, always drawn, cheap enough for the floor card, never gated. Worst case is flat, never absent. _Water is always blue._ And it resolves `keyhole-stage6-effects-approach`'s standing tension (design the full feature set, ship a minimal slice, don't let the deferred list rot) by making the ladder and the deferred list **the same artifact**: tier 0 IS the MVP, tiers 1..N ARE the recorded feature set, ordered and costed, and a rung nobody's hardware reaches is visible rather than forgotten.
...
`Effects.md` says how an effect is TIERED. **`Effects-API.md` says what an effect may TOUCH** — designed from a direct audit of V2's ~97,000 lines of effects, at the author's direction (_"look for the things that each effect consumes and produces... a sensible API structure... would turn a mess of individual effect concepts into a unified plan"_).
...
**THE finding, and it reframes what "design a better API" has to mean:** `legacy/effects/EffectComposer.js` ALREADY has the right design — declared layers with numeric order (`BASE:0 … POST_PROCESSING:500`), independently the same shape as this plan's own sort law. **It is used in one place outside its own file, a console helper. It has 5 importers; `FloorCompositor` has 92.** V2 did not fail from lack of design — the good abstraction existed and 46 effects walked past it, because the god-object was easier. **A good API that is optional is a good API that loses.** This is doctrine #5 (_"make relapse harder than doing it right"_) proven in this codebase at a cost of 97k lines — so the contract must be the ONLY door, enforced by absence (an effect's `ctx` holds exactly its declared reads and there is nothing else to reach with), never by policy.

**The hopeful finding:** the Lighting↔Fire knot is **fake**. `wallPaddingPx` is a _default param value_ — a constant — that Fire fetches by rummaging in Lighting. Two effects SHARING AN INPUT, expressed as one reaching into the other. The real graph is a DAG. Declaring inputs does not manage that knot, it **dissolves** it — and most of V2's "interweaving" is likely the same shape, which makes Stage 6 less entangled than 97k lines suggests. Much of that bulk is _wiring_ (bespoke setters, floor-index resolution, overlay lifecycle, perf plumbing) that exists ONLY because there is no contract, and evaporates under one.

### 4.5 Loading & floor switching — reliability > smooth > quick, made structural

- **Initial load:** mount page tables → stream **coarse pins** (tiny — the whole world soft-focus in well under a second of decode) → reveal behind the existing curtain with _honest_ progress (pages resident / pages needed) → sharpen progressively under the upload governor. No 45-second all-or-nothing storm; no un-chunkable populate loop; no fake "Ready!". Interactive target on the torture scene: **≤ 10 s** on the 3070.
- **Floor switch = uniform write + working-set shift.** Coarse pins for _every_ floor are always resident, so the target floor renders instantly (soft for ~200–500 ms while its sharp ring streams). **No curtain, no rebuild, no `canvas.scene.view` redraw cost** (and with proxy textures, even Foundry's own level redraw becomes ~100× lighter). The level-transition curtain is deleted with V2. This is the "floor changes without loading screens" headline promise, delivered by construction rather than by cache-warming heroics.

### 4.6 Enforcement — making the law physical

- `graph/ThreeAllocator` **throws** on any texture/RT allocation with a dimension > 2048 unless it is the page atlas itself or on the explicit whitelist (`fogExploration`, present chain). A world-res allocation is a crash _in dev, at the call site, with a stack_ — not a context loss in the field three weeks later.
...
- Gating the input LISTENERS was not enough. The render loop's camera integration runs **per-frame regardless of input**, easing toward a target captured at load and REPLACING `view` wholesale — so it overwrote each adopted value and dragged the view back every frame. _Two cameras fighting, reproduced inside the very change meant to prevent it._ **This also caused the "misplaced tiles while zooming"** (see the eviction bug in CURRENT STATUS — the fight was thrashing residency into the dangling-texel defect).
...
- Driving the camera off the `canvasPan` hook and `await`ing a full residency rebuild per event made pan _laggy_. The frame loop reads `canvas.stage` directly instead — two property reads, so per-frame is cheaper than the hook AND cannot drift: no event to miss, no ordering to get wrong.
...
**Consequence for §4.3's gameplay severance:** "tokens/walls/templates rendered natively in `src/gameplay/`" still holds for _rendering_. It does **not** extend to input. Foundry hit-tests; MSA draws.

### 4.8 Tokens as drawables (`src/foundry/scene-tokens.js`) — landed 2026-07-16

Tokens are **not a subsystem**: they enter the ONE flat draw list at `SORT_LAYERS.TOKENS` (700) through the same law as everything else. No new pass, no new layering machinery — _the whole point of the law being one flat list is that a new kind of drawable is just another key._ No vision, no fog, no detection modes, no light, no rings (§4.7: Foundry hit-tests; and vision stays Foundry's per §4.3).
...
This satisfies the fresh-white-sheet need _and_ keeps the parts bin and the reference shaders one folder away.
...
> ⚠️ **2026-07-27 — two entries struck.** `GpuPassTimer.js` and `v3-perf.js` were harvested as planned, sat with zero callers, and are now DELETED (§Track 2 item 6; `docs/planning/Performance.md`). Do not re-harvest them: the first is WebGL2-only and cannot measure the WebGPU backend, the second's budget table is keyed to stage names that match nothing in `graph/passes.js`. `FullscreenPresent.js` is likewise slated for deletion (GLSL in an all-TSL codebase — see `graph/index.js`'s header). **A harvest manifest is a list of what to move, not proof that what moved earned its place.**
...
**UI shells (rebound to new internals):** loading-screen service/manager, graphics-settings manager+dialog (drops V2-era knobs), tweakpane panels per effect as each effect lands (the _parameter schemas_ are part of the product).
...
**Stage 1 — The law, running** _(~1 wk)_
...
**Stage 2 — Real art + the proxy severance** _(~1 wk)_
Real scenes (Church, Mansion) through the pyramid/VT path. Foundry proxy-texture interception live.
**Gate:** both scenes render sharp albedo at native; **PIXI ≤ 60 MB**; zero `texImage2D` > 32 ms in a full session report; interactive ≤ 10 s. _(The crash class observed all 2026-07-15 is dead at this gate.)_

**Stage 3 — Unified geometry + attribute buffer + lighting** _(~1–2 wk, mostly harvest)_
...
**Stage 4 — Masks virtualized** _(~1–2 wk)_
...
**Stage 6 — Effects long tail, look-parity per effect** _(~3–6 wk, parallelizable; water first, then fire, vegetation render, weather/clouds, shadows-unified, stylizers)_
**Gate per effect:** side-by-side vs 0.5.x reference screenshots signed off by the author; budgets hold; soak clean. The module is _playable and pretty_ throughout — effects arrive as upgrades, not as blockers.

**Stage 7 — Exorcism** _(days)_
...
**Honest total: ~2.5–4 months to full parity**, with the crash class dead at ~week 2–3 (Stage 2), the architecture proven on the torture scene before any effect is ported, and a usable dev build from Stage 3 onward. Every stage ends with something the author can _see_.
...
5. **Relapse into band-aids** — the doctrine (§0), the allocator throw, and the import fence exist precisely to make relapse _harder than doing it right_.
...
_V2 is dead. Long live the V3._
