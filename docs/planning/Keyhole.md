...
6. **Per-pass GPU timing on WebGPU** (timestamp queries). Rides the pass runner — the runner is the one place a timing wrap belongs, so building them together costs less than either alone. Harvested `GpuPassTimer` is the reference; the deliverable is a debug-panel report: ms per pass, headroom vs the frame budget. Without this, every future "effect X is slow" hunt starts blind.
