...
1d. **⬜ A 404'd asset is retried FOREVER, every pass** (found in the same audit, 2026-07-17, confirmed by the author's own report: 10 identical `HTTP 404` entries for one token image, `mainThreadFallbackSourceDecodes: 12`). `ensureItemLoaded` throws for a broken source, so `itemStates` never gets an entry — and it is called again on **every** residency pass (77 passes in that thrash session). Each retry costs a ranged fetch + a worker dimensions round-trip + a main-thread fallback decode attempt. `itemLoadErrors` dedupes the *report* but nothing dedupes the *work*. Real, cheap to fix (remember permanently-failed item ids and skip them), and a genuine contributor to the freeze budget — but a behaviour change (a fixed asset would then need a reload), so worth doing deliberately rather than folding into an artifact fix.
