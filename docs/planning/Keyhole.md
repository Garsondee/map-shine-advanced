...
### ✅ THE PARAMS HARVEST — DONE (2026-07-17, commit `c87500f`)

45 real `getControlSchema()` definitions (2 of the earlier "47" were call sites, not definitions — 11,922 lines of source, ~2,200 params) extracted by `tools/harvest-params.mjs` into `src/effects/params-harvest/*.js`, mechanically — not transcribed by hand. **2,225 params, 45 effects, 0 `validateParamsSchema` errors.** Wired for real: `effects/index.js` exports `checkParamsHarvest`; `boot.js`'s new `params-harvest-health` debug report re-validates every harvested schema live, every session — the generator ran once, but the CHECK runs every time, on purpose (this whole session's own lesson).

Five real extraction bugs found and fixed against real data (not synthetic fixtures) — a destructured-default parameter's own `{` truncating a schema mid-literal, `const` TDZ errors from append-ordering a dependency chain (fixed with a genuine topological sort, not a heuristic), a regex naming an effect "with" from a JSDoc comment's prose. Two genuine V2 data quirks caught and fixed with reported, honest exclusions (a `min===max` deprecated-locked slider; an enum default authored in label-space instead of value-space). One real collision with an existing wall: `LightingEffectV2.dynamicLightShadowOverrideStrength` is the exact fossil `shadow/no-lift-no-combine` forbids — excluded from the harvest using THAT WALL'S OWN pattern as the exclusion criterion, so the two can never drift apart. Full account in the commit message and `tools/harvest-params.mjs`'s own header.
...
3. **Ratchet paydown** — 15 one-door, 33 catches, 41 clocks, 12 Foundry reaches, 7 global-bus, 13 unreachable. Shrink-only, satisfying, no design needed.
4. **The params service** (get/set/subscribe) — the harvest gives it 2,225 real params to serve immediately; `Params.md` §6 build order item 3.
