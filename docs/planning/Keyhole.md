...
1. **The eviction bug** (dangling indirection texels) — **MECHANISM NOW CONFIRMED, and it has a live symptom** (2026-07-17): a moved tile "leaves a second copy behind". `vt-pan-viewer.js:1298`'s comment claims rebuilding the indirection fresh each pass "keeps it correct across evictions" — **true only for packs streamed in THAT pass**, because the rebuild lives inside `streamPackResidency`, which is per-pack and gated on `onScreen`. When item B's new pages evict item A's slots, A's indirection dangles until A's own next stream. It normally self-heals within a frame; on the author's real scene it cannot, because the cache never has slack. Fix per the original diagnosis: the cache owns an evict callback; no slot is reusable until every texel pointing at it is cleared (fall back to coarse = blur, never another item's pixels).
1b. **⚠️ THE COARSE-PIN GUARANTEE IS BROKEN ON A REAL SCENE** (found 2026-07-17, same report — this is arguably ahead of the eviction bug, because it is what makes the eviction bug fire constantly instead of healing). Real 16050×7650 3-floor scene, 13 packs: `coarseIntendedPages: 808` vs `coarsePinnedPages: 562` → **`coarsePinShortfall: 246`**, `freePages: 0`, `residentPages: 2048/2048`, all pinned (562 coarse + 1486 view), `misses: 1044`, `evictions: 660`, **`frameGapMaxMs: 2594.8`** (a 2.6-second freeze). §4.1's "not loaded yet means SOFT, not wrong" does not hold here — 246 coarse pages that are supposed to be permanently resident are not. The 2026-07-15 admission-control work was demoted to "a safety net for genuinely oversubscribed scenes"; **this is a genuinely oversubscribed scene, and it is the author's ordinary map, not the torture fixture.** Needs a real decision on the per-pack coarse budget (13 packs × 82 = 1066 coarse pages alone), not a tweak — and Stage 4's masks multiply every item.
