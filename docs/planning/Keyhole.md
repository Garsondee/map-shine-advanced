...
**Author's ask:** *"a single source of truth for serving things... the mask textures themselves but... also the distribution center for derived or procedural masks/textures and resources"* — with sky-reach ("I'm on the ground floor, are there any opaque art layers above me?") as the worked example. **Built and Node-proven; live browser verification pending (fixture button + `mask-authority` report).**
...
2. **Uncompressed alpha layers** (overhead/roof overlays — BC1 has no alpha channel) stayed raw at ~549 MB each; two floors' worth breached the wall. **Author's call, verbatim: *"BC7 compress alpha — and don't be shy about it, no half measures."*** Full 12,000² pixel-perfect quality over any downscale, chosen knowingly against the tradeoff below.
...
**Author's ask, verbatim intent:** with albedo passes, tiles, background/foreground art and token rendering all live, choose next moves that are *"critical infrastructure for making this the best rendering engine possible"* — explicitly **not** effects. This menu is the curated answer. It ABSORBS AND RE-RANKS the overlapping items in "Recommended next steps" (2026-07-17, in THE FRAMEWORK section below); where the two disagree, this section is newer.
