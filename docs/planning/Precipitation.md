...
- **TRAMPLE STAMPS (footprints) are NOT built.** The `trample01` channel exists, is integrated, and heals on schedule; nothing WRITES it. Trample is the one row of §5.2 that is not an integrator — it needs a CPU-side queue fed from token motion and a seam into the token layer — and half of it shipped beside a working mantle would be a channel that heals but never dents, which reads as *"footprints are broken"* rather than *"footprints are next"*.
- **§5.3's specular REQUEST is still a request.** `stay.surface.roughnessDelta` is authored on both rows (snow roughens, wet stone smooths — opposite signs, which is itself why they are two channels with two blend ops) and nothing consumes it yet. Same protocol as §4.2's water ask: an input nothing reads is rot on the far side of a seam.

Also worth stating precisely: the mantle's BUFFER is verified on real hardware (the bench reads texels back through a full snow→thaw→dry cycle), while its OVERLAY DRAW is not — the two blend meshes join the world scene inside the geometry pass's MRT scope, which the precipitation bench has no way to reproduce. The MRT neutrals follow `water-render.js`'s documented discipline exactly (white for the multiply mesh, zero for the alpha mesh) but that is an argument, not a measurement.

Consequently the species rows carry `arrive.kind`/`splashArchetype01`/`smearWithWind` and the four archetypes — and a `stay` block, and still carry **no** `waterRing`, `bounces` or `restSec`, each of which is asserted absent by `precip-species.test.mjs` until its own reader exists.
