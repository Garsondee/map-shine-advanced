...
| 3   | **`light.visibility`** — the shadow pass                             | `seam`                     | **Shadows.** Sun visibility = authored `_Shadow` ∧ building ∧ sky-reach ∧ cloud, min-combined. Dynamic lights already have Foundry wall-clipped LOS. Shadow = _absence of a specific light_, no lift, no combined-shadow. | `Light-and-Shadow.md`               | frame.snapshot, scene.attr                |
