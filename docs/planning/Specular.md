...
⚠️ **SUPERSEDED 2026-08-05 — read this note before the mechanism below.** Everything in this section describes the `buf:scene.attr`-based floor gate as it existed THROUGH 2026-08-04 (R11/R12, then the 2026-08-01 polarity inversion). It was replaced entirely by the depth authority's single ordinal rank comparison against `buf:scene.depth` — one `step(uExpectedDepth, depthHere)`, no floor-index equality, no separate presence bit, because a Tile's `sortLayer`/a Level foreground's `zIndex` already outrank their own background in the real paint-order rank, subsuming what `PRESENCE_BIT_OCCLUDES_BACKGROUND` used to flag by hand. Current mechanism: `docs/planning/Depth-Buffer.md` §9i and `keyhole-specular-built` memory's own "ROUND 21" section. Kept below for the lessons — the alpha-blend margin trap and the fail-open polarity argument are both still real, general lessons even though the specific bits they were fixing no longer exist.
