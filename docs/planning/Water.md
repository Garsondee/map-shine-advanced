...
1. **⚠️ SUPERSEDED, 2026-07-26 — Resolution is 3× the mask's own grid, not equal to it.** The Phase 2 correction here read: *"the pack is the size of its input… a flood run at 2× its input stores interpolation, not information."* Live-confirmed WRONG: run 1:1 with a 512-wide mask, the flood is provably exact (Node-tested against brute force) but its SEEDS sit only every ~21 world px, so a shoreline crossing at an angle bakes in a real staircase of seed positions — not a filtering artefact, and no amount of downstream blur on the resolved field recovers it. Reported live: *"an extremely pixelated mask which has been blurred… the shoreline looks like a square wave."* Fix: `WATER_BODY_SUPERSAMPLE = 3` (worst case 512×3=1,536, still safely under Keyhole's 2,048px cap) plus switching `WATER_MASK_FILTER` to `linear`, so the flood samples a *continuously reconstructed* version of the same coarse source at 3× its native resolution — the standard technique for exactly this problem, not "inventing data" (the coarse mask is genuinely all the information there is; supersampling with linear reconstruction is the best estimate of what happens between its texels). ~25 MB, still comfortably inside the water/fog reservation, and lands close to the *original* 1024² spec's own cost — this restores the step the first correction had cut too far, rather than reopening the whole decision. Named, not solved: the presence test's near-zero threshold against a linearly-reconstructed value places the interface slightly toward the land side of the source texel rather than at its exact midpoint (bounded by one *source* mask texel); a binarised-presence-channel refinement would center it and is a recorded, deferred rung.
...
2. **Interactive ripple integrator.** A damped wave equation `h'' = c²∇²h − k·h'`, with the body pack's SDF as a **reflecting boundary** — waves bounce off banks. Impulse sources: rain (from `res:env.weather`), tokens wading, doors swinging into water, splash impacts, authored disturbance anchors. **V2 had nothing that propagated.** *(2026-08-16: the rain impulse now has a designed producer — `Precipitation.md` §4.2. It also files a smaller pre-sim request against tiers 2/3: a statistical `rainAgitation01` expanding-ring term on the wave normal, so rain-on-water doesn't wait for this rung; the integrator subsumes it when built.)*
...
| **4** | 🔶 tier 1 (volume) BUILT — Beer-Lambert depth + the wet band; tiers 2-3 (motion, light) remain | side-by-side vs 0.5.x reference, author sign-off |
