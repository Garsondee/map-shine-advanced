...
- **O1 — cross-floor interleave artifact:** transparents from different floors that must visually interleave at the _same_ depth slice (e.g. falling ash through a stairwell reading as popping when the camera crosses floor boundaries), or
...
If triggered: OIT applies to the _offending band only_ (per-floor-band accumulation), never the whole transparent set. This is deliberately narrow — full-scene OIT changes the look of every Normal-blended overlay and would violate golden-scene parity by construction.
