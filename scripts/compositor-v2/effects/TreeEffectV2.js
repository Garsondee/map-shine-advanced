  ...
  _applyClumpCoordMapForOverlay(tileId, tex, deriveAlpha, placement, imageDataOpts = null) {
    ...
    if (sharedClumpKey) {
      ...
      if (shared?.texture) {
        ...
        log.debug(`TreeEffectV2 clump map (shared): ${tileId} (${shared.islandCount} islands)`);
        return;
      }
    }
...
${VEGETATION_PAINTED_SHADOW_APPLY_GLSL}
    ...
    this._applyTreeOverlayRenderOrders(shadowMesh, mesh, tileId, floorIndex, opts);
      ...
      maskLoadQueue.enqueue(() => {
        ...
        try {
          if (liveEntry?.shadowMesh && liveEntry?.mesh) {
            this._applyTreeOverlayRenderOrders(liveEntry.shadowMesh, liveEntry.mesh, tileId, floorIndex, liveEntry);
          }
        } catch (_) {}
      }).catch(() => {});
    }).catch((err) => {
    ...
    for (const [tileId, entry] of this._overlays) {
      if (entry?.streamingCellKey && entry?.streamingGridKey && entry?.mesh && entry?.shadowMesh) {
        ...
        if (Number.isFinite(streamRo) && streamRo !== entry._lastStreamAlbedoOrder) {
          ...
          this._applyTreeOverlayRenderOrders(
            entry.shadowMesh, entry.mesh, tileId, entry.floorIndex, entry,
          );
        }
      }
   ...
   * @private
   */
  _applyTreeOverlayRenderOrders(shadowM, canopyM, tileId, floorIndex, opts = {}) {
    try {
