...
import { createCloudDisplaySpriteMaterial, createCloudShadowSpriteMaterial } from './cloud-shaders.js';
...
export const MAX_ACTIVE_SPRITES = 120;
...
export class CloudSprite {
  constructor(THREE, sparseTextures, fullTextures, params) {
    ...
    this.warpSeedX = 0;
    this.warpSeedY = 0;
    /** Secondary norm-space orbit for independent drift. */
    this.orbitPhase = 0;
    ...
    this.displayMaterial = createCloudDisplaySpriteMaterial(THREE);
  ...
  assignTexture(texture) {
    if (this.displayMaterial.uniforms?.map) {
      this.displayMaterial.uniforms.map.value = texture;
    } else {
  ...
  setDisplayOpacity(opacity) {
    ...
    if (this.displayMaterial.uniforms?.opacity) {
      this.displayMaterial.uniforms.opacity.value = v;
    } else {
      this.displayMaterial.opacity = v;
    }
  }
