/**
 * Node verification for ThreeAllocator.js + its FrameGraph integration, using a
...
export function makeTHREE() {
  const T = {
    ...
    RGBAFormat: 'RGBA', UnsignedByteType: 'U8', HalfFloatType: 'F16', FloatType: 'F32',
    NoColorSpace: 'none', SRGBColorSpace: 'srgb', LinearSRGBColorSpace: 'linear-srgb',
  };
  T.Vector2 = class { constructor(x = 0, y = 0) { this.x = x; this.y = y; } };
  T.WebGLRenderTarget = class {
    constructor(w, h, opts = {}) {
      this.width = w; this.height = h; this.opts = opts; this.name = '';
      const count = opts.count || 1;
      const mk = (i) => ({ minFilter: opts.minFilter, magFilter: opts.magFilter, type: opts.type, format: opts.format, colorSpace: opts.colorSpace, name: '', _i: i });
      if (count > 1) { this.textures = Array.from({ length: count }, (_, i) => mk(i)); this.texture = this.textures[0]; }
      else { this.texture = mk(0); this.textures = undefined; }
      this._disposed = false; this._resizes = [];
    }
    setSize(w, h) { this.width = w; this.height = h; this._resizes.push([w, h]); }
...
export function run(t) {
  ...
  // describe(): the attribute-buffer MRT shape.
  {
    ...
    const d = ThreeAllocator.describe(T, {
      resolvedW: 1920, resolvedH: 1080, mrtCount: 2, depth: true,
      type: T.HalfFloatType, colorSpace: T.LinearSRGBColorSpace,
  ...
  // create(): applies per-attachment plan.
  {
    ...
    const rt = alloc.create('attr', { resolvedW: 100, resolvedH: 100, mrtCount: 2, attachments: [null, { filter: 'nearest', colorSpace: T.NoColorSpace }] });
  ...
  // Integration with FrameGraph.
  {
    ...
    g.declareResource('albedo', { size: 'screen', mrtCount: 2, depth: true, type: T.HalfFloatType, attachments: [{ filter: 'linear' }, { filter: 'nearest', type: T.UnsignedByteType, colorSpace: T.NoColorSpace }] });
    g.declareResource('hdr', { size: 'screen', type: T.HalfFloatType });
    let albedo = null, hdr = null;
    g.addPass({ name: 'geometry', writes: ['albedo'], execute: (c) => { albedo = c.target('albedo'); } });
    g.addPass({ name: 'lighting', reads: ['albedo'], writes: ['hdr'], execute: (c) => { c.get('albedo'); hdr = c.target('hdr'); } });
