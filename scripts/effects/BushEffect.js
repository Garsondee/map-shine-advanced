...
export class BushEffect extends EffectBase {
  constructor() {
    ...
    this.params = {
      ...
      // -- Wind Physics --
      windSpeedGlobal: 0.5326,   // Multiplier for actual game wind speed
      windRampSpeed: 0.89,       // Inertia: Lower = slower fade in/out of movement
      gustFrequency: 0.01,       // How distinct the "waves" of wind are (Spatial)
      gustSpeed: 0.2028,         // How fast the noise field scrolls

      // -- Bush Movement --
      branchBend: 0.007,         // How far the "branches" move in strong wind
      elasticity: 4.48,          // Higher = snappier return, Lower = lazy heavy branches

      // -- Leaf Flutter --
      flutterIntensity: 0.0001,  // Base vibration
      flutterSpeed: 2.31,        // Speed of vibration
      flutterScale: 0.02,        // Spatial scale of flutter clusters (leaf size)
  ...
  static getControlSchema() {
    return {
      ...
      parameters: {
        ...
        windSpeedGlobal: { type: 'slider', label: 'Wind Strength', min: 0.0, max: 3.0, default: 0.5326 },
        windRampSpeed: { type: 'slider', label: 'Wind Responsiveness', min: 0.1, max: 10.0, default: 0.89 },
        gustFrequency: { type: 'slider', label: 'Gust Spacing', min: 0.01, max: 0.5, default: 0.01 },
        gustSpeed: { type: 'slider', label: 'Gust Speed', min: 0.0, max: 2.0, default: 0.2028 },
        branchBend: { type: 'slider', label: 'Branch Bend', min: 0.0, max: 0.05, step: 0.001, default: 0.007 },
        elasticity: { type: 'slider', label: 'Springiness', min: 0.5, max: 5.0, default: 4.48 },
        flutterIntensity: { type: 'slider', label: 'Leaf Flutter Amount', min: 0.0, max: 0.005, step: 0.0001, default: 0.0001 },
        flutterSpeed: { type: 'slider', label: 'Leaf Flutter Speed', min: 1.0, max: 20.0, default: 2.31 },
        ...
        shadowLength: { type: 'slider', label: 'Shadow Length', min: 0.0, max: 0.1, default: 0.05 },
        shadowSoftness: { type: 'slider', label: 'Shadow Softness', min: 0.5, max: 5.0, default: 4.0 }
      }
    };
  }
