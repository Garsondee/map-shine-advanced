    ...
    this.material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // DEBUG: Test if explored texture is using wrong UVs
          // If explored moves with camera but vision doesn't, the explored texture
          // might have been accumulated with screen-space UVs instead of world-space
          // Uncomment one of these to debug:
          // gl_FragColor = vec4(visionUv, 0.0, 1.0); return; // Show world UVs (should change when panning)
          // gl_FragColor = vec4(vUv, 0.0, 1.0); return; // Show screen UVs (should NOT change when panning)
          // gl_FragColor = vec4(explored, 0.0, 0.0, 1.0); return; // Show explored in red
          // gl_FragColor = vec4(0.0, vision, 0.0, 1.0); return; // Show vision in green
          // gl_FragColor = vec4(explored, vision, 0.0, 1.0); return; // Show both (explored=red, vision=green)
