...
import {
  LOADING_PROGRESS_STAGES,
  formatEffectsInitMessage,
  formatGpuUploadMessage,
  formatShaderCompileMessage,
  formatTextureLoadMessage,
} from '../ui/loading-stages.js';
...
import { loadCoordinator, CoordinatorState } from '../core/LoadCoordinator.js';
...
function _setCreateThreeCanvasProgress(step) {
  ...
  try {
    if (window.MapShine) {
      window.MapShine.__createThreeCanvasProgress = { ..._createThreeCanvasProgress };
    }
  } catch (_) {
  }
}
  ...
  setTimeout(() => {
    safeCall(() => {
      ...
      const bail = !activeScene
        ...
        || _threeContextLost
        || !sceneSettings.isMapShineRenderingActive(activeScene);
      if (bail) {
...
async function _attemptWebglRecoveryRebuild(reason) {
  ...
  if (!scene || !sceneSettings.isMapShineRenderingActive(scene)) return false;
...
/**
 * First `canvasReady` on a cold world load can run before `#map-shine-canvas` has a
 * non-zero layout. `setSize(0,0)` / SceneComposer camera math then produce a broken
 * first paint; `ResizeHandler` also ignores 0×0 debounced updates, so nothing heals
 * until a full reload path (e.g. scene round-trip) re-initializes with a real rect.
 *
 * @param {HTMLElement|null} canvasEl - Usually `renderer.domElement` after attach.
 * @returns {{ width: number, height: number }}
 */
function _resolveInitialViewportCssPixels(canvasEl) {
  ...
  try {
    ...
    const bw = Number(br?.width) || 0;
    const bh = Number(br?.height) || 0;
    if (bw >= 1 && bh >= 1) {
      log.warn(`[loading] map-shine-canvas was ${w}x${h} at attach; using #board ${bw}x${bh} for initial viewport`);
      return { width: bw, height: bh };
    }
  } catch (_) {
  }

  const iw = Math.max(1, Math.floor(Number(window.innerWidth) || 800));
  const ih = Math.max(1, Math.floor(Number(window.innerHeight) || 600));
  log.warn(`[loading] map-shine-canvas and #board not laid out; using window ${iw}x${ih} for initial viewport`);
...
/** Legacy V1 input routing; removed — kept null so dead branches stay safe. */
...
function _publishPixiVisibilityDiagnostics(extra = {}) {
  try {
    ...
    mapShine.__pixiVisibilityState = {
      ...
      msaSceneEnabled: !!(canvas?.scene && sceneSettings.isMapShineRenderingActive?.(canvas.scene)),
...
/**
 * Whether a Region document represents a legacy MeasuredTemplate (Foundry v14+).
 * @param {object|null|undefined} doc
 * @returns {boolean}
 * @private
 */
function _msIsMeasuredTemplateRegionDoc(doc) {
  try {
    return !!foundry?.utils?.getProperty(doc, 'flags.core.MeasuredTemplate');
  } catch (_) {
    return false;
  }
}
...
/**
 * Region placeables that back template visuals (v14). Falls back to deprecated
 * {@link canvas#templates#placeables} only when {@link canvas#regions} is absent (v12–v13).
 * @returns {any[]}
 * @private
 */
function _msMeasuredTemplateOverlayPlaceables() {
  try {
    if (canvas?.regions && Array.isArray(canvas.regions.placeables)) {
      return canvas.regions.placeables.filter((p) => _msIsMeasuredTemplateRegionDoc(p?.document));
    }
  } catch (_) {}
  try {
    if (canvas?.templates && Array.isArray(canvas.templates.placeables)) {
      return canvas.templates.placeables;
    }
  } catch (_) {}
  return [];
}
...
/**
 * True if the scene embeds at least one template-backed region (v14), or legacy templates (v13).
 * @returns {boolean}
 * @private
 */
function _msSceneHasMeasuredTemplateRegions() {
  try {
    const coll = canvas?.scene?.regions;
    if (coll && typeof coll[Symbol.iterator] === 'function') {
      for (const doc of coll) {
        if (_msIsMeasuredTemplateRegionDoc(doc)) return true;
      }
      return false;
    }
  } catch (_) {}
  try {
    if (!canvas?.regions && canvas?.scene?.templates) {
      return (Number(canvas.scene.templates.size) || 0) > 0;
    }
  } catch (_) {}
  return false;
}
...
function _rehydrateNativeTemplateOverlays(reason = 'unspecified') {
  ...
  const templates = _msMeasuredTemplateOverlayPlaceables();
  ...
  for (const tpl of templates) {
    ...
    try {
      if (tpl.renderFlags?.set) {
        ...
        if (typeof tpl.highlightGrid === 'function') {
          tpl.renderFlags.set({ refreshGrid: true, refreshText: true });
        } else {
          tpl.renderFlags.set({ refreshGeometry: true, refreshMeasurements: true });
        }
      }
    } catch (_) {}
...
function _shouldThreeOwnDrawingsInGameplay() {
  ...
  if (!sceneSettings.isMapShineRenderingActive(canvas?.scene)) return false;
  return true;
}
...
function _updateFoundrySelectRectSuppression(forceValue = null) {
  ...
  if (!sceneSettings.isMapShineRenderingActive(canvas?.scene)) {
...
/**
 * updateScene can occasionally deliver dimension/grid keys without a user-facing Scene Config edit
 * (core batching, timing, or replication). Full createThreeCanvas from onUpdateScene is expensive and
 * causes the "random loading screen + degraded performance" failure mode. Debounce + per-scene throttle
 * reduce bursts; set window.MapShine.__forceNextUpdateSceneReinit = true to bypass throttle once.
 */
const UPDATE_SCENE_REINIT_DEBOUNCE_MS = 800;
const UPDATE_SCENE_REINIT_MIN_INTERVAL_MS = 25000;

/** @type {ReturnType<typeof setTimeout>|null} */
let _updateSceneReinitDebounceTimer = null;

/** @type {Map<string, number>} */
const _updateSceneReinitLastBySceneId = new Map();

/** @type {string|null} */
let _updateSceneReinitPendingSceneId = null;

const MSA_V14_LIFECYCLE_TRACE = '[MSA-V14-LIFECYCLE]';
...
/**
 * Derive lightweight scene effect hints from the current asset bundle.
 * These hints are advisory only and are used to skip unnecessary shader warmup
 * for mask-driven effects that cannot run on this scene.
 *
 * @param {object|null} bundle
 * @returns {{maskIds: string[]}}
 */
function deriveSceneEffectHints(bundle) {
  const maskIds = new Set();
  try {
    const masks = bundle?.masks;
    if (Array.isArray(masks)) {
      for (const m of masks) {
        const id = String(m?.id ?? m?.type ?? '').trim().toLowerCase();
        if (id) maskIds.add(id);
      }
    }
  } catch (_) {}
  return { maskIds: [...maskIds] };
}
...
function refreshLevelsInteropDiagnostics(options = {}) {
  ...
  const state = {
    ...
    runtimeAuthority: {
      visibility: 'foundry-v14-levels',
      fog: 'foundry-v14-levels',
      renderLayering: 'foundry-v14-levels',
    },
  };
  ...
  void emitWarning;
  void lastLevelsInteropWarningKey;
...
export function initialize() {
  ...
  try {
      ...
      safeCall(() => {
        ...
        safeCall(() => {
          const traceEnabled = (
            ...
            globalThis?.MapShine?.__debugPixiAssetTrace === true
          );
    ...
    // Foundry draws a PIXI.Graphics outline between sceneRect and padded canvas.rect on the
    // interface group (see InterfaceCanvasGroup.#drawOutline). Hide it for Map Shine scenes.
    if (!globalThis.__msaSuppressInterfaceOutlineHook) {
      globalThis.__msaSuppressInterfaceOutlineHook = true;
      Hooks.on('drawInterfaceCanvasGroup', () => {
        safeCall(() => _suppressFoundryInterfaceSceneOutline(), 'hook.drawInterfaceCanvasGroup.outline', Severity.COSMETIC);
      });
    }
    ...
    Hooks.on('mapShineLevelContextChanged', (payload) => {
      ...
      const levelMaskRebuildPromise = safeCallAsync(async () => {
        ...
        if (hasV14NativeLevels(canvas?.scene) && sc && bus && fd) {
          ...
          if (viewedBgSrc && newBasePath && needsBgSwap) {
            if (pathChanged) {
              ...
              log.info(`V14 viewed level changed (${levelId}); refreshing bus bg for shared art: ${newBasePath}`);
            }
            ...
            if (pathChanged) {
              try {
                const result = await sc._loadMasksOnlyForBasePath(newBasePath);
                if (result?.masks?.length) {
                  sc._lastMaskBasePath = newBasePath;
                  log.info(`Reloaded ${result.masks.length} mask(s) for ${newBasePath}`);
                }
              } catch (err) {
                log.warn('Failed to reload masks for new level background:', err);
              }
            }
        ...
        if (
          ...
          // Warm every visible floor for stacked Camera Grade _Outdoors so upper
          // bands (e.g. floor 3) contribute to post-merge CC, not just the active band.
          try {
            const visibleFloors = ms?.floorStack?.getVisibleFloors?.() ?? [];
    ...
    // Snapshot store hooks remain active for diagnostics.
...
function computeNativeSameSceneDrawIntent(drawArgs) {
  try {
    ...
    let viewedId = null;
    try {
      const v = game?.scenes?.current?.id ?? game?.scenes?.viewed?.id;
      viewedId = v != null ? String(v) : null;
    } catch (_) {
      viewedId = null;
    }
    ...
    try {
      ...
      lastResolved = lr != null ? String(lr) : null;
    } catch (_) {
      lastResolved = null;
    }

    // Primary: live canvas document matches draw target.
    // V14 same-scene redraw can invoke Canvas#draw while `canvas.scene` is briefly
    // cleared — then `cur` is null and we must not treat that as a cross-scene
    // transition (full loading overlay + redundant createThreeCanvas).
    const sameByCanvas = !!(cur && String(cur.id) === targetId);
    const sameByStaleCanvas = !!(
      !cur
      && lastResolved
      && targetId === lastResolved
      && viewedId
      && targetId === viewedId
    );
    const sameSceneRedraw = sameByCanvas || sameByStaleCanvas;

    if (!sameSceneRedraw) {
      return { sameSceneRedraw: false, sameSceneLevelSwitch: false };
    }

    const levelsScene = cur ?? drawTarget;
    if (!hasV14NativeLevels(levelsScene)) {
...
function installCanvasDrawWrapper() {
  ...
  safeCall(() => {
    const _drawWrapper = async function(wrapped, ...args) {
      ...
      let _pollCount = 0;
      const _hangTimerIds = [];
      const isDrawPollerVerbose = (
        debugLoadingProfiler?.debugMode === true ||
        window.MapShine?.__debugCanvasStatePoller === true
      );
      try {
        if (isDrawPollerVerbose) {
          // Periodic state poller ->-> fires every 2s and dumps full canvas state
          _pollerId = setInterval(() => {
            _pollCount++;
            try {
              _collectCanvasStateDiagnostic();
            } catch (e) {
            }
          }, 2000);
      ...
      try {
        ...
        const isMsaEnabledForHangCheck = sceneForHangCheck ? sceneSettings.isMapShineRenderingActive(sceneForHangCheck) : false;
        ...
        if (!_canvasReadyFiredThisDraw) {
          try {
            ...
            const isMsaEnabled = scene ? sceneSettings.isMapShineRenderingActive(scene) : false;
...
function installAmbientSoundAudibilityPatch() {
  ...
  const installed = safeCall(() => {
    ...
    const wrappedGetter = function mapShineAmbientSoundIsAudible() {
      ...
      if (!sceneSettings.isMapShineRenderingActive(canvas?.scene)) return baseAudible;
 ...
 * @private
 */
function _resolveUpdateSceneUserId(userId, options) {
  if (typeof userId === 'string' && userId.length) return userId;
  const o = options;
  if (o && typeof o.userId === 'string' && o.userId.length) return o.userId;
  if (o && typeof o.user === 'string' && o.user.length) return o.user;
  return null;
}

/**
 * True when the scene flag diff only touches keys we persist from live UI (time/weather),
 * not settings or Levels-adjacent data.
...
/**
 * True when `updateScene` delta cannot affect scene dimensions, grid geometry, or background asset —
 * only document metadata / flags. Prevents spurious `createThreeCanvas` (full loading screen) from
 * weather slider flag writes or similar.
 * @param {object} changes
 * @returns {boolean}
 */
...
async function onUpdateScene(scene, changes, _options, _userId) {
  ...
  if (!sceneSettings.isMapShineRenderingActive(scene)) return;

  // Foundry passes userId as the 4th argument (see hookEvents.updateDocument). Some
  // code paths omit it; StateApplier also calls setFlag without going through the panel.
  // Re-applying controlState / weather-snapshot on the echoed update re-enters
  // applyTimeOfDay + _loadWeatherSnapshotFromScene (which calls applyTimeOfDay again),
  // stacking canvas.scene.update(darkness) and breaking V2 (grey canvas).
  const originatingUserId = _resolveUpdateSceneUserId(_userId, _options);
  ...
  safeCall(() => {
    ...
    if (skipAuthoritativeMsaResync) return;
  ...
  // IMPORTANT: live effect toggles (for example Lighting enabled/disabled in the
  // panel) persist through `flags.map-shine-advanced.settings`. Those updates are
  // intended to be hot-applied and must not force a full scene re-init/loading
  // overlay cycle. Rebuilding here causes the exact "entire map reloads when I
  // tick a checkbox" bug.
  if (hasMsaSettingsDiff) {
    log.debug('updateScene: settings-flag change detected -> skipping full scene reinit');
    return;
  }

  if (echoOnly) {
    log.debug('updateScene: MSA control/weather flag echo -> skipping Map Shine canvas rebuild');
    return;
  }
...
/**
 * Debounced + throttled full reinit from {@link onUpdateScene} shouldReinit path.
 * @param {*} scene - Foundry Scene document
 * @param {object} changes - updateScene changes that triggered the reinit
 */
function _scheduleUpdateSceneDrivenReinit(scene, changes = undefined) {
  const sceneId = scene?.id != null ? String(scene.id) : '';
  if (!sceneId) return;
  const changeKeys = _msaFlattenChangeKeys(changes);

  try {
    if (_updateSceneReinitDebounceTimer) {
      clearTimeout(_updateSceneReinitDebounceTimer);
      _updateSceneReinitDebounceTimer = null;
    }
  } catch (_) {}

  _updateSceneReinitPendingSceneId = sceneId;
  _updateSceneReinitDebounceTimer = setTimeout(async () => {
    _updateSceneReinitDebounceTimer = null;
    const pendingId = _updateSceneReinitPendingSceneId;
    _updateSceneReinitPendingSceneId = null;
    if (!pendingId) return;
    ...
    try {
      ...
      let force = false;
      try {
        force = !!window.MapShine?.__forceNextUpdateSceneReinit;
        if (force) window.MapShine.__forceNextUpdateSceneReinit = false;
      } catch (_) {}

      if (!force) {
        const last = _updateSceneReinitLastBySceneId.get(pendingId) || 0;
        const now = performance.now();
        if (last > 0 && (now - last) < UPDATE_SCENE_REINIT_MIN_INTERVAL_MS) {
          log.warn(
            `updateScene: skipping Map Shine full reinit (throttled; ${Math.round((now - last) / 1000)}s since last for this scene). ` +
            'Set window.MapShine.__forceNextUpdateSceneReinit = true then change scene config if you need an immediate rebuild.'
          );
          return;
        }
      }

      const sceneDoc = game?.scenes?.get?.(pendingId) ?? scene;
      _updateSceneReinitLastBySceneId.set(pendingId, performance.now());
      log.info('Scene configuration changed, reinitializing Map Shine canvas');

      if (!_createThreeCanvasRunning && _hasLiveMapShineRuntime()) {
        log.warn(`${MSA_V14_LIFECYCLE_TRACE} updateScene direct full reinit: destroying existing runtime before createThreeCanvas`, {
          sceneId: pendingId,
          changeKeys,
        });
        destroyThreeCanvas();
      ...
      await createThreeCanvas(sceneDoc);
    } catch (err) {
      try {
        log.warn('updateScene: createThreeCanvas from guarded updateScene path failed:', err?.message ?? err);
      } catch (_) {}
    }
  }, UPDATE_SCENE_REINIT_DEBOUNCE_MS);
}
 ...
 */
function _installPixiSuppressionHooksForV2() {
  safeCall(() => {
...
/**
 * (Re)attach FrameCoordinator post-PIXI callback; updates window.MapShine.frameCoordinator.
 */
function _attachFrameCoordinatorPostPixiForV2() {
  const mapShine = window.MapShine;
  if (!mapShine) return;
 ...
 * @param {*} scene
 */
async function resyncMapShineAfterNativeLevelRedraw(scene) {
  try {
    const ec = effectComposer;
    const sc = sceneComposer;
    if (!ec || !sc || !threeCanvas || !renderLoop) {
      log.warn('[level-resync] Missing core refs — falling back to full createThreeCanvas');
      await createThreeCanvas(scene, { skipLoadingOverlay: true });
      return;
    }

    log.info('[level-resync] Native same-scene level redraw — resyncing Map Shine (no full teardown)');

    const cf = window.MapShine?.cameraFollower;
    if (cf && typeof cf._syncToViewedLevel === 'function') {
      cf._syncToViewedLevel({ emit: true, reason: 'native-level-redraw' });
    }

    const tm = tokenManager;
    if (tm?.tokenSprites?.size) {
      const ids = [...tm.tokenSprites.keys()];
      for (const id of ids) tm.removeTokenSprite(id);
    }
    if (tm && typeof tm.syncAllTokensAsync === 'function') {
      await tm.syncAllTokensAsync({ yieldEvery: 40 });
    }
    ...
    renderLoop?.requestContinuousRender?.(400);
  } catch (e) {
    log.error('[level-resync] failed; falling back to full scene init', e);
    if (!_createThreeCanvasRunning && _hasLiveMapShineRuntime()) {
      log.warn(`${MSA_V14_LIFECYCLE_TRACE} same-scene resync failed: destroying existing runtime before fallback create`, {
        sceneId: scene?.id ?? null,
      });
      destroyThreeCanvas();
    }
...
function _hasLiveMapShineRuntime() {
...
async function onCanvasReady(canvas) {
  ...
  try {
    ...
    if (canvas?.scene?.id != null) {
      window.MapShine.__msaLastResolvedSceneId = String(canvas.scene.id);
    }
  } catch (_) {}

  if (sceneSettings.isNativeFoundryRenderingEnabled()) {
    log.info('Native Foundry rendering mode — Map Shine canvas bypassed for this client');
    if (!window.MapShine) window.MapShine = {};
    window.MapShine.nativeRenderingBypass = true;
    window.MapShine.bootstrapComplete = true;
    safeCall(() => _restoreFoundryNativeSelectRectForVanillaScene(), 'nativeBypass.restoreSelectRect', Severity.COSMETIC);
    safeCall(() => loadingOverlay.fadeIn(500).catch(() => {}), 'overlay.nativeBypass', Severity.COSMETIC);
    return;
  }

  // LoadCoordinator: begin tracking this scene load.
  // If createThreeCanvas is already active, defer beginSceneLoad to the replayed
  // createThreeCanvas entry point so we don't reset coordinator state mid-run.
  const sceneForCoordinator = canvas?.scene;
  const sameSceneRuntimePreservedAtReady = nativeSameSceneRedrawAtReady && _hasLiveMapShineRuntime();
  if (sceneForCoordinator && !_createThreeCanvasRunning && !sameSceneRuntimePreservedAtReady) {
    loadCoordinator.beginSceneLoad(sceneForCoordinator.id, sceneForCoordinator.name);
  } else if (sceneForCoordinator && _createThreeCanvasRunning) {
    log.debug('LoadCoordinator: deferring beginSceneLoad during active createThreeCanvas');
  } else if (sceneForCoordinator && sameSceneRuntimePreservedAtReady) {
    log.info(`${MSA_V14_LIFECYCLE_TRACE} canvasReady same-scene redraw: preserving existing load coordinator/runtime`, {
      sceneId: sceneForCoordinator.id,
  ...
  if (!window.MapShine || !window.MapShine.initialized) {
    ...
    if (!window.MapShine?.initialized) {
      if (window.MapShine?.nativeRenderingBypass || sceneSettings.isNativeFoundryRenderingEnabled()) {
        log.info('Native Foundry rendering mode — skipping Map Shine bootstrap wait');
        safeCall(() => _restoreFoundryNativeSelectRectForVanillaScene(), 'nativeBypass.restoreSelectRect', Severity.COSMETIC);
        safeCall(() => loadingOverlay.fadeIn(500).catch(() => {}), 'overlay.nativeBypass', Severity.COSMETIC);
        return;
      }
  ...
  if (nativeSameSceneRedrawAtReady && _hasLiveMapShineRuntime()) {
    log.info(`${MSA_V14_LIFECYCLE_TRACE} canvasReady same-scene redraw: resyncing without loading overlay/full create`, {
      sceneId: scene.id,
      levelSwitch: nativeSameSceneLevelSwitchAtReady,
    });
    await resyncMapShineAfterNativeLevelRedraw(scene);
    safeCall(() => loadingOverlay.fadeIn(200).catch(() => {}), 'overlay.sameSceneRedrawResync', Severity.COSMETIC);
    return;
  }

  if (nativeSameSceneRedrawAtReady && !_hasLiveMapShineRuntime()) {
    log.warn(`${MSA_V14_LIFECYCLE_TRACE} canvasReady same-scene redraw had no live Map Shine runtime; allowing full create`, {
      sceneId: scene.id,
      levelSwitch: nativeSameSceneLevelSwitchAtReady,
    });
  }
  ...
  if (!skipNativeLevelSwitchOverlay) {
    ...
    safeCall(() => {
      ...
      loadingOverlay.configureStages(LOADING_PROGRESS_STAGES);
      loadingOverlay.startStages();
      loadingOverlay.setStage('scene.settings', 0.0, undefined, { immediate: true });
  ...
  const isHeartbeatVerbose = (
    debugLoadingProfiler?.debugMode === true ||
    window.MapShine?.__debugLoadingHeartbeat === true
  );
  ...
  const _watchdogId = setInterval(() => {
    ...
    if (step !== _watchdogLastStep) {
      ...
      if (isHeartbeatVerbose) {
        log.debug(`[loading] createThreeCanvas heartbeat [diag=step-tracker-v1] (step=${step}, progressing)`);
      }
      return;
    }
    ...
    if (stuckMs >= WATCHDOG_STUCK_THRESHOLD_MS) {
      ...
      if (isHeartbeatVerbose) {
        const level = stuckMs > 30000 ? 'warn' : 'debug';
...
function onCanvasTearDown(canvas) {
  ...
  if (lightNativeSameScenePath || preserveInFlightSameSceneLoad) {
    log.info(`${MSA_V14_LIFECYCLE_TRACE} canvasTearDown same-scene/preserved path`, {
      tearDownSceneId,
...
async function createThreeCanvas(scene, createOptions = {}) {
  ...
  if (_lastCreateThreeCanvasFailure && _lastCreateThreeCanvasFailure.sceneId === sceneId) {
    const ageMs = performance.now() - _lastCreateThreeCanvasFailure.atMs;
    if (ageMs >= 0 && ageMs < CREATE_THREE_CANVAS_FAILURE_COOLDOWN_MS) {
      log.warn(
        `[loading] Suppressing immediate createThreeCanvas retry for scene "${scene?.name ?? sceneId}" ` +
        `(${Math.round(ageMs)}ms since last failure: ${_lastCreateThreeCanvasFailure.reason})`
      );
      return;
    }
  }
  ...
  safeCall(() => {
    ...
    if (!sid) return;
    if (loadCoordinator.sceneId !== sid || loadCoordinator.state !== CoordinatorState.AWAITING_CANVAS_READY) {
      loadCoordinator.beginSceneLoad(sid, scene?.name);
    }
  }, 'loadCoordinator.normalizeAtCreateStart', Severity.COSMETIC);

  const _transitionOrBail = (nextState, reason, label) => {
    const ok = loadCoordinator.transition(nextState, reason);
    if (!ok) {
      // Soft-fail transition handling: keep loading so we don't strand users
      // in a half-initialized overlay state due to coordinator race noise.
  ...
  _transitionOrBail(CoordinatorState.PREPARING_CONTEXT, 'createThreeCanvas entered', 'preparing_context');
  console.log(' ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->');
  console.log(' Scene load started:', scene?.name ?? 'unknown');
  console.log(' ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->');

  // Expose LoadCoordinator on window.MapShine for diagnostics
  if (window.MapShine) window.MapShine.loadCoordinator = loadCoordinator;
  ...
  let _overlaySceneReadyDismissTimer = null;
  const isProgressHeartbeatVerbose = (
    debugLoadingProfiler?.debugMode === true ||
    window.MapShine?.__debugLoadingHeartbeat === true
  );
  try {
    if (isProgressHeartbeatVerbose) {
      _progressHeartbeatId = setInterval(() => {
        try {
          ...
          log.debug(`[loading] createThreeCanvas heartbeat [diag=step-tracker-v1] (step=${step}, stuck=${stuckText})`);
        } catch (_) {
        }
      }, 5000);
    }
  ...
  const isStepTraceEnabled = isDebugLoad || window.MapShine?.__debugLoadingSteps === true;
  const stepLog = (...args) => {
    if (!isStepTraceEnabled) return;
    console.log(...args);
  };
  if (isDebugLoad) {
  ...
  const _bailIfSessionStale = (label = 'createThreeCanvas') => {
    if (!session.isStale()) return false;
    log.info(`[loading] ${label}: stale/aborted load session -> bailing out`);
    // Ensure stale exits cannot strand coordinator/overlay in a loading state.
    safeCall(() => {
      _setCreateThreeCanvasProgress(`${label}:stale`);
      loadCoordinator.tearDown(`stale-session:${label}`);
    }, 'loadCoordinator.staleBailTearDown', Severity.COSMETIC);
    safeCall(() => {
      try { loadingOverlay.fadeIn?.(300, 100)?.catch?.(() => {}); } catch (_) {}
      try { loadingOverlay.hide?.(); } catch (_) {}
    }, 'overlay.staleBailDismiss', Severity.COSMETIC);
    return true;
  };
  ...
  // try/catch block that normally handles overlay dismissal on errors.
    ...
    loadCoordinator.enterFailed(reason);
    safeCall(() => {
      loadingOverlay.surfaceLoadFailure(reason, loadCoordinator.getDiagnostics());
  ...
  if (!mapShine || !mapShine.renderer) {
    ...
    if (_bailIfSessionStale('lazyBootstrap.post')) return;
  ...
  try {
    ...
    if (_bailIfSessionStale('waitForCanvasReady.post')) return;
    ...
    safeCall(() => loadingOverlay.setStage('scene.settings', 0.0, undefined, { immediate: true, keepAuto: true }), 'overlay.sceneSettings.start', Severity.COSMETIC);
    await safeCallAsync(async () => {
    ...
    safeCall(() => loadingOverlay.setStage('scene.settings', 1.0, undefined, { immediate: true, keepAuto: true }), 'overlay.sceneSettings.done', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.canvas', 0.0, undefined, { immediate: true, keepAuto: true }), 'overlay.canvasCreate.start', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.canvas', 1.0, undefined, { immediate: true, keepAuto: true }), 'overlay.canvasCreate.done', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.renderer', 0.0, undefined, { immediate: true, keepAuto: true }), 'overlay.rendererAttach.start', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.renderer', 1.0, undefined, { immediate: true, keepAuto: true }), 'overlay.rendererAttach.done', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('assets.load', 0.0, undefined, { immediate: true, keepAuto: true }), 'overlay.assetsLoad.start', Severity.COSMETIC);
    ...
    const sceneEffectHints = deriveSceneEffectHints(bundle);
    safeCall(() => {
      if (window.MapShine) {
        window.MapShine.__sceneEffectHints = sceneEffectHints;
      }
    }, 'sceneEffectHints.expose', Severity.COSMETIC);
    ...
    if (_bailIfSessionStale('sceneComposer.initialize.post')) {
    ...
    safeCall(() => {
      loadingOverlay.setStage('effects.bootstrap', 0.0, undefined, { immediate: true, keepAuto: true });
      loadingOverlay.startAutoProgress(0.55, 0.015);
    }, 'overlay.effectsCore', Severity.COSMETIC);
    log.info('[loading] entered effects.bootstrap stage');
    ...
    stepLog(' -> Step: weatherController.initialize');
    ...
    safeCall(() => loadingOverlay.setStage('effects.bootstrap', 1.0, undefined, { keepAuto: true }), 'overlay.weather', Severity.COSMETIC);
    ...
    _sectionEnd('effectInit');
      ...
      safeCall(() => {
        loadingOverlay.setStage('effects.core', 0.10, undefined, { keepAuto: true });
        ...
        fc = effectComposer._getFloorCompositorV2({
          onProgress: (index, total) => {
            safeCall(() => {
              ...
              loadingOverlay.setStage('effects.core', p, formatEffectsInitMessage(index, total), { keepAuto: true });
            }, 'overlay.compositor.progress', Severity.COSMETIC);
          },
          effectHints: sceneEffectHints,
        });
      ...
      // Yield so the progress bar can repaint after the heavy compositor init block.
      safeCall(() => {
        loadingOverlay.setStage('effects.core', 1.0, undefined, { keepAuto: true });
    ...
    safeCall(() => {
      loadingOverlay.setStage('scene.tokens', 0.05, undefined, { keepAuto: true });
    ...
    // Step 4b.0: Initialize FloorStack
    if (!_transitionOrBail(CoordinatorState.POPULATING_FLOORS, 'FloorStack + managers', 'populating_floors')) return;
    ...
    safeCall(() => loadingOverlay.setStage('scene.layers', 0.30, undefined, { keepAuto: true }), 'overlay.tiles', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.movement', 0.50, undefined, { keepAuto: true }), 'overlay.walls', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.interaction', 0.40, undefined, { keepAuto: true }), 'overlay.remaining', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.interaction', 0.90, undefined, { keepAuto: true }), 'overlay.interaction', Severity.COSMETIC);
    ...
    safeCall(() => {
      ...
      if (fc && typeof fc._applyCurrentFloorVisibility === 'function') {
        fc._applyCurrentFloorVisibility({
          context: window.MapShine?.activeLevelContext ?? null,
        });
      }
      // V14 cold load: FloorRenderBus may have populated before FloorStack existed or before
      // Foundry finalized `canvas.level`, reusing a stale composer albedo. Navigating floors
      // fixes it because swapBackgroundImage runs — mirror that once after level sync.
      const sc = window.MapShine?.sceneComposer;
      const bus = fc?._renderBus;
      const fd = sc?.foundrySceneData;
      const v14Scene = canvas?.scene;
      if (
        hasV14NativeLevels(v14Scene)
        && (v14Scene?.levels?.size ?? 0) > 1
        && sc
        && bus
        && typeof bus.swapBackgroundImage === 'function'
        && fd
      ) {
    ...
    // Legacy Levels perspective bridge removed. V14 native levels are authoritative.
    ...
    safeCall(() => loadingOverlay.setStage('scene.camera', 0.93, undefined, { keepAuto: true }), 'overlay.camera', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.sync', 0.5, undefined, { keepAuto: true }), 'overlay.sceneSync', Severity.COSMETIC);
    ...
    safeCall(() => loadingOverlay.setStage('scene.sync', 1.0, undefined, { immediate: true }), 'overlay.sceneSync.done', Severity.COSMETIC);
    _sectionEnd('sceneSync');
    _sectionStart('finalization');
    // Transition to UI init stage — the next blocking phase is initializeUI.
    safeCall(() => {
      loadingOverlay.setStage('ui.bootstrap', 0.0, undefined, { immediate: true });
    ...
    // Step 8: Resize handling before first rAF — avoids frames with a 0×0 or stale
    // drawing buffer when the initial layout was resolved via #board/window fallback.
    if (resizeHandler) {
    ...
    safeCall(() => {
      ...
      if (rw >= 1 && rh >= 1) resizeHandler.resize(rw, rh);
    }, 'resizeHandler.syncAfterSetup', Severity.COSMETIC);

    // Step 8.5: Create render loop (start is deferred — see post-`prewarmForLoading` block
    // in finalization). If RAF begins before async bus/effect populate finishes,
    // FloorCompositor paints `[POPULATE RENDER-SLIM]` (bus + blit only, no lighting /
    // water / level-composite), which can look like a "good first frame" then snap
    // once populate completes and the real post chain runs every frame thereafter.
    stepLog(' -> Step: renderLoop.create (start deferred)');
    renderLoop = new RenderLoop(renderer, threeScene, camera, effectComposer);
    stepLog(' -> Step: renderLoop.create DONE (start deferred)');
    ...
    exposeGlobals(mapShine, {
      ...
      sceneDebug, physicsRopeManager, assetLoader,
      externalEffects,
    ...
    safeCall(() => {
      ...
      if (!healthEvaluator) {
        healthEvaluator = new HealthEvaluatorService({
          ...
          gpuSceneMaskCompositor: window.MapShine?.gpuSceneMaskCompositor
            ?? window.MapShine?.sceneComposer?._sceneMaskCompositor
            ?? null,
        ...
        healthEvaluator.gpuSceneMaskCompositor = window.MapShine?.gpuSceneMaskCompositor
          ?? window.MapShine?.sceneComposer?._sceneMaskCompositor
          ?? null;
        try {
          healthEvaluator.refreshInstrumentation?.();
        } catch (_) {}
      }
    ...
    await safeCallAsync(async () => {
      ...
      safeCall(
        () => loadingOverlay.setStage('scene.prepare', 0.0, undefined, { keepAuto: false }),
    ...
    stepLog(' -> Step: initializeUI');
    await safeCallAsync(async () => {
      if (_bailIfSessionStale('initializeUI')) return;

      safeCall(() => loadingOverlay.setStage('ui.bootstrap', 0.20, undefined, { keepAuto: false }), 'overlay.uiInit.graphicsSettings', Severity.COSMETIC);
      safeCall(() => { if (window.MapShine) window.MapShine.stateApplier = stateApplier; }, 'exposeStateApplier', Severity.COSMETIC);
      ...
      safeCall(() => {
        ...
        safeCall(() => loadingOverlay.setStage('ui.panels', 0.45, undefined, { keepAuto: false }), 'overlay.uiInit.weatherStart', Severity.COSMETIC);
        ...
        safeCall(() => {
          ...
          const onWeatherUpdate = (_effectId, paramId, value) => {
            try {
              ...
              if (applyWeatherManualParam(weatherController, paramId, value, { syncMainTweakpane: false })) {
        ...
        safeCall(() => loadingOverlay.setStage('ui.panels', 0.60, undefined, { keepAuto: false }), 'overlay.uiInit.weatherDone', Severity.COSMETIC);
        await new Promise(r => setTimeout(r, 0)); // yield

        if (!controlPanel && isUserGM()) {
          controlPanel = new ControlPanelManager();
          await controlPanel.initialize();
          safeCall(() => loadingOverlay.setStage('ui.panels', 0.75, undefined, { keepAuto: false }), 'overlay.uiInit.controlPanelReady', Severity.COSMETIC);
          await new Promise(r => setTimeout(r, 0)); // yield
        ...
        safeCall(() => loadingOverlay.setStage('ui.panels', 1.0, undefined, { keepAuto: false }), 'overlay.uiInit.registerEffects', Severity.COSMETIC);

        // ->->->-> Register V2 effect controls in Tweakpane ->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->->
        safeCall(() => loadingOverlay.setStage('final.controls', 0.25, undefined, { keepAuto: true }), 'overlay.ui.p1', Severity.COSMETIC);
        ...
        const _STYLISTIC_FC_KEYS_OUTER = new Set([
          '_asciiEffect', '_dotScreenEffect', '_halftoneEffect',
          '_visionModeEffect', '_invertEffect', '_sepiaEffect',
        ]);
        ...
        const _propagateToV2 = (effectKey, paramId, value) => {
          try {
            ...
            // Stylistic effects must NOT queue enabled/masterEnabled — the per-frame
            // scene-flag gate is authoritative, and stale queued values cause flicker.
            if (window.MapShine) {
              const _skipQueue = _STYLISTIC_FC_KEYS_OUTER.has(effectKey)
                && (paramId === 'enabled' || paramId === 'masterEnabled');
              if (!_skipQueue) {
            ...
            const _STYLISTIC_FC_KEYS = _STYLISTIC_FC_KEYS_OUTER;
            try {
              ...
              if (pend && typeof pend === 'object') {
                for (const [k, v] of Object.entries(pend)) {
                  ...
                  // Never flush enabled/masterEnabled for stylistic effects — the
                  // per-frame scene-flag gate in EffectComposer.render() is
                  // authoritative. Flushing stale queued enabled:true here causes
                  // ASCII/dot/halftone to flicker on for random frames.
                  if (k === 'enabled' || k === 'masterEnabled') {
                    if (_STYLISTIC_FC_KEYS.has(effectKey)) continue;
        ...
        safeCall(() => {
          uiManager?.buildEnvironmentSection?.();
        }, 'v2.buildSunShadowsSectionUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('player-light', 'Player Light',
            PlayerLightEffectV2.getControlSchema(), _makeV2Callback('_playerLightEffect'), 'gameplay');
        }, 'v2.registerPlayerLightUI(V2)', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('fog', 'Fog of War',
            FogOfWarEffectV2.getControlSchema(), _makeV2Callback('_fogEffect'), 'gameplay');
        }, 'v2.registerFogUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('visionMode', 'Vision Mode',
            VisionModeEffectV2.getControlSchema(), _makeV2Callback('_visionModeEffect'), 'gameplay');
        }, 'v2.registerVisionModeUI', Severity.DEGRADED);
        ...
        }, 'v2.registerGridUI', Severity.DEGRADED);
        ...
        }, 'v2.buildTokensSectionUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('lighting', 'Light Physics',
            LightingEffectV2.getControlSchema(), _makeV2Callback('_lightingEffect'), 'lighting');
        }, 'v2.registerLightingUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('sky-color', 'Sky Environment',
            SkyColorEffectV2.getControlSchema(), _makeV2Callback('_skyColorEffect'), 'lighting');
        }, 'v2.registerSkyColorUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('windowLight', 'Window Light',
            WindowLightEffectV2.getControlSchema(), _makeV2Callback('_windowLightEffect'), 'lighting');
        }, 'v2.registerWindowLightUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager.registerEffect('bloom', 'Bloom Highlights',
            BloomEffectV2.getControlSchema(), _makeV2Callback('_bloomEffect'), 'lighting');
        ...
        }, 'v2.registerIridescenceUI', Severity.DEGRADED);
        ...
        safeCall(() => {
          uiManager.registerEffect('filter', 'Ink & Line AO',
            FilterEffectV2.getControlSchema(), _makeV2Callback('_filterEffect'), 'surface');
        }, 'v2.registerFilterUI', Severity.DEGRADED);
        safeCall(() => loadingOverlay.setStage('final.controls', 0.45, undefined, { keepAuto: true }), 'overlay.ui.p2', Severity.COSMETIC);
        ...
        }, 'v2.registerWaterSplashesUI', Severity.DEGRADED);
        ...
        }, 'v2.registerUnderwaterBubblesUI', Severity.DEGRADED);
        ...
        }, 'v2.registerSmellyFliesUI(V2)', Severity.DEGRADED);
        ...
        safeCall(() => {
          uiManager.registerEffect('lens', 'Lens',
            LensEffectV2.getControlSchema(), _makeV2Callback('_lensEffect'), 'post');
        ...
        safeCall(() => {
          uiManager?.buildPostSection?.();
        }, 'v2.buildPostSectionUI', Severity.DEGRADED);

        safeCall(() => {
          uiManager?.reorderEffectCategoryFolders?.();
        }, 'v2.reorderEffectCategoriesUI', Severity.COSMETIC);

        log.info('V2: registered effect controls (Gameplay, Lighting, Atmosphere, Surface, Particles, Post)');

        safeCall(() => loadingOverlay.setStage('ui.panels', 1.0, undefined, { immediate: true }), 'overlay.uiInit.done', Severity.COSMETIC);
    ...
    safeCall(() => {
      loadingOverlay.setStage('scene.prepare', 0.0, undefined, { immediate: true });
    }, 'overlay.scenePrepare.start', Severity.COSMETIC);
    stepLog(' -> Step: overlay.finalProgress DONE');
    ...
    safeCall(() => loadingOverlay.setStage('scene.prepare', 0.05, undefined, { keepAuto: false }), 'overlay.prepareTiles', Severity.COSMETIC);
    {
    ...
    safeCall(() => {
      loadingOverlay.setStage('scene.prepare', 0.10, undefined, { keepAuto: false });
    ...
    safeCall(() => loadingOverlay.setStage('scene.prepare', 0.45, undefined, { keepAuto: false }), 'overlay.prewarm.start', Severity.COSMETIC);
    await safeCallAsync(async () => {
      const fc = window.MapShine?.effectComposer?._floorCompositorV2;
    ...
    safeCall(() => {
      window.MapShine?.uiManager?.refreshAllEffectMaskStatuses?.();
    ...
    if (renderLoop && !renderLoop.running()) {
      stepLog(' -> Step: renderLoop.start (post-populate)');
      renderLoop.start();
      stepLog(' -> Step: renderLoop.start DONE (post-populate)');
      try {
    ...
    if (_bailIfSessionStale('fin.shaderCompile.pre')) return;
    if (!_transitionOrBail(CoordinatorState.COMPILING_WARMUP, 'shader compile + warmup', 'compiling_warmup')) return;
    _sectionStart('fin.shaderCompile');
    if (isDebugLoad) dlp.begin('fin.shaderCompile', 'finalize');
    safeCall(() => loadingOverlay.setStage('shaders.compile', 0.0, undefined, { immediate: true }), 'overlay.shaderCompile.start', Severity.COSMETIC);
    _setCreateThreeCanvasProgress('shaderCompile');
    stepLog(' -> Step: shaderCompile');
    await safeCallAsync(async () => {
      await effectComposer.ensureFloorCompositorV2Initialized();
      const fc = window.MapShine?.effectComposer?._floorCompositorV2;
      if (!fc || typeof fc.warmupAsync !== 'function') {
        log.warn('fin.shaderCompile: no FloorCompositor available, shaders will compile lazily');
        safeCall(() => loadingOverlay.setStage('shaders.compile', 1.0, undefined, { immediate: true }), 'overlay.shaderCompile.skip', Severity.COSMETIC);
        return;
      }
      ...
      const compiled = await fc.warmupAsync(10000, (progress, label) => {
        safeCall(() => {
          loadingOverlay.setStage('shaders.compile', progress, formatShaderCompileMessage(label), { keepAuto: false });
      ...
      safeCall(() => loadingOverlay.setStage('shaders.compile', 1.0, undefined, { immediate: true }), 'overlay.shaderCompile.done', Severity.COSMETIC);
      try {
    ...
    if (!_transitionOrBail(CoordinatorState.ACTIVATING, 'shader warmup complete', 'activating')) return;
    safeCall(() => {
      const busPopulated = !!(fc?._populateComplete ?? fc?._renderBus?._populateComplete);
      const invariants = loadCoordinator.checkPreRunningInvariants({
        foundrySceneId: scene?.id,
        busPopulated,
        enablementConsensus: true,
      });
      if (!invariants.ok) {
        for (const f of invariants.failures) {
          log.warn(`Pre-running invariant failed: ${f}`);
        }
        loadCoordinator.enterDegraded(`invariant failures: ${invariants.failures.join('; ')}`);
      }
    }, 'loadCoordinator.preRunningInvariants', Severity.DEGRADED);
    ...
    // LoadCoordinator: enter RUNNING (or stay DEGRADED if invariants failed)
    if (_bailIfSessionStale('fin.running.pre')) return;
    if (loadCoordinator.state === CoordinatorState.ACTIVATING) {
      if (!_transitionOrBail(CoordinatorState.RUNNING, 'scene load complete', 'running')) return;
    } else if (loadCoordinator.isDegraded) {
      log.warn('LoadCoordinator: remaining in DEGRADED after scene load (invariant failures logged above)');
    }
    ...
    try {
      if (_overlaySceneReadyDismissTimer) {
        clearTimeout(_overlaySceneReadyDismissTimer);
        _overlaySceneReadyDismissTimer = null;
      }
    } catch (_) {}
    try {
      if (window.MapShine) window.MapShine.__msaSceneLoading = false;
    ...
    const currentSceneId = String(scene?.id ?? '');
...
/**
 * Hide Foundry's scene-vs-padding frame (PIXI.Graphics from InterfaceCanvasGroup.#drawOutline).
 * First child of canvas.interface after each interface draw is that outline.
 * @private
 */
function _suppressFoundryInterfaceSceneOutline() {
  if (!canvas?.ready || isMapMakerMode || !sceneSettings.isMapShineRenderingActive(canvas?.scene)) return;
...
function _enforcePersistentOverlayAdornments() {
  ...
  const touchRegionTemplate = (region) => {
    if (!region) return;
    forceVisible(region);
    try {
      if (Array.isArray(region.children)) {
        for (const ch of region.children) forceVisible(ch);
      }
    } catch (_) {}
  };
  ...
  try {
    if (canvas.regions) {
      canvas.regions.visible = true;
      canvas.regions.renderable = true;
      for (const r of _msMeasuredTemplateOverlayPlaceables()) touchRegionTemplate(r);
    }
    if (canvas.templates) {
      canvas.templates.visible = true;
      canvas.templates.renderable = true;
      const legacyTemplateObjs = (!canvas.regions && Array.isArray(canvas.templates.placeables))
        ? [...canvas.templates.placeables,
          ...(Array.isArray(canvas.templates.objects?.children) ? canvas.templates.objects.children : [])]
        : (Array.isArray(canvas.templates.preview?.children) ? canvas.templates.preview.children : []);
      for (const t of legacyTemplateObjs) touchTemplate(t);
    }
  } catch (_) {}
...
function updateInputMode() {
    ...
    setTimeout(() => {
      ...
      if (wallManager && wallManager.setVisibility) {
        // Gameplay: Three.js wall/door overlay. Walls layer: native PIXI only (no duplicates).
