...
export class EnvironmentPlaybackDriver {
  ...
  async holdEnd(end = null) {
    ...
    this._rampElapsedMs = this._rampDurationMs;
    if (this._end) {
  ...
  update(timeInfo) {
    ...
    this._rampElapsedMs += dtMs;

    const t = this._rampDurationMs > 0
      ? Math.max(0, Math.min(1, this._rampElapsedMs / this._rampDurationMs))
      : 1;

    void environmentControlApi.applyInterpolated(this._start, this._end, t, {
      persist: false,
      syncUi: false,
      applyDarkness: true,
      syncFoundryTime: false,
