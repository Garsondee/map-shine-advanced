...
function _ensurePlayerLightToolStateStyles() {
  try {
    ...
    style.textContent = `
      #ui-left [data-tool="map-shine-player-light"],
      #scene-controls [data-tool="map-shine-player-light"],
      #controls [data-tool="map-shine-player-light"] {
        transition: filter 120ms ease, color 120ms ease, text-shadow 120ms ease, opacity 120ms ease;
      }
      #ui-left [data-tool="map-shine-player-light"].msa-tool-active,
      #scene-controls [data-tool="map-shine-player-light"].msa-tool-active,
      #controls [data-tool="map-shine-player-light"].msa-tool-active {
        color: #ffae33 !important;
        filter: brightness(1.45) saturate(1.3);
        text-shadow: 0 0 8px rgba(255, 153, 0, 0.7);
      }
      #ui-left [data-tool="map-shine-player-light"].msa-tool-inactive,
      #scene-controls [data-tool="map-shine-player-light"].msa-tool-inactive,
      #controls [data-tool="map-shine-player-light"].msa-tool-inactive {
...
function _applyPlayerLightToolStateSync() {
  try {
    ...
    _setToolActiveStateOnSceneControls('map-shine-player-light', active);
    const els = _collectSceneControlToolElements('map-shine-player-light');
    els.forEach((el) => {
...
function _applyPlayerLightToolDisabledStates() {
  try {
    ...
    const allowed = canUserAccessPlayerLightTools();
    const els = _collectSceneControlToolElements('map-shine-player-light');
    els.forEach((el) => {
      if (!allowed) {
...
Hooks.once('init', async function() {
  ...
  Hooks.on('getSceneControlButtons', (controls) => {
    try {
      ...
      ensureTool(tokenControls, {
        name: 'map-shine-player-light',
        title: 'Player Light',
        icon: 'fas fa-lightbulb',
        button: true,
        order: 104,
        visible: canUserAccessPlayerLightTools(),
        toolclip: {
          src: '',
          heading: 'Player Light',
          items: [{ paragraph: 'Choose a light mode for the selected token' }]
        },
        onChange: () => {
          try {
            getPlayerLightPickerDialog().toggle();
          } catch (e) {
            console.error('Map Shine: failed to open Player Light picker', e);
            ui.notifications?.warn?.('Player Light picker is not available yet.');
          }
        }
      });
