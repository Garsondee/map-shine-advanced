...
/** Reliability clamp: prevent drips from spending most of life far below visible map weather band. */
const ROOF_DRIP_KILL_Z_MARGIN_MAX_SAFE = 220.0;
...
/** Robust simplified tree drip spacing (rectangle perimeter under canopy bounds). */
const ROOF_DRIP_TREE_EDGE_SPACING_SIMPLE = 42;
...
const ROOF_DRIP_SIZE_MIN_DEF = 1.2;
const ROOF_DRIP_SIZE_MAX_DEF = 2.4;
...
export class WeatherParticles {
  constructor(batchRenderer, scene) {
    ...
    this._ashCurlBaseScale = null;
    /** @type {number} */
    this._ashCurlBaseTimeScale = 1;
    /** @type {THREE.Vector3|null} */
    this._ashEmberCurlBaseScale = null;
    /** @type {number} */
    ...
    this._roofDripWarmupState = 'idle';
    this._roofDripWarmupReason = null;
    this._roofDripWarmupLastAtMs = null;
    this._roofDripWarmupFrameBudgetMs = 3.0;
    this._roofDripPendingStaleReason = null;
    this._roofDripDiagLastReason = 'init';
    this._roofDripDiagStats = {
      lastBuildMs: 0,
      lastBuildSource: null,
      lastTileEligible: 0,
      lastTreeEligible: 0,
      lastBasePointCount: 0,
      lastViewPointCount: 0
    };
    // Reliability guard: if drips should emit but points are missing, force async recovery rebuilds.
    this._roofDripNoPointTicks = 0;
    this._roofDripRecoveryCooldownSec = 0.0;
    this._roofDripRecoveryAttempts = 0;
  ...
  beginRoofDripWarmup(reason = 'manual') {
    if (this._roofDripWarmupState === 'warming') return false;
    this._roofDripWarmupState = 'warming';
    this._roofDripWarmupReason = reason;
    this._roofDripWarmupLastAtMs = (typeof performance !== 'undefined' && performance.now)
      ? performance.now()
      : Date.now();
    this._roofDripDiagLastReason = `warmup:${reason}`;
    return true;
  }
  ...
  stepRoofDripWarmup(frameBudgetMs = this._roofDripWarmupFrameBudgetMs) {
    ...
    if (this._roofDripRebuildJob) {
      ...
      let done = false;
      while (!done) {
        done = this._roofDripRebuildJobTick();
        const now = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
        if (!done && (now - started) >= budgetMs) break;
      }
      if (!this._roofDripRebuildJob && this.isRoofDripWarmupReady()) {
        this._roofDripWarmupState = 'ready';
        this._roofDripDiagLastReason = 'warmup:ready';
        return true;
      }
      return false;
    }
    ...
    if (!this._roofDripBasePoints || sig !== this._roofDripSourceSignature) {
      this._roofDripBeginPointsRebuild(sig, this._roofDripWarmupReason || 'warmup');
      return false;
    }

    this._roofDripWarmupState = 'ready';
    this._roofDripDiagLastReason = 'warmup:cache-hit';
    return true;
  }

  isRoofDripWarmupReady() {
    return !!(this._roofDripBasePoints && this._roofDripBasePoints.length >= 5 && !this._roofDripRebuildJob);
  }
  ...
  _roofDripCreateRebuildContext() {
    ...
    try {
      const fc = this._getFloorCompositorV2();
      const addOverlays = (ov) => {
        if (!ov || typeof ov.values !== 'function') return;
        for (const entry of ov.values()) {
          const mesh = entry?.mesh;
          const gp = mesh?.geometry?.parameters;
          const bw = Number(gp?.width ?? 0);
          const bh = Number(gp?.height ?? 0);
          if (!mesh || bw <= 8 || bh <= 8) continue;
          treeEntries.push(entry);
          treeCount++;
        }
      };
      addOverlays(fc?._treeEffect?._overlays);
      // Bush overlays are decorative ground foliage and should not emit canopy drips.
      // Keep drip sourcing limited to roof tiles + tree canopies.
    } catch (_) {}
    ...
    const useGpu = false;
    const nRoofSources = tileEligible.length;
    ...
    return {
      ...
      totalH,
      THREE,
      sceneW: Number(this._sceneBounds?.z ?? d.sceneWidth ?? d.width ?? 0) || 0,
      sceneH: Number(this._sceneBounds?.w ?? d.sceneHeight ?? d.height ?? 0) || 0,
      tileDocs,
      tileEligible,
      treeCount,
      globalB,
  ...
  _roofDripTryGpuRoofSegment(_c) { return null; }

  _roofDripAppendTreeDripSegmentsSlice(c, segments, from, toExclusive) {
    const treeEntries = c.treeEntries || [];
    if (!treeEntries.length) return;
    const THREE = c.THREE;
    if (!THREE) return;
    const spacing = ROOF_DRIP_TREE_EDGE_SPACING_SIMPLE;
    const groundZ = window.MapShine?.sceneComposer?.groundZ ?? 1000;
    // Use the same stable overhead-style spawn Z as roof drips for reliability.
    // Mesh-derived Z proved inconsistent across canopy assets/scenes.
    const zTreeSpawn = groundZ + Z_OVERHEAD_TILE_OFFSET - ROOF_DRIP_SPAWN_BELOW;
    const sceneW = Math.max(1, Number(c.sceneW) || 1);
    const sceneH = Math.max(1, Number(c.sceneH) || 1);
    // Guardrail: if mesh scale explodes, tree drip source should not become map-wide.
    const maxTreeW = sceneW * 0.22;
    const maxTreeH = sceneH * 0.22;
    ...
    for (let i = from; i < toExclusive; i++) {
      ...
      const bh = Number(gp?.height ?? 0);
      if (!mesh || bw <= 8 || bh <= 8) continue;
      const sourceTileId = String(mesh?.userData?.mapShineTreeTileId ?? '');
      // Exclude known background pseudo-tiles from canopy drip sourcing.
      if (sourceTileId.startsWith('__')) continue;
      ...
      let w = Math.max(8, bw * Math.max(1e-6, Math.abs(ws.x)));
      let h = Math.max(8, bh * Math.max(1e-6, Math.abs(ws.y)));
      let cx = wp.x;
      let cy = wp.y;
      let rot = Math.atan2(mesh.matrixWorld.elements[4], mesh.matrixWorld.elements[0]);

      // Prefer original tile dimensions/transform when available.
      // Tree/bush mesh scaling can be huge for visual effects and is not reliable for drip source bounds.
      ...
      // Final safety clamp against pathological map-scale canopy rectangles.
      w = Math.max(8, Math.min(w, maxTreeW));
      h = Math.max(8, Math.min(h, maxTreeH));
      // Reject entries that are still effectively map-scale after normalization.
      // These are usually background-level overlays, not local canopy footprints.
      const area = w * h;
      if (area > (sceneW * sceneH * 0.06) || w > sceneW * 0.35 || h > sceneH * 0.35) continue;
      const zTree = zTreeSpawn;
      const seg = [];
      this._pushRectEdgeUvPoints(seg, cx, cy, w, h, rot, spacing, zTree);
  ...
  _buildEmergencyRoofDripSceneFallbackPoints(targetK = 320) {
    const d = (typeof canvas !== 'undefined' && canvas) ? canvas.dimensions : null;
    if (!d) return null;
    const sceneW = Number(d.sceneWidth ?? d.width ?? 0);
    const sceneH = Number(d.sceneHeight ?? d.height ?? 0);
    if (!(sceneW > 1 && sceneH > 1)) return null;

    const groundZ = window.MapShine?.sceneComposer?.groundZ ?? 1000;
    const zSpawn = groundZ + 6.0;
    const k = Math.max(80, Math.min(1200, Math.floor(targetK)));
    const perSide = Math.max(8, Math.floor(k / 4));
    const out = [];
    ...
    for (let i = 0; i < perSide; i++) {
      ...
      // Top edge (v=0), normal down
      push(t, 0.0, 0.0, 1.0);
      // Bottom edge (v=1), normal up
      push(t, 1.0, 0.0, -1.0);
      // Left edge (u=0), normal right
      push(0.0, t, 1.0, 0.0);
      // Right edge (u=1), normal left
      push(1.0, t, -1.0, 0.0);
    }
    return out.length >= 5 ? new Float32Array(out) : null;
  }
  ...
  _roofDripRebuildJobTick() {
    ...
    if (job.phase === 'trees') {
      const nTrees = c.treeEntries?.length ?? 0;
      if (nTrees > 0) this._roofDripAppendTreeDripSegmentsSlice(c, job.segments, 0, nTrees);
      job.phase = 'finalize';
    }
    if (job.phase === 'finalize') {
      ...
      this._roofDripDiagStats.lastBuildMs = Math.max(0, finishedAt - (job.startedMs ?? finishedAt));
      this._roofDripDiagStats.lastBuildSource = `slice:${job.sourceLabel || 'runtime'}`;
      this._roofDripDiagStats.lastBasePointCount = buf ? Math.floor(buf.length / 5) : 0;
      if (this._roofDripWarmupState === 'warming') {
        this._roofDripWarmupState = (buf && buf.length >= 5) ? 'ready' : 'idle';
      }
      this._roofDripDiagLastReason = (buf && buf.length >= 5) ? 'rebuild:slice-ready' : 'rebuild:slice-empty';
      this._roofDripRebuildJob = null;
      this._roofDripViewSourceRef = null;
      this._roofDripLastViewQU0 = null;
      this._roofDripLastViewQU1 = null;
   ...
   */
  _roofDripBeginPointsRebuild(sig, sourceLabel = 'runtime') {
    const buildStartMs = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
    const c = this._roofDripCreateRebuildContext();
    if (!c) {
      this._roofDripBasePoints = null;
      this._roofDripSourceSignature = sig;
      this._roofDripRebuildJob = null;
      this._roofDripDiagLastReason = 'rebuild:no-context';
      this._roofDripInvalidateRoofDripViewCache();
      return;
    }
    this._roofDripDiagStats.lastTileEligible = c.tileEligible?.length ?? 0;
    this._roofDripDiagStats.lastTreeEligible = c.treeEntries?.length ?? 0;
    this._roofDripRebuildJob = null;
    const segments = [];
    const nt = c.tileEligible.length;
    const shouldSlice = nt > ROOF_DRIP_CPU_TILES_PER_CONTROL_TICK;
    if (shouldSlice) {
      this._roofDripSourceSignature = sig;
      ...
      this._roofDripRebuildJob = {
        ...
        segments: [],
        tileIdx: 0,
        startedMs: buildStartMs,
        sourceLabel,
        phase: 'tiles'
      };
      this._roofDripRebuildJobTick();
      this._roofDripDiagLastReason = 'rebuild:sliced-running';
      this._roofDripInvalidateRoofDripViewCache();
      return;
    }

    this._roofDripAppendCpuTileSlice(c, segments, 0, nt);
    if ((c.treeEntries?.length ?? 0) > 0) {
      this._roofDripAppendTreeDripSegmentsSlice(c, segments, 0, c.treeEntries.length);
    }
    const buf = this._roofDripMergeSegmentsToBuffer(c, segments);
    this._roofDripBasePoints = buf;
    this._roofDripSourceSignature = sig;
    this._roofDripDiagStats.lastBuildMs = ((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now()) - buildStartMs;
    this._roofDripDiagStats.lastBuildSource = `cpu:${sourceLabel}`;
    this._roofDripDiagStats.lastBasePointCount = buf ? Math.floor(buf.length / 5) : 0;
    this._roofDripDiagLastReason = buf ? 'rebuild:cpu-ready' : 'rebuild:cpu-empty';
    this._roofDripInvalidateRoofDripViewCache();
  }
  ...
  /**
   * Camera-focused fallback: pick the nearest drip points to current view center
   * when strict in-view filtering is too sparse.
   * @param {Float32Array} pts
   * @param {number} targetCount
   * @returns {Float32Array|null}
   */
  _getNearestRoofDripPointsToView(pts, targetCount = 180) {
    if (!pts || pts.length < 5) return null;
    const sceneX = this._viewSceneX;
    const sceneY = this._viewSceneY;
    const sceneW = this._viewSceneW;
    ...
    const n = Math.floor(pts.length / 5);
    if (n < 1) return null;
    const k = Math.max(24, Math.min(n, Math.floor(targetCount)));
  ...
  _createAshTexture() {
    ...
    // Per-atlas-tile palette: soot → brown-grey → mid → pale (pairs with wide vertex ColorRange).
    const ashPalettes = [
      { g0: [0.12, 0.1, 0.09], g1: [0.06, 0.05, 0.045], g2: [0.03, 0.025, 0.022], fill: [0.11, 0.09, 0.08], stroke: [0.04, 0.035, 0.03] },
      { g0: [0.35, 0.28, 0.22], g1: [0.2, 0.16, 0.13], g2: [0.08, 0.065, 0.055], fill: [0.38, 0.3, 0.24], stroke: [0.12, 0.09, 0.07] },
      { g0: [0.55, 0.52, 0.48], g1: [0.35, 0.33, 0.3], g2: [0.12, 0.11, 0.1], fill: [0.58, 0.55, 0.5], stroke: [0.28, 0.26, 0.23] },
      { g0: [0.78, 0.76, 0.72], g1: [0.5, 0.48, 0.45], g2: [0.2, 0.19, 0.18], fill: [0.82, 0.8, 0.76], stroke: [0.45, 0.43, 0.4] },
    ];
    ...
    const drawAshInCell = (cellX, cellY, variant) => {
      ...
      const cy = cellY * cellSize + cellSize / 2;
      const maxRadius = (cellSize / 2) - 4;
      const pal = ashPalettes[Math.min(3, Math.max(0, variant))] || ashPalettes[0];

      ctx.save();
      ctx.translate(cx, cy);

      const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, maxRadius);
      glow.addColorStop(0, rgba(pal.g0[0], pal.g0[1], pal.g0[2], 0.72));
      glow.addColorStop(0.42, rgba(pal.g1[0], pal.g1[1], pal.g1[2], 0.35));
      glow.addColorStop(1, rgba(pal.g2[0], pal.g2[1], pal.g2[2], 0));
      ctx.fillStyle = glow;
      ctx.beginPath();
      ctx.arc(0, 0, maxRadius, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = rgba(pal.fill[0], pal.fill[1], pal.fill[2], 0.88);
      ctx.strokeStyle = rgba(pal.stroke[0], pal.stroke[1], pal.stroke[2], 0.62);
      ctx.lineWidth = 1;
  ...
  _initSystems() {
    ...
    this.rainSystem.emitter.position.set(centerX, centerY, safeEmitterZ);
     ...
     // Simplified robust route: keep drips on the same stable main pass as rain.
     try {
       this.roofDripSystem.emitter.userData.msOverlayLayer = false;
     } catch (_) {}
    ...
    // --- ASH ---
    // Ash precipitation: slower, heavier than snow; vertex ColorRange carries soot→pale variation.
    ...
    const ashWind = new ApplyForce(new THREE.Vector3(1, 0, 0), new ConstantValue(400));

    // Curl: broad lazy cells; strength scaled each frame from ashTuning.curlStrength.
    const ashCurl = new CurlNoiseField(
      new THREE.Vector3(1180, 1180, 1480),
      new THREE.Vector3(72, 72, 18),
      0.042
    );

    // ColorOverLife multiplies particle.startColor (see Quarks ColorOverLife); brighten toward end of life.
    const ashColorOverLife = new ColorOverLife(new ColorRange(
      new Vector4(1.0, 0.99, 0.97, 1.0),
      new Vector4(2.28, 2.18, 1.98, 0.32)
    ));
    ...
    this.ashSystem = new ParticleSystem({
      ...
      startColor: new ColorRange(new Vector4(0.07, 0.055, 0.045, 0.78), new Vector4(0.82, 0.8, 0.76, 0.55)),
      worldSpace: true,
      maxParticles: 6000,
      emissionOverTime: new ConstantValue(0),
      shape: new RandomRectangleEmitter({ width: sceneW, height: sceneH }),
    ...
    const ashEmberCurl = new CurlNoiseField(
      new THREE.Vector3(880, 880, 1180),
      new THREE.Vector3(155, 155, 46),
      0.076
    );
    ...
    this._ashCurlBaseScale = ashCurl.scale.clone();
    this._ashCurlBaseTimeScale = ashCurl.timeScale;
    ...
    this._ashEmberCurlBaseScale = ashEmberCurl.scale.clone();
    this._ashEmberCurlBaseTimeScale = ashEmberCurl.timeScale;
  ...
  _migrateRoofEdgeDripShader(fs) {
    ...
    if (out.includes('float visibleBlock = rb * rv;')) {
      out = out.replace(
        'float visibleBlock = rb * rv;',
        'float hiddenBlock = rb * (1.0 - rv);'
      );
      out = out.replace(
        'visibleBlock = smoothstep(0.02, 0.28, visibleBlock);',
        'hiddenBlock = smoothstep(0.02, 0.28, hiddenBlock);'
      );
      out = out.replace(
        'msWaterFade *= (1.0 - visibleBlock);',
  ...
  _patchRoofMaskMaterial(material) {
    ...
    const fragmentMaskCode =
      ...
      '      float rb = clamp(roofBlockAlpha, 0.0, 1.0);\n' +
      '      float rv = clamp(roofAlpha, 0.0, 1.0);\n' +
      '      // Rain should be blocked only while roof/tree visuals are hidden/revealed.\n' +
      '      float hiddenBlock = rb * (1.0 - rv);\n' +
      '      hiddenBlock = smoothstep(0.02, 0.28, hiddenBlock);\n' +
      '      msWaterFade *= (1.0 - hiddenBlock);\n' +
      '      if (msWaterFade <= 0.001) discard;\n' +
  ...
  update(dt, sceneBoundsVec4) {
    ...
    // Decouple TreeEffect hover/fade state from precipitation roof blocking.
    // Tree canopy overlays are visual-only and should not suppress rain/drips.
    const treeHoverRevealActive = false;
    const hoverRevealActive = !!weatherController?.roofMaskActive;
    const rainRoofBlockTexture = roofBlockTexture;
    ...
    if (shouldBuildDrips && !this._roofDripRebuildJob && (!this._roofDripBasePoints || this._roofDripWarmupState === 'stale')) {
      const sig = this._computeRoofDripSourceSignature();
      this._roofDripBeginPointsRebuild(sig, this._roofDripWarmupState === 'stale' ? 'stale' : 'runtime');
      this._roofDripWarmupState = 'ready';
      this._roofDripPendingStaleReason = null;
    }
    ...
    const activeDripPointCount = (this._roofDripActivePointsRef && this._roofDripActivePointsRef.length >= 5)
      ? Math.floor(this._roofDripActivePointsRef.length / 5)
      : 0;
    ...
    if (this.roofDripSystem) {
      ...
      // Simplified robust route: keep drip masking disabled.
      const dripMaskU = 0.0;
      ...
      if (dripEmission && typeof dripEmission.value === 'number') {
        ...
        if (dripOn && !suppressPrecip) {
          ...
          dripReason = dripRate > 0.001 ? 'emit:active' : 'emit:zero-rate';
        } else if (!dripOn) {
          dripReason = 'emit:disabled';
      ...
      const killMRaw = _roofDripTuningVal('killZMargin', ROOF_DRIP_KILL_Z_MARGIN);
      // Self-recovery visibility guard:
      // In some scenes/camera setups drips were simulated mostly below the visible weather band.
      // Clamp effective kill margin so particles are culled earlier and replenished near spawn.
      const killM = Math.min(
        Math.max(0, Number.isFinite(killMRaw) ? killMRaw : ROOF_DRIP_KILL_Z_MARGIN),
        ROOF_DRIP_KILL_Z_MARGIN_MAX_SAFE
      );
      ...
      const tunSpd = _roofDripTuningVal('speedFactor', 0.02);
      ...
      const lifeA = _roofDripTuningVal('lifeMin', 2.8);
      const lifeB = Math.max(lifeA, _roofDripTuningVal('lifeMax', 5.2));
      ...
      const spA = _roofDripTuningVal('particleSpeedMin', 120);
      const spB = Math.max(spA, _roofDripTuningVal('particleSpeedMax', 260));
      ...
      if (roofDripDiagLogsEnabled()) {
        ...
        if (due) {
          ...
          this._emitRoofDripDiag({
            ...
            roofDrip: {
              poolBuilt: !!(this._roofDripBasePoints && this._roofDripBasePoints.length >= 5),
              poolPointCount: this._roofDripDiagStats?.lastBasePointCount ?? 0,
              eligibleRoofTileCount: this._roofDripDiagStats?.lastTileEligible ?? 0,
              dripEmission: dripEmission?.value ?? null,
              dripParticleNum: this.roofDripSystem.particleNum,
              batchRegistered: !!(this.batchRenderer?.systemToBatchIndex?.has?.(this.roofDripSystem)),
              warning: (dripEmission?.value ?? 0) > 0.001 && activeDripPointCount < 1 ? 'drip_pool_empty' : null,
              edgeSampleCount: ptN
            },
            wiring: {
    ...
    if (this.ashSystem) {
      ...
      const ashMinAlpha = (ashTuning.opacityStartMin ?? 0.58) * ashAlphaScale;
      const ashMaxAlpha = (ashTuning.opacityStartMax ?? 0.82) * ashAlphaScale;
      const cStart = ashTuning.colorStart ?? { r: 0.07, g: 0.055, b: 0.045 };
      const cEnd = ashTuning.colorEnd ?? { r: 0.82, g: 0.8, b: 0.76 };
      ...
      if (this._ashMaterial && THREE) {
        const mt = Number(ashTuning.materialTint);
        const t = (Number.isFinite(mt) ? THREE.MathUtils.clamp(mt, 0.35, 1.5) : 1.0);
        this._ashMaterial.color.setRGB(t, t, t);
        if (this._ashBatchMaterial && this._ashBatchMaterial.color) {
          this._ashBatchMaterial.color.setRGB(t, t, t);
        }
      }

      if (this._ashColorOverLife?.color?.a && this._ashColorOverLife.color?.b) {
        const br = Number(ashTuning.ashLifeBrighten);
        const brighten = (Number.isFinite(br) && br > 0) ? THREE.MathUtils.clamp(br, 0.5, 4.0) : 2.28;
        const fadeA = Number(ashTuning.ashLifeAlphaFade);
        const endMulA = (Number.isFinite(fadeA) ? THREE.MathUtils.clamp(fadeA, 0.05, 1.0) : 0.32);
        this._ashColorOverLife.color.a.set(1.0, 0.99, 0.97, 1.0);
        this._ashColorOverLife.color.b.set(brighten, brighten * 0.97, brighten * 0.9, endMulA);
      }

      // Scale curl noise for ash (strength + spatial scale + time evolution)
      if (this._ashCurl && this._ashCurlBaseStrength) {
        const curlStrength = debugDisableWeatherBehaviors ? 0.0 : (ashTuning.curlStrength ?? 2.6);
        this._ashCurl.strength.copy(this._ashCurlBaseStrength).multiplyScalar(curlStrength);
      }
      if (this._ashCurl && this._ashCurlBaseScale) {
        const ns = Number(ashTuning.curlNoiseScale);
        const mul = (Number.isFinite(ns) && ns > 0) ? ns : 1.0;
        this._ashCurl.scale.copy(this._ashCurlBaseScale).multiplyScalar(mul);
      }
      if (this._ashCurl && typeof this._ashCurlBaseTimeScale === 'number') {
        const ts = Number(ashTuning.curlTimeScale);
        const tmul = (Number.isFinite(ts) && ts > 0) ? ts : 1.0;
        this._ashCurl.timeScale = this._ashCurlBaseTimeScale * tmul;
      }
    ...
    if (this.ashEmberSystem) {
      ...
      if (this._ashEmberCurl && this._ashEmberCurlBaseStrength) {
        const emberCurlStrength = debugDisableWeatherBehaviors ? 0.0 : (ashTuning.emberCurlStrength ?? 4.2);
        this._ashEmberCurl.strength.copy(this._ashEmberCurlBaseStrength).multiplyScalar(emberCurlStrength);
      }
      if (this._ashEmberCurl && this._ashEmberCurlBaseScale) {
        const ns = Number(ashTuning.emberCurlNoiseScale);
        const mul = (Number.isFinite(ns) && ns > 0) ? ns : 1.0;
        this._ashEmberCurl.scale.copy(this._ashEmberCurlBaseScale).multiplyScalar(mul);
      }
      if (this._ashEmberCurl && typeof this._ashEmberCurlBaseTimeScale === 'number') {
        const ts = Number(ashTuning.emberCurlTimeScale);
        const tmul = (Number.isFinite(ts) && ts > 0) ? ts : 1.0;
        this._ashEmberCurl.timeScale = this._ashEmberCurlBaseTimeScale * tmul;
      }
    ...
    if (THREE && (this._rainWindForce || this._roofDripWindForce || this._snowWindForce || this._rainGravityForce || this._roofDripGravityForce || this._snowGravityForce || this._ashWindForce || this._ashEmberWindForce || this._ashGravityForce || this._ashEmberGravityForce)) {
      ...
      if (this._ashWindForce && this._ashWindForce.direction) {
        ...
        if (this._ashWindForce.magnitude && typeof this._ashWindForce.magnitude.value === 'number') {
          const baseMagRaw = Number(ashTuning.ashWindBase);
          const baseMag = (Number.isFinite(baseMagRaw) && baseMagRaw > 0) ? baseMagRaw : 400;
          const align = THREE.MathUtils.clamp(windSpeed, 0, 1);
          const windInfluence = ashTuning.windInfluence ?? 1.85;
      ...
      if (this._ashEmberWindForce && this._ashEmberWindForce.direction) {
        ...
        if (this._ashEmberWindForce.magnitude && typeof this._ashEmberWindForce.magnitude.value === 'number') {
          const baseMagRaw = Number(ashTuning.emberWindBase);
          const baseMag = (Number.isFinite(baseMagRaw) && baseMagRaw > 0) ? baseMagRaw : 650;
          const align = THREE.MathUtils.clamp(windSpeed, 0, 1);
          const windInfluence = ashTuning.emberWindInfluence ?? 0.52;
