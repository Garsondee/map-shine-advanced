...
export class TweakpaneManager {
  ...
  buildSceneSetupSection() {
    ...
    // Save the full MSA config (scene + tiles) into the module's Adventure pack
    // flags so it can be injected back on import without going through the
    // EmbeddedDataField schema that strips third-party flags.
    setupFolder.addButton({
      title: '📦 Save Config to Adventure Pack'
    }).on('click', async () => {
      await this._saveConfigToAdventurePack();
    });

    // Show / copy the current scene's config hash so it can be compared against
    // the token shown at pack-time to confirm the config survived import.
    setupFolder.addButton({
      title: '🔍 Show Current Config Hash'
    }).on('click', async () => {
      await this._showCurrentConfigHash();
    });
  ...
  /**
   * Return the list of Adventure packs that come from installed modules.
   * Each entry can be passed to saveSceneConfigToAdventurePack.
   * @returns {Array<{id: string, label: string, moduleId: string}>}
   * @private
   */
  _getModuleAdventurePacks() {
    const results = [];
    for (const pack of (game.packs ?? [])) {
      if (pack.metadata.packageType !== 'module') continue;
      if (pack.metadata.type !== 'Adventure') continue;
      const packKey = pack.collection ?? pack.metadata?.id ?? pack.id;
      if (!packKey) continue;
      results.push({
        id: packKey,
        label: `${pack.metadata.packageName} — ${pack.metadata.label ?? pack.metadata.id}`,
        moduleId: pack.metadata.packageName
      });
    }
    return results;
  }
  ...
  /**
   * Orchestrate saving the current scene's MSA config into an Adventure pack.
   * Prompts for pack selection if multiple packs exist, then shows the hash token.
   * @private
   */
  async _saveConfigToAdventurePack() {
    if (!game?.user?.isGM) {
      ui.notifications?.warn?.('Map Shine: Only the GM can save config to a pack.');
      return;
    }
    ...
    const packs = this._getModuleAdventurePacks();
    if (packs.length === 0) {
      ui.notifications?.warn?.('Map Shine: No Adventure packs found from installed modules. Make sure your map module is installed and active.');
      return;
    }

    // Resolve the target pack — auto-select if only one, otherwise prompt.
    let packId;
    if (packs.length === 1) {
      packId = packs[0].id;
    } else {
      packId = await new Promise((resolve) => {
        const buttons = {};
        for (const p of packs) {
          buttons[p.id] = { label: p.label, callback: () => resolve(p.id) };
        }
        buttons._cancel = { label: 'Cancel', callback: () => resolve(null) };
        new Dialog({
          title: 'Select Adventure Pack',
          content: '<p>Multiple Adventure packs found. Select the one to save the MSA config into:</p>',
          buttons,
          default: '_cancel'
        }).render(true);
      });
    ...
    try {
      ui.notifications?.info?.('Map Shine: Saving config to Adventure pack...');
      const saveFn = window.MapShine?.saveSceneConfigToAdventurePack;
      if (typeof saveFn !== 'function') {
        throw new Error('saveSceneConfigToAdventurePack not available — module may not be fully initialised.');
      }
      if (!packId) {
        throw new Error('Adventure pack selection failed (no valid pack id).');
      }
      const result = await saveFn(packId);
      if (!result) return;

      const { hash } = result;
      try { await navigator.clipboard.writeText(hash); } catch (_) {}

      // Show a styled result dialog with the hash token.
      new Dialog({
        title: 'MSA Config Saved ✓',
        content: `
          <div style="font-family:monospace;padding:8px">
            <p>Config saved to Adventure pack successfully.</p>
            <p><b>Config Hash Token:</b></p>
            <div style="background:#1a1a2e;color:#00ff88;padding:12px;border-radius:6px;font-size:20px;text-align:center;letter-spacing:6px;margin:8px 0">
              ${hash}
            </div>
            <p style="font-size:11px;color:#888">
              Hash copied to clipboard.<br>
              After customers import this map, ask them to run
              <b>🔍 Show Current Config Hash</b> and confirm it matches <b>${hash}</b>.
            </p>
          </div>`,
      ...
      log.error('Failed to save MSA config to Adventure pack:', e);
      ui.notifications?.error?.(`Map Shine: ${e.message ?? 'Failed to save config.'}`);
    }
  }

  /**
   * Compute and display the hash of the current scene's MSA config.
   * Use this after import to verify the config matches the author's token.
   * @private
   */
  async _showCurrentConfigHash() {
    ...
    const msaFlags = scene.flags?.[NS];
    if (!msaFlags || typeof msaFlags !== 'object') {
      ui.notifications?.warn?.('Map Shine: No MSA config found on this scene.');
      return;
    }

    // Build the same config object that the author's save function would hash.
    const cleanFlags = Object.assign({}, msaFlags);
    ...
    const hash = await msaComputeHash(configObj);
    const expected = msaFlags._expectedHash ?? null;

    let matchHTML;
    if (!expected) {
      matchHTML = '<span style="color:#aaa">No expected hash stored on this scene (config was not saved to a pack yet).</span>';
    } else if (expected === hash) {
      matchHTML = `<span style="color:#00ff88">✓ MATCH — config matches author token <b>${expected}</b></span>`;
    } else {
      matchHTML = `<span style="color:#ff4444">✗ MISMATCH — expected <b>${expected}</b></span>`;
    }

    try { await navigator.clipboard.writeText(hash); } catch (_) {}

    new Dialog({
      title: 'Current MSA Config Hash',
      content: `
        <div style="font-family:monospace;padding:8px">
          <p><b>Current Config Hash:</b></p>
          <div style="background:#1a1a2e;color:#00ff88;padding:12px;border-radius:6px;font-size:20px;text-align:center;letter-spacing:6px;margin:8px 0">
            ${hash}
          </div>
          <p>${matchHTML}</p>
          <p style="font-size:11px;color:#888">Hash copied to clipboard.</p>
        </div>`,
      buttons: { ok: { label: 'OK', callback: () => {} } },
      default: 'ok'
    }).render(true);
