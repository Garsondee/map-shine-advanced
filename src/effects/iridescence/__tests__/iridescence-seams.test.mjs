/**
 * iridescence-seams.test.mjs — `getIridescenceMaskItems`, mythica-machina-press#136.
 *
 * Mirrors `prism-seams.test.mjs`'s own job, one effect later: prove a
 * Tile-authored `_Iridescence` mask is discovered, that a hidden/wrong-floor/
 * unresolved-art tile is excluded, and that unwired optional seam inputs fail
 * safe rather than throwing.
 */
import { createIridescenceSeams } from '../iridescence-seams.js';
...
export function run(t) {
  ...
  const items = [
    ...
    // A level's own background carries an `_Iridescence` file too, but
    // Iridescence v1 has no floor-level door at all (this module's own
    // header) — it must never surface here just because it happens to share
    // an id shape with specular's own excluded case.
    { id: 'level:levelA:background', kind: 'levelBackground', levelId: 'levelA', hidden: false },
  ];

  const authoredByItemId = {
    tileA: 'mask://tileA_Iridescence.webp',
    tileC: 'mask://tileC_Iridescence.webp',
    tileD: 'mask://tileD_Iridescence.webp',
    tileE: 'mask://tileE_Iridescence.webp',
    'level:levelA:background': 'mask://levelA_Iridescence.webp',
  };
  ...
  const floor0Items = getIridescenceMaskItems(0);
  ...
  ok('its url is the discovered file, not fabricated', only?.url === 'mask://tileA_Iridescence.webp');
  ...
  ok(
    'the level BACKGROUND item never appears here — Iridescence v1 has no floor-level door at all',
  ...
  const floor1Items = getIridescenceMaskItems(1);
  ok('floor 1’s own tile is found independently of floor 0', floor1Items.length === 1 && floor1Items[0].id === 'tileC');

  ok('an unresolved floor index returns an empty list, never a throw', getIridescenceMaskItems(99).length === 0);

  // ── UNWIRED SEAM INPUTS FAIL SAFE, NOT LOUD ─────────────────────────────
  const bare = createIridescenceSeams({
    maskAuthority: stubMaskAuthority(authoredByItemId),
    getFloors: () => [FLOOR0],
  });
  ok(
    'with no getItems/getItemCorners wired, getIridescenceMaskItems returns [] rather than throwing',
    bare.getIridescenceMaskItems(0).length === 0
  );
}
