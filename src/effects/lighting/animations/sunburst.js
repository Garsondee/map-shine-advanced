...
/** `pow(max(light, mirrorTriangle(beam)), 3)` — the shared ray-burst pattern, coefficients supplied per channel. */
function sunburstPattern(TSL, dist, time, uIntensityRaw, lpulseHigh, lpulseLow) {
  const { float, atan, fract, pow, abs, max, positionLocal } = TSL;
 ...
 * @returns {{finalColor: *}}
 */
export function buildSunburstIlluminationSeed({ THREE, dist, defaultSeed, time, uIntensityRaw }) {
  const { float } = THREE.TSL;
  const pattern = sunburstPattern(THREE.TSL, dist, time, uIntensityRaw, float(1.3), float(0.85));
 ...
 * @returns {{finalColor: *}}
 */
export function buildSunburstColorationSeed({ THREE, uLightColor, uColorationAlpha, dist, time, uIntensityRaw }) {
  const { float } = THREE.TSL;
  const pattern = sunburstPattern(THREE.TSL, dist, time, uIntensityRaw, float(1.1), float(0.85));
