...
export function groupRopeChainAnchorsIntoSources(anchors) {
  ...
  for (const [linkId, bucket] of byLink) {
    if (bucket.start && bucket.end) {
      ...
      // mythica-machina-press#576 — same gap as groupLightningAnchorsIntoSources'
      // own (this function mirrors it structurally): a valid pair forming a
      // source does NOT mean every anchor on this linkId was consumed. A
      // second anchor claiming an already-taken start/end role lands in
      // `waypoints` (never overwriting the first), and without this check it
      // vanished from both `sources` and `orphaned` — inert, no diagnostic
      // trail. Only a genuine duplicate-role claim is reported here; a real
      // `role:'waypoint'` entry is left alone, matching the else branch below.
