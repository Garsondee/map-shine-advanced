/**
 * window-seams.test.mjs — `getWindowMaskItems`, mythica-machina-press#538.
 *
 * The regression this file exists to catch: `getWindowMaskUrl` (this same
 ...
 * diagnosed and fixed for Fluid, one effect earlier. `getWindowMaskItems` is
 * Window's own fix; this proves a Tile-authored mask is discovered where the
 * floor-only door would have missed it, that a floor's OWN level-background
 * item is deliberately excluded (it stays on the existing door, so a floor
 * with `_Window` painted into its background art is never double-drawn), and
 * that the usual item-population edge cases (hidden, wrong floor, art not
 * yet resolved) are all honoured.
 */
import { createWindowSeams } from '../window-seams.js';
...
export function run(t) {
  ...
  const items = [
    // The floor's OWN background — has a `_Window` file, but must stay on
    // `getWindowMaskUrl`'s own door, never appear here too.
  ...
  const authoredByItemId = {
    'level:levelA:background': 'mask://levelA_Window.webp',
    tileA: 'mask://tileA_Window.webp',
    tileC: 'mask://tileC_Window.webp',
    tileD: 'mask://tileD_Window.webp',
    tileE: 'mask://tileE_Window.webp',
  };
  ...
  const { getWindowMaskItems } = createWindowSeams({
  ...
  const floor0Items = getWindowMaskItems(0);
  ...
  ok('its url is the discovered file, not fabricated', only?.url === 'mask://tileA_Window.webp');
  ...
  ok(
    'the level BACKGROUND item never appears here, even though it has an authored file — ' +
      'it stays on getWindowMaskUrl’s own door so a floor is never double-drawn',
  ...
  const floor1Items = getWindowMaskItems(1);
  ok('floor 1’s own tile is found independently of floor 0', floor1Items.length === 1 && floor1Items[0].id === 'tileC');

  ok('an unresolved floor index returns an empty list, never a throw', getWindowMaskItems(99).length === 0);
  ...
  const bare = createWindowSeams({ maskAuthority: stubMaskAuthority(authoredByItemId), getFloors: () => [FLOOR0] });
  ok(
    'with no getItems/getItemCorners wired, getWindowMaskItems returns [] rather than throwing',
    bare.getWindowMaskItems(0).length === 0
  );
}
