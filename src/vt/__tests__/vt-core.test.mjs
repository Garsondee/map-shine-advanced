...
export function run(t) {
  ...
  // --- PageTable: anchor against Keyhole.md's own stated number -----------
  {
    ...
    ok('table: 12000/248 world = 49x49 pages at mip0 (Keyhole.md §4.1 exact figure)',
      table.pagesPerAxis(0) === 49);
    ok('table: payload defaults to 248 (256px page - 4px border x2)',
  ...
  // --- PageTable: key identity + clamping + coarse fallback ---------------
  {
    ...
    ok('table: finestResident returns null when nothing is resident anywhere', table.finestResident(0, 40, 40) === null);
  }
  ...
  // --- PageCache: THE core guarantee — capacity never grows ---------------
  {
    ...
    ok('cache: 5000 distinct requests against a 16-page budget NEVER exceeds capacity',
  ...
  // --- PageCache: LRU evicts the least-recently-used UNPINNED slot --------
  {
    ...
    cache.tick(); cache.request('a');
    cache.tick(); cache.request('b');
    cache.tick(); cache.request('c');
  ...
  // --- PageCache: pin classes are never evicted while pinned --------------
  {
    ...
    cache.tick(); cache.request('coarseA', { pin: 'coarse' });
    cache.tick(); cache.request('viewB', { pin: 'view' });
    ...
    ok('cache: request when all slots pinned is a clean miss, not a throw', res.resident === false && res.slot === null);
    ...
    ok('cache: after unpin, the previously-view-pinned slot is now evictable', res2.resident === true && res2.evictedKey === 'viewB');
  ...
  // --- residency: computeVisiblePages basic range + guard ring ------------
  {
    ...
    ok('residency: a single-page rect with no guard yields exactly 1 page', noGuard.length === 1 && noGuard[0].px === 10 && noGuard[0].py === 10);
  ...
  // --- residency: clamps to grid bounds at world edges (no negative/OOB) ---
  {
    ...
    ok('residency: near-origin rect clamps to px/py >= 0 (no negative page coords)',
    ...
    ok('residency: far-edge rect clamps to px/py <= n-1 (no OOB page coords)',
  ...
  // --- residency: chooseMip picks finer detail for a tighter zoom ---------
  {
    ...
    ok('residency: zoomed-out (whole 12000 world in 1920px) picks a coarser mip than zoomed-in',
      wideMip > tightMip);
  ...
  // --- residency: planResidency bundles fine + coarser prefetch -----------
  {
    ...
    ok('residency: planResidency returns a mip + fine set + coarser prefetch set',
      typeof plan.mip === 'number' && Array.isArray(plan.fine) && Array.isArray(plan.prefetchCoarser));
    ok('residency: prefetch set uses a coarser (>=) mip than fine', plan.prefetchCoarser.length === 0 || plan.prefetchCoarser[0].mip > plan.fine[0].mip);
  }
