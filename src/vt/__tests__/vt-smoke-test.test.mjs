...
export function run(t) {
  ...
  ok(
    'not resident -> all-zero (the GLSL decode reads a>0.5 as the residency test)',
    JSON.stringify(encodeIndirectionTexel(999, false)) === JSON.stringify([0, 0, 0, 0])
  );

  ok(
    'slot 0, resident -> [0,0,0,255]',
    JSON.stringify(encodeIndirectionTexel(0, true)) === JSON.stringify([0, 0, 0, 255])
  );
  ok(
    'slot 255, resident -> low byte maxed, high byte 0',
    JSON.stringify(encodeIndirectionTexel(255, true)) === JSON.stringify([255, 0, 0, 255])
  );
  ok(
    'slot 256, resident -> low byte wraps to 0, high byte 1 (matches GLSL: r + g*256 decode)',
    JSON.stringify(encodeIndirectionTexel(256, true)) === JSON.stringify([0, 1, 0, 255])
  );
  ok(
    'slot 2047 (max at the 512MB/8GB-tier default capacity), resident',
