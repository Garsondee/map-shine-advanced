/** V4-Testament S2.1 — capture a canvas snapshot for the shared-shading-core
 * refactor's byte-identical gate.
 *
 * UNLIKE S1.5's pixel-diff (a live flag flip within one session, comparing
 * two states of the SAME code), S2.1 has no flag: it is a pure refactor of
 * the SAME always-active code path, so "before" and "after" are necessarily
 * two separate process runs against two different git states of
 * point-light-illumination.js / point-light-coloration.js. This script
 * captures ONE labelled snapshot; run it once before the refactor
 * (`node ... baseline`), once after (`node ... after`), then diff the two
 * saved JSON payloads with s2-1-diff.mjs.
 *
 * Same rigor as S1.5's capture: real map-art load wait, switch to First
 * Floor (candles + Foundry lights both visible there), freeze time (fire/
 * candle/wind animation would otherwise differ between two runs seconds or
 * minutes apart for reasons unrelated to the refactor), non-vacuity via
 * `vt-pan-viewer-diagnostics`'s own `pointLights` report field.
 *
 * usage: node tests/playwright-artifacts/look/s2-1-capture.mjs <baseline|after>
 */
...
if (label !== 'baseline' && label !== 'after') {
  console.error('usage: node s2-1-capture.mjs <baseline|after>');
  process.exit(1);
}
...
// reasoning as stage1-earlyz-pixel-diff.mjs's own identical note).
...
console.log(`[s2-1-capture:${label}] waiting for real map art…`);
await waitForMapArtLoaded(600000);
await waitForSceneSettled(page, { label: 'ground floor' });

// FIRST FLOOR — candles (207 anchors, S2.0 census) and Foundry lights (50,
// all `flame`-animated) are both present there, exercising both the
// illumination AND coloration shading cores, animated AND (via the flame
// lights) the general seeded-animation path.
...
console.log(`[s2-1-capture:${label}] floor switch:`, JSON.stringify(clicked));
...
console.log(`[s2-1-capture:${label}] time freeze:`, JSON.stringify(frozen));
...
async function grabCanvas() {
  ...
  console.log(`[s2-1-capture:${label}] captured: ${data.w}x${data.h}`);
  return data;
}
...
console.log(`[s2-1-capture:${label}] pointLights (non-vacuity):`, JSON.stringify(pointLights));

const frame = await grabCanvas();
await page
  .locator('canvas#msa-vt-pan-viewer-canvas')
  .screenshot({ path: path.join(OUT_DIR, `s2-1-${label}.png`) })
  .catch(() => {});

fs.mkdirSync(OUT_DIR, { recursive: true });
fs.writeFileSync(
  path.join(OUT_DIR, `s2-1-${label}.json`),
  JSON.stringify({
    label,
    capturedAt: Date.now(),
    floorInfo,
...
console.log(`[s2-1-capture:${label}] wrote s2-1-${label}.json (${frame.w}x${frame.h})`);
